...
NN_TensorDesc* createTensorDesc(const int32_t* shape, size_t shapeNum, OH_NN_DataType dataType, OH_NN_Format format)
{
    ...
    if (tensorDescTmp == nullptr) {
        OHPrintf("[NNRtTest]OH_NNTensorDesc_Create failed!");
        return nullptr;
    }

    OH_NN_ReturnCode ret = OH_NNTensorDesc_SetDataType(tensorDescTmp, dataType);
    if (ret != OH_NN_SUCCESS) {
        OHPrintf("[NNRtTest]OH_NNTensorDesc_SetDataType failed!ret = %d\n", ret);
        return nullptr;
    }
    ...
    if (shape != nullptr) {
        ...
        if (ret != OH_NN_SUCCESS) {
            OHPrintf("[NNRtTest]OH_NNTensorDesc_SetShape failed!ret = %d\n", ret);
            return nullptr;
        }
    }
...
int SingleModelBuildEndStep(OH_NNModel* model, const OHNNGraphArgs& graphArgs)
{
    ...
    if (graphArgs.addOperation) {
        ...
        if (ret != OH_NN_SUCCESS) {
            OHPrintf("[NNRtTest] OH_NNModel_AddOperation failed! ret=%{public}d\n", ret);
            return ret;
        }
    }
    ...
    if (graphArgs.specifyIO) {
        ...
        if (ret != OH_NN_SUCCESS) {
            OHPrintf("[NNRtTest] OH_NNModel_SpecifyInputsAndOutputs failed! ret=%{public}d\n", ret);
            return ret;
        }
    }
...
int BuildSingleOpGraph(OH_NNModel* model, const OHNNGraphArgs& graphArgs)
{
    ...
    for (size_t i = 0; i < graphArgs.operands.size(); i++) {
        ...
        NN_TensorDesc* tensorDesc = createTensorDesc(
        ...
        if (ret != OH_NN_SUCCESS) {
            OHPrintf("[NNRtTest] OH_NNBackend_SetModelTensorType failed! ret=%d\n", ret);
            return ret;
        }
        ...
        if (std::find(graphArgs.paramIndices.begin(), graphArgs.paramIndices.end(), i) !=
            ...
            if (ret != OH_NN_SUCCESS) {
                OHPrintf("[NNRtTest] OH_NNModel_SetTensorData failed! ret=%{public}d\n", ret);
                return ret;
            }
        }
...
OH_NN_ReturnCode GetDeviceID(size_t* deviceId)
{
    ...
    if (devicesCount <= NO_DEVICE_COUNT) {
        OHPrintf("[NNRtTest] devicesCount <= 0  devicesCount=%d\n", devicesCount);
        return OH_NN_FAILED;
    }
    ...
    for (uint32_t i = 0; i < devicesCount; i++) {
        ...
        if (ret != OH_NN_SUCCESS) {
            OHPrintf("[NNRtTest] OH_NNDevice_GetName failed! ret=%d\n", ret);
            return ret;
        }
...
bool IsSupportNPU()
{
    static std::string nnrtLibraryName = "libneural_network_runtime_ext.so";
    void* handle = dlopen(nnrtLibraryName.c_str(), RTLD_LAZY | RTLD_NODELETE);
    if (handle == nullptr) {
        OHPrintf("is not support NPU");
        return false;
    }
    return true;
}
