...
void EGLRender::createStar() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        starVertices.push_back(inner * cosf(-PI * 0.5f - pentaAngle * 2.f - pentaAngle * i + angle) * ratio);
        starVertices.push_back(inner * sinf(-PI * 0.5f - pentaAngle * 2.f - pentaAngle * i + angle));
        starVertices.push_back(outer * cosf(PI * 0.5f - pentaAngle * i + angle) * ratio);
        starVertices.push_back(outer * sinf(PI * 0.5f - pentaAngle * i + angle) );
        starVertices.push_back(inner * cosf(-PI * 0.5f - pentaAngle * 3.f - pentaAngle * i + angle) * ratio);
        starVertices.push_back(inner * sinf(-PI * 0.5f - pentaAngle * 3.f - pentaAngle * i + angle));
    }
}
...
void EGLRender::DrawBackground(const GLfloat* color) {
    OH_LOG_Print(LOG_APP, LOG_ERROR, 0xff00, "EGLCore", "wds DrawBackground");
    eglMakeCurrent(eglDisplay_, eglSurface_, eglSurface_, eglContext_);
...
void EGLRender::DrawTriangleSP() {
    eglMakeCurrent(eglDisplay_, eglSurface_, eglSurface_, eglContext_);
    glViewport(0, 0, width_, height_);
    glClearColor(0.9, 0.9, 0.9, 1);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glUseProgram(program_);
    const GLfloat color[] = {226.0f / 255, 43.0f / 255, 151.0f / 255, 1.0f};
    drawTriangleFan(starVertices.data(), starVertices.size());
    glFlush();
    glFinish();
    float ratio = 0.5 * height_ / width_ ;
    ...
    glFinish();
    auto ret = eglSwapBuffers(eglDisplay_, eglSurface_);
    OH_LOG_Print(LOG_APP, LOG_ERROR, 0xff00, "XComponent_Native", "CYC_draw ret %{public}d", ret);
}

void EGLRender::DrawStarSP() {
    eglMakeCurrent(eglDisplay_, eglSurface_, eglSurface_, eglContext_);
    glViewport(0, 0, width_, height_);
    glClearColor(0.9, 0.9, 0.9, 1);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glUseProgram(program_);
    const GLfloat color[] = {0.0f / 255, 143.0f / 255, 251.0f / 255, 1.0f};
    drawTriangleFan(starVertices.data(), starVertices.size());
    glFlush();
    glFinish();
    float ratio = 0.5 * height_ / width_ ;
