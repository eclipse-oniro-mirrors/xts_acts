...
private:
    ...
    static std::shared_ptr<ArkUIAniTest::ColumnComponent> CreateWaterFlowByNestedScroll(
        int32_t forward, int32_t backward, bool up = true)
    {
        auto outerColumn = std::make_shared<ArkUIAniTest::ColumnComponent>();
        auto scroll = std::make_shared<ArkUIAniTest::ScrollComponent>();
        auto column = std::make_shared<ArkUIAniTest::ColumnComponent>();
        auto waterFlow = std::make_shared<ArkUIAniTest::WaterFlowComponent>();
        auto input = std::vector<std::shared_ptr<Component>>();
        auto adapter = std::make_shared<ItemAdapter<FlowItemComponent>>(input);
        auto reserveTop = std::make_shared<ArkUIAniTest::ColumnComponent>();
        auto reserveDown = std::make_shared<ArkUIAniTest::ColumnComponent>();
        bool isWaterFlowBackToTop = true;
        auto button2 = std::make_shared<ArkUIAniTest::ButtonComponent>();
        ...
        for (int i = 0; i < size; i++) {
            auto col = std::make_shared<ArkUIAniTest::ColumnComponent>();
        ...
        if (up) {
            ...
            OH_LOG_Print(LOG_APP, LOG_INFO, ArkUIAniTest::LOG_PRINT_DOMAIN, "WaterFlowEventTest", "OnReachStart");
