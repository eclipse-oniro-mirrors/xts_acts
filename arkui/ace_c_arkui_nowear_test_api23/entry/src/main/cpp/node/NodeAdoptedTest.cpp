...
#include "NodeAdoptedTest.h"
#include <arkui/native_type.h>
#include <arkui/native_node_napi.h>

const unsigned int LOG_PRINT_DOMAIN = 0xFF00;
static ArkUI_NativeNodeAPI_1* nodeAPI;
static ArkUI_NodeContentHandle nodeContentHandle_ = nullptr;
...
ArkUI_NodeHandle CreateColumnMatchParentNodeHandle(const std::string &tag)
{
    ...
    ArkUI_NodeHandle btn1 = nodeAPI->createNode(ARKUI_NODE_BUTTON);
    OH_ArkUI_NativeModule_AdoptChild(column, btn1);
    ArkUI_AttributeItem id_item = {};
    ArkUI_NodeHandle text1 = nodeAPI->createNode(ARKUI_NODE_TEXT);
    id_item.string = "5";
    nodeAPI->setAttribute(text1, NODE_TEXT_CONTENT, &id_item);
    ArkUI_AttributeItem textId1 = {.string = "btn_test1"};
    nodeAPI->setAttribute(text1, NODE_ID, &textId1);

    ArkUI_NodeHandle text2 = nodeAPI->createNode(ARKUI_NODE_TEXT);
    id_item.string = "19";
    nodeAPI->setAttribute(text2, NODE_TEXT_CONTENT, &id_item);
    ArkUI_AttributeItem textId2 = {.string = "btn_test2"};
    ...
    auto result1 = OH_ArkUI_NodeContent_AddNode(nodeContentHandle_, btn1);
    if (result1 == ARKUI_ERROR_CODE_NODE_IS_ADOPTED) {
        nodeAPI->setAttribute(text1, NODE_BACKGROUND_COLOR, &background_color_item);
    }
    auto result2 = OH_ArkUI_NodeContent_InsertNode(nodeContentHandle_, btn1, -1);
    if (result2 == ARKUI_ERROR_CODE_NODE_IS_ADOPTED) {
...
napi_value NodeAdoptedTest::testAddNodeInsertNode(napi_env env, napi_callback_info info)
{
