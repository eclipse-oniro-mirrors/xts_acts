...
#include "scroll_onscrollframebegin_test.h"
#include <string>

#define ON_SCROLLFRAMEBEGIN_EVENT_ID 6007
#define SIZE_120 120
...
static ArkUI_NodeHandle CreateChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI)
{
    ...
    nodeAPI->registerNodeEvent(scroll, NODE_SCROLL_EVENT_ON_SCROLL_FRAME_BEGIN, ON_SCROLLFRAMEBEGIN_EVENT_ID, nullptr);
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ScrollOnScrollFrameBeginTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ScrollOnScrollFrameBeginTest",
    ...
    if (eventId == ON_SCROLLFRAMEBEGIN_EVENT_ID) {
...
napi_value ScrollOnScrollFrameBeginTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ScrollOnScrollFrameBeginTest", "CreateNativeNode");
