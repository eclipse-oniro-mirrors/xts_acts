...
#include "scroll_onscrollstart_test.h"
#include <string>

#define ON_SCROLLSTART_EVENT_ID 6007
#define SIZE_120 120
...
static ArkUI_NodeHandle CreateChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI)
{
    ...
    ArkUI_NumberValue button_length_value[] = {{.f32 = SIZE_120}};
    ...
    nodeAPI->registerNodeEvent(scroll, NODE_SCROLL_EVENT_ON_SCROLL_START, ON_SCROLLSTART_EVENT_ID, nullptr);
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ScrollOnScrollStartTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ScrollOnScrollStartTest", "OnEventReceive: event is null");
        return;
    }
    ...
    if (eventId == ON_SCROLLSTART_EVENT_ID) {
...
napi_value ScrollOnScrollStartTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ScrollOnScrollStartTest", "CreateNativeNode");
    ...
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ScrollOnScrollStartTest", "GetContext env or info is null");
        return nullptr;
    }
    ...
    if (OH_NativeXComponent_AttachNativeRootNode(PluginManager::GetInstance()->GetNativeXComponent(id), rootColumn) ==
        INVALID_PARAM) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ScrollOnScrollStartTest",
