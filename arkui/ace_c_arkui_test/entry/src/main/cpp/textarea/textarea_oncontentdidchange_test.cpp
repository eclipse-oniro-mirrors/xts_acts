...
#include "textarea_oncontentdidchange_test.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TextAreaOnContentDidChangeTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(
            LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextAreaOnContentDidChangeTest", "OnEventReceive: event is null");
        return;
    }
    ...
    switch (eventId) {
        ...
        case ON_CLICK_EVENT_ID: {
            ...
            auto ret = nodeAPI->setAttribute(textArea, NODE_FONT_SIZE, &valueItem);
            g_fontSize = nodeAPI->getAttribute(textArea, NODE_FONT_SIZE)->value[PARAM_0].f32;
            return;
        }
        default:
            break;
...
napi_value TextAreaOnContentDidChangeTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    size_t argc = PARAM_1;
    ...
    if ((env == nullptr) || (info == nullptr)) {
        OH_LOG_Print(
            LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextAreaOnContentDidChangeTest", "GetContext env or info is null");
        return nullptr;
    }
    ArkUI_NativeNodeAPI_1* nodeAPI = nullptr;
    OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);
    auto area = CreateChildNode(nodeAPI);
    textArea = area;
    ...
    id_item1.string = "TextAreaOnContentDidChange_buttonId1";
    nodeAPI->setAttribute(button, NODE_ID, &id_item1);
    id_item1.string = "OnContentDidChangeTestTextArea";
    nodeAPI->setAttribute(textArea, NODE_ID, &id_item1);
    ...
    nodeAPI->registerNodeEvent(
        textArea, NODE_TEXT_AREA_ON_CONTENT_SIZE_CHANGE, ON_TEXT_AREA_CONTENT_DID_CHANGE_ID, nullptr);
