...
#include "customcomponent_enabled_test.h"
#include <string>
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
{
    ...
    ArkUI_NumberValue width_value[] = {{.f32 = PARAM_0_POINT_2}};
    ArkUI_AttributeItem width_item = {width_value, sizeof(width_value) / sizeof(ArkUI_NumberValue)};
    nodeAPI->setAttribute(nodeHandle, NODE_WIDTH_PERCENT, &width_item);
    nodeAPI->setAttribute(nodeHandle, NODE_HEIGHT_PERCENT, &width_item);
...
napi_value CustomComponentEnabledTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    ...
    id_item.string = "EnabledTestCustomComponent";
    nodeAPI->setAttribute(custom, NODE_ID, &id_item);
    id_item.string = "EnabledTestCustomComponentCompare";
    nodeAPI->setAttribute(customCompare, NODE_ID, &id_item);
    id_item.string = "EnabledTestCustomComponentDisable";
