...
#ifndef ARKUI_CAPI_XTS_WATERFLOW_EVENT_TEST_H
#define ARKUI_CAPI_XTS_WATERFLOW_EVENT_TEST_H
...
constexpr int32_t EVENT_FLOW_ITEM_COUNT = PARAM_15;

class WaterFlowEventTest {
public:
    ~WaterFlowEventTest() = default;
    static napi_value CreateNativeNodeReachStart(napi_env env, napi_callback_info info);
    static napi_value CreateNativeNodeReachEnd(napi_env env, napi_callback_info info);
    static napi_value CreateNativeNodeScrollStop(napi_env env, napi_callback_info info);
...
private:
    template<int32_t size = EVENT_FLOW_ITEM_COUNT, typename... Args>
...
#endif // ARKUI_CAPI_XTS_WATERFLOW_EVENT_TEST_H