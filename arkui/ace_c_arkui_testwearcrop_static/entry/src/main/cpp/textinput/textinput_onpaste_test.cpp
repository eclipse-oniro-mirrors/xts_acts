...
#include "textinput_onpaste_test.h"
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI)
{
    ...
    nodeAPI->registerNodeEvent(nodeHandle, NODE_TEXT_INPUT_ON_PASTE, ON_PASTE_EVENT_ID, nullptr);
...
static void OnEventReceive(ArkUI_NodeEvent *event)
{
    ...
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TextInputOnPasteTest", "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeEvent_GetTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TextInputOnPasteTest", "OnEventReceive eventId: %{public}d",
                 eventId);
    ...
    if (eventId == ON_PASTE_EVENT_ID) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TextInputOnPasteTest", "OnEventReceive pStr: %{public}s",
                     result->pStr);
        if (::strcmp(result->pStr, TEXT_INPUT_ON_PASTE_TEST_STRING) == 0) {
...
void TextInputOnPasteTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    OH_LOG_INFO(LOG_APP, " [cf]TextInputOnPasteTest::CreateNativeNode start -> 66");
    //获取节点的前置框架代码，直接写就行
    ArkUI_NativeNodeAPI_1 *nodeAPI = nullptr;
    OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);
    ...
    id_item.string = "OnPasteTestTextInput";
    nodeAPI->setAttribute(textInput, NODE_ID, &id_item);
    id_item.string = "OnPasteTestTextInputCompare";
    nodeAPI->setAttribute(textInputDisable, NODE_ID, &id_item);
