...
{
    // 子线程创建组件
    auto xc = nodeAPI->createNode(ARKUI_NODE_XCOMPONENT_TEXTURE);
    // 需要手动保存到nodeList中，nodeList中的组件会由框架负责挂载到主树上
    nodeList.push_back(xc);
    // 子线程写属性
    // XComponent组件属性--id
    SetNodeId(nodeAPI, xc, "xcomponent_texture_success");
    ArkUI_AttributeItem id_item = {};
    std::string text = "xcomponent_texture";
    ...
    if (idGet != text) {
        SetNodeId(nodeAPI, xc, "xcomponent_texture_failed_get_id_multithread");
    }
    ...
    if (size->value[0].i32 != 0 || size->value[1].i32 != 0) {
        SetNodeId(nodeAPI, xc, "xcomponent_texture_failed_get_surfacesize_multithread");
    }
    ...
    if (size->value[0].i32 != 0 || size->value[1].i32 != 0 || size->value[PARAM_2].i32 != 0 || size->value[PARAM_3].i32 != 0) {
        SetNodeId(nodeAPI, xc, "xcomponent_texture_failed_get_surfacerect_multithread");
    }
    const ArkUI_AttributeItem* type = nodeAPI->getAttribute(xc, NODE_XCOMPONENT_TYPE);
    if (type->value[0].i32 != ARKUI_XCOMPONENT_TYPE_TEXTURE) {
        SetNodeId(nodeAPI, xc, "xcomponent_texture_failed_get_type_multithread");
    }
    const ArkUI_AttributeItem* enable = nodeAPI->getAttribute(xc, NODE_XCOMPONENT_ENABLE_ANALYZER);
    if (type->value[0].i32 != 1) {
        SetNodeId(nodeAPI, xc, "xcomponent_texture_failed_get_enableanalyzer_multithread");
    }
}
...
napi_value XComponentTextureTest::DisposeNativeNode(napi_env env, napi_callback_info info)
{
    // 释放组件
    return tester->DisposeCNodeTree(env, info);
}

napi_value XComponentTextureTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
