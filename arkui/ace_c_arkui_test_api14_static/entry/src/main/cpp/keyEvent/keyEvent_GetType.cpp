...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "KeyEventGetType", "CreateNativeNode");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "KeyEventGetType", "KeyEventGetType: event is null");
        return;
    }
    auto get_ArkuI_UIInputEvent = OH_ArkUI_NodeEvent_GetInputEvent(event);

    auto type = OH_ArkUI_KeyEvent_GetType(get_ArkuI_UIInputEvent);
    ...
    if (type != -1) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "KeyEventGetType",
                     "KeyEventGetType result : %{public}d", type);
    }
}

void KeyEventGetTypeTest::KeyEventGetTypeTest_001(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    id_item.string = "KeyEventGetType";
