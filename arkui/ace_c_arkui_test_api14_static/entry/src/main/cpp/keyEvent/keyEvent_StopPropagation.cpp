...
#include "keyEvent_StopPropagation.h"
...
static void OnEventReceive(ArkUI_NodeEvent *event)
{
    ...
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "KeyEventStopPropagation", "OnEventReceive: event is null");
        return;
    }
...
void KeyEventStopPropagationTest::KeyEventStopPropagationTest_001(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    auto button = createChildNode(nodeAPI);
     ArkUI_AttributeItem textId = {.string = "KeyEventStopPropagation"};
