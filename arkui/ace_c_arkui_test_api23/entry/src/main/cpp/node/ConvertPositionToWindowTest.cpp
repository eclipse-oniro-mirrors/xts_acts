...
#include "ConvertPositionToWindowTest.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TestConvertPositionToWindow001", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "TestConvertPositionToWindow001",
    ...
    if (targetId == 0) {
        ...
        auto result1 = OH_ArkUI_NativeModule_ConvertPositionToWindow(node, position2, &pos1);
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager",
                     "kkk  OH_ArkUI_NativeModule_ConvertPositionToWindow: %{public}d-%{public}d", pos1.x, pos1.y);
        ArkUI_NumberValue background_color_value[] = {{.u32 = COLOR_GREEN}};
        ArkUI_AttributeItem background_color_item = {background_color_value,
            sizeof(background_color_value) / sizeof(ArkUI_NumberValue)};
...
static ArkUI_NodeHandle CreateChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI)
{
    ...
    ArkUI_IntOffset pos1;
    auto result1 = OH_ArkUI_NativeModule_ConvertPositionToWindow(button1, position2, pos3);
    OH_LOG_Print(LOG_APP, LOG_INFO, 1, "event", "zlk result1 is %{public}d", result1);
    if (result1 == ARKUI_ERROR_CODE_PARAM_INVALID) {
        nodeAPI->setAttribute(button1, NODE_BACKGROUND_COLOR, &background_color_item);
    }
    auto result9 = OH_ArkUI_NativeModule_ConvertPositionToWindow(button9, position2, &pos1);
...
napi_value ConvertPositionToWindowTest::TestConvertPositionToWindow001(napi_env env, napi_callback_info info)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "TestConvertPositionToWindow001", "CreateNativeNode");
    size_t argc = PARAM_1;
