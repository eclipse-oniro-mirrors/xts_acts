...
static ArkUI_NodeHandle Row = nullptr;
...
napi_value RowSpaceTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    size_t argc = 1;
    ...
    BackGroundColorTest(Row, 0xAFEEEE00);
    WidthPercent(Row, PARAM_0_POINT_9);

    // 设置Column Space
    ArkUI_NumberValue JustifyValue[] = {{.i32 = ARKUI_FLEX_ALIGNMENT_START}};
    ArkUI_AttributeItem JustifyItem = {JustifyValue, sizeof(JustifyValue) / sizeof(ArkUI_NumberValue)};
    nodeAPI->setAttribute(Row, NODE_COLUMN_JUSTIFY_CONTENT, &JustifyItem);
    ArkUI_NumberValue SpaceValue[] = {{.f32 = 5}};
    ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
    nodeAPI->setAttribute(Row, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);

    nodeAPI->addChild(colum1, Row);
    ...
    IDTest(Text1, "rowSpaceText");

    BackGroundColorTest(Text1, 0xF5DEB300);
    BackGroundColorTest(Text2, 0xD2B48C00);

    nodeAPI->addChild(Row, Text1);
    nodeAPI->addChild(Row, Text2);
    ...
    nodeAPI->registerNodeEventReceiver([](ArkUI_NodeEvent *event) {
        ...
        switch (targetId) {
            case PARAM_1: {
                ArkUI_NumberValue SpaceValue[] = {{.f32 = PARAM_10}};
                ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
                nodeAPI->setAttribute(Row, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
                auto getResSpace = nodeAPI->getAttribute(Row, NODE_LINEAR_LAYOUT_SPACE);
                auto value_getResSpace = getResSpace->value[0].f32;
                std::string contentTextStr = std::to_string(value_getResSpace);
            ...
            case PARAM_2: {
                ArkUI_NumberValue SpaceValue[] = {{.f32 = g_rowSpace}};
                ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
                nodeAPI->setAttribute(Row, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
                auto getResSpace = nodeAPI->getAttribute(Row, NODE_LINEAR_LAYOUT_SPACE);
                auto value_getResSpace = getResSpace->value[0].f32;
            ...
            case PARAM_3: {
                ArkUI_NumberValue SpaceValue[] = {{.f32 = PARAM_0}};
                ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
                nodeAPI->setAttribute(Row, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
                auto getResSpace = nodeAPI->getAttribute(Row, NODE_LINEAR_LAYOUT_SPACE);
                auto value_getResSpace = getResSpace->value[0].f32;
