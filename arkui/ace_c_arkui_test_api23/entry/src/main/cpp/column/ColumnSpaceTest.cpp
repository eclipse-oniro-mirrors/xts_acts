...
napi_value ColumnSpaceTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    size_t argc = 1;
    ...
    nodeAPI->setAttribute(Column, NODE_COLUMN_JUSTIFY_CONTENT, &JustifyItem);
    ArkUI_NumberValue SpaceValue[] = {{.f32 = PARAM_5}};
    ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
    nodeAPI->setAttribute(Column, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
    ...
    nodeAPI->registerNodeEventReceiver([](ArkUI_NodeEvent *event) {
        ...
        switch (targetId) {
            case PARAM_1: {
                ...
                ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
                nodeAPI->setAttribute(Column, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
                auto getResSpace = nodeAPI->getAttribute(Column, NODE_LINEAR_LAYOUT_SPACE);
            ...
            case PARAM_2: {
                ArkUI_NumberValue SpaceValue[] = {{.f32 = g_columnSpace}};
                ArkUI_AttributeItem SpaceItem = {SpaceValue, sizeof(SpaceValue) / sizeof(ArkUI_NumberValue)};
                nodeAPI->setAttribute(Column, NODE_LINEAR_LAYOUT_SPACE, &SpaceItem);
                auto getResSpace = nodeAPI->getAttribute(Column, NODE_LINEAR_LAYOUT_SPACE);
                auto value_getResSpace = getResSpace->value[0].f32;
