...
napi_value TextInputTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    size_t argc = 1;
    ...
    if (nodeAPI != nullptr) {
        ...
        if (nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
            ...
            auto textInputA = nodeAPI->createNode(ARKUI_NODE_TEXT_INPUT);
            auto textInputB = nodeAPI->createNode(ARKUI_NODE_TEXT_INPUT);
            auto textInputC = nodeAPI->createNode(ARKUI_NODE_TEXT_INPUT);
            auto textInputD = nodeAPI->createNode(ARKUI_NODE_TEXT_INPUT);
            
            setNodeId(textInputA, "textInputA");
            setNodeId(textInputB, "textInputB");
            setNodeId(textInputC, "textInputC");
            setNodeId(textInputD, "textInputD");
        
            ArkUI_AttributeItem textItem5 = {.string = "ရှည်လျားသောသမိုင်းရှိရှည်လျား"};
            nodeAPI->setAttribute(textInputA, NODE_TEXT_INPUT_TEXT, &textItem5);
            nodeAPI->setAttribute(textInputB, NODE_TEXT_INPUT_TEXT, &textItem5);
            nodeAPI->setAttribute(textInputC, NODE_TEXT_INPUT_TEXT, &textItem5);
            nodeAPI->setAttribute(textInputD, NODE_TEXT_INPUT_TEXT, &textItem5);;
            ...
            nodeAPI->setAttribute(textInputA, NODE_WIDTH, &textWeightItem);
            nodeAPI->setAttribute(textInputB, NODE_WIDTH, &textWeightItem);
            nodeAPI->setAttribute(textInputC, NODE_WIDTH, &textWeightItem);
            nodeAPI->setAttribute(textInputD, NODE_WIDTH, &textWeightItem);
            ...
            nodeAPI->setAttribute(textInputA, NODE_HEIGHT, &textHeightItem);
            nodeAPI->setAttribute(textInputB, NODE_HEIGHT, &textHeightItem);
            nodeAPI->setAttribute(textInputC, NODE_HEIGHT, &textHeightItem);
            nodeAPI->setAttribute(textInputD, NODE_HEIGHT, &textHeightItem);
            ...
            nodeAPI->setAttribute(textInputA, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textInputB, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textInputC, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textInputD, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            ...
            nodeAPI->setAttribute(textInputA, NODE_TEXT_INPUT_INCLUDE_FONT_PADDING, &textIncludePaddingItem1);
            nodeAPI->setAttribute(textInputB, NODE_TEXT_INPUT_INCLUDE_FONT_PADDING, &textIncludePaddingItem2);
            nodeAPI->setAttribute(textInputC, NODE_TEXT_INPUT_FALLBACK_LINE_SPACING, &textFallbackSpacingItem1);
            nodeAPI->setAttribute(textInputD, NODE_TEXT_INPUT_FALLBACK_LINE_SPACING, &textFallbackSpacingItem2);
            
            nodeAPI->addChild(column, textInputA);
            nodeAPI->addChild(column, textInputB);
            nodeAPI->addChild(column, textInputC);
            nodeAPI->addChild(column, textInputD);
