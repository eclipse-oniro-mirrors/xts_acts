...
napi_value TextTest::CreateNativeNode(napi_env env, napi_callback_info info)
{
    size_t argc = 1;
    ...
    if (nodeAPI != nullptr) {
        ...
        if (nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
            ...
            auto textD = nodeAPI->createNode(ARKUI_NODE_TEXT);
            
            setNodeId(textA, "textA");
            setNodeId(textB, "textB");
            setNodeId(textC, "textC");
            setNodeId(textD, "textD");
        
            ArkUI_AttributeItem textItem4 = {.string = "ရှည်လျားသောသမိုင်းရှိရှည်လျား"};
            nodeAPI->setAttribute(textA, NODE_TEXT_CONTENT, &textItem4);
            nodeAPI->setAttribute(textB, NODE_TEXT_CONTENT, &textItem4);
            nodeAPI->setAttribute(textC, NODE_TEXT_CONTENT, &textItem4);
            nodeAPI->setAttribute(textD, NODE_TEXT_CONTENT, &textItem4);
            ...
            nodeAPI->setAttribute(textD, NODE_WIDTH, &textWeightItem);
            ...
            nodeAPI->setAttribute(textD, NODE_HEIGHT, &textHeightItem);
            ...
            nodeAPI->setAttribute(textA, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textB, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textC, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            nodeAPI->setAttribute(textD, NODE_TEXT_LINE_HEIGHT, &textLineHeightItem);
            ...
            nodeAPI->setAttribute(textA, NODE_TEXT_INCLUDE_FONT_PADDING, &textIncludePaddingItem1);
            nodeAPI->setAttribute(textB, NODE_TEXT_INCLUDE_FONT_PADDING, &textIncludePaddingItem2);
            nodeAPI->setAttribute(textC, NODE_TEXT_FALLBACK_LINE_SPACING, &textFallbackSpacingItem1);
            nodeAPI->setAttribute(textD, NODE_TEXT_FALLBACK_LINE_SPACING, &textFallbackSpacingItem2);
            
            nodeAPI->addChild(column, textA);
            nodeAPI->addChild(column, textB);
            nodeAPI->addChild(column, textC);
            nodeAPI->addChild(column, textD);
