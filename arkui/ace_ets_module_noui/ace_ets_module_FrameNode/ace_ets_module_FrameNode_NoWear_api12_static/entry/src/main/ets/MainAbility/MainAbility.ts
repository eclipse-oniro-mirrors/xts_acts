...
export default class MainAbility extends Ability {
    ...
    onWindowStageCreate(windowStage) {
        ...
        // globalThis.windowStage1 = windowStage.getMainWindowSync().getUIContext()
        windowStage.loadContent("MainAbility/pages/index/index", (err, data) => {
            if (err.code) {
                console.error('Failed to load the content. Cause:' + JSON.stringify(err));
                return;
            }
            console.log('windowStage3232323')
            AppStorage.SetOrCreate<UIContext>('UIContent',windowStage.getMainWindowSync().getUIContext())
        });
        console.log('2233213')
    }
