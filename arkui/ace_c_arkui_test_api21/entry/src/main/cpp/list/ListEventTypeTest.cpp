...
struct ArkUI_NodeEvent_List {
    int32_t category;
    int32_t kind;
};

static napi_value Enum_GetEventType_List(napi_env env, napi_callback_info info) {
    NAPI_START(list, ARKUI_NODE_LIST);
    ArkUI_NodeEvent_List *event = new ArkUI_NodeEvent_List;
    event ->category = 0;
    int eventTypes[] = {
    ...
    for (int i = 0; i < sizeof(eventTypes) / sizeof(eventTypes[0]); i++) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "List GetEventType", "result:%{public}d", eventType);
