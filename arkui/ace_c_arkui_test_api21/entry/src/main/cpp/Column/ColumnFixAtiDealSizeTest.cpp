...
#include "ColumnFixAtiDealSizeTest.h"
...
ArkUI_NodeHandle CreateColumnFixAtiDealSizeNodeHandle(const std::string &tag)
{
    ...
    item.string = "ColumnFixAtiDealSizeParent";
    nodeAPI->setAttribute(column1, NODE_ID, &item);
    
    // column1
    value[0].i32 = ARKUI_LAYOUTPOLICY_FIXATIDEALSIZE;
    ...
    item.string = "ColumnFixAtiDealSizeChild";
    nodeAPI->setAttribute(column1, NODE_ID, &item);
...
napi_value ColumnFixAtiDealSizeTest::CreateColumnFixAtiDealSizeNativeRoot(napi_env env, napi_callback_info info)
{
    ...
    if (nodeAPI != nullptr && nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
        ...
        auto nodeContentEvent = [](ArkUI_NodeContentEvent *event) {
            ...
            if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_ATTACH_TO_WINDOW) {
                ...
                if (userDate) {
                    testNode = CreateColumnFixAtiDealSizeNodeHandle(*userDate);
                    delete userDate;
                    userDate = nullptr;
                } else {
                    testNode = CreateColumnFixAtiDealSizeNodeHandle("noUserData");
                }
