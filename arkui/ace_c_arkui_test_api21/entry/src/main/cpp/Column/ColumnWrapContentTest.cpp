...
#include "ColumnWrapContentTest.h"
...
ArkUI_NodeHandle CreateColumnWrapContentNodeHandle(const std::string &tag)
{
    ...
    item.string = "ColumnWrapContentParent";
    nodeAPI->setAttribute(column1, NODE_ID, &item);
    
    // column1
    value[0].i32 = ARKUI_LAYOUTPOLICY_WRAPCONTENT;
    ...
    item.string = "ColumnWrapContentChild";
    nodeAPI->setAttribute(column1, NODE_ID, &item);
...
napi_value ColumnWrapContentTest::CreateColumnWrapContentNativeRoot(napi_env env, napi_callback_info info)
{
    ...
    if (nodeAPI != nullptr && nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
        ...
        auto nodeContentEvent = [](ArkUI_NodeContentEvent *event) {
            ...
            if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_ATTACH_TO_WINDOW) {
                ...
                if (userDate) {
                    testNode = CreateColumnWrapContentNodeHandle(*userDate);
                    delete userDate;
                    userDate = nullptr;
                } else {
                    testNode = CreateColumnWrapContentNodeHandle("noUserData");
                }
