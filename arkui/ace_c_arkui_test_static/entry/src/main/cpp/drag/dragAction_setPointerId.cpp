...
#include "dragAction_setPointerId.h"
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
{
    ...
    ArkUI_AttributeItem LABEL_Item = {.string = "dragAction_SetPointerId Test"};
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragAction_SetPointerIdTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "SetPointerId", "OnEventReceive: event is null");
        return;
    }
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragAction_SetPointerId", "result:%{public}p", dragAction);
    ...
    if (dragAction != nullptr && returnValue != INVALID_PARAM && startDragReturnValue != INVALID_PARAM) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragAction_SetPointerId",
                     "returnValue:%{public}d", returnValue);
    }
}

void SetPointerIdTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
