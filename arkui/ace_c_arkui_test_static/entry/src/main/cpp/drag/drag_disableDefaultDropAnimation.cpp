...
#include "drag_disableDefaultDropAnimation.h"
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
{
    ...
    ArkUI_AttributeItem LABEL_Item = {.string = "drag_disableDefaultDropAnimation Test"};
    ...
    nodeAPI->registerNodeEvent(nodeHandle, NODE_ON_CLICK, 1, nullptr);
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "DragDisableDefaultDropAnimationTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "DragDisableDefaultDrop", "OnEventReceive: event is null");
        return;
    }

    auto dragEvent = OH_ArkUI_NodeEvent_GetDragEvent(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "DragDisableDefaultDropAnimation", "OnEventReceive : %{public}p",
                 dragEvent);
    ...
    int32_t result = OH_ArkUI_DragEvent_DisableDefaultDropAnimation(dragEvent, false);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "DragDisableDefaultDropAnimation", "result:%{public}d", result);
    ...
    if (result == 0) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "DragDisableDefaultDropAnimation",
                     "result:%{public}d", result);
    }
}

void DisableDefaultDropAnimationTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
