...
#include "dragAction_setDragPreviewOption.h"
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
{
    ...
    ArkUI_AttributeItem LABEL_Item = {.string = "dragAction_SetDragPreviewOption Test"};
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragAction_SetDragPreviewOptionTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "SetDragPreviewOption", "OnEventReceive: event is null");
        return;
    }
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "drag_SetDragPreviewOption", "result:%{public}p", dragAction);
    ...
    if (dragAction != nullptr && returnValue != INVALID_PARAM && startDragReturnValue != INVALID_PARAM) {
        ArkUI_NumberValue background_color_value[] = {{.u32 = COLOR_GREEN}};
        ArkUI_AttributeItem background_color_item = {background_color_value,
                                                     sizeof(background_color_value) / sizeof(ArkUI_NumberValue)};
        nodeAPI->setAttribute(nodeHandler, NODE_BACKGROUND_COLOR, &background_color_item);
    } else {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragAction_SetDragPreviewOption",
                     "returnValue:%{public}d", returnValue);
    }
}

void SetDragPreviewOptionTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
