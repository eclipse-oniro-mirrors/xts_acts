...
#include "drag_getDropOperation.h"
...
static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
{
    ...
    ArkUI_AttributeItem LABEL_Item = {.string = "drag_getDropOperation Test"};
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "getDropOperation", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "getDropOperation", "OnEventReceive: event is null");
        return;
    }

    auto dragEvent = OH_ArkUI_NodeEvent_GetDragEvent(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "getDropOperation", "OnEventReceive : %{public}p",
                 dragEvent);
    ...
    ArkUI_DropOperation dropOperation;
    ...
    if (result == 0 && dropOperation == 0) {
        ...
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "getDropOperation",
                     "Drag getDropOperation result : %{public}d", result);
    }
}

void GetDropOperationTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    id_item.string = "GetDropOperation";
