...
static ani_double TestCustomDialogClose001(ani_env* env, ani_object info)
{
    ...
    auto ret = dialogAPI->close(customDialog);
    ASSERT_EQ(ret, SUCCESS);
    NAPI_END;
}
static ani_double TestCustomDialogClose002(ani_env* env, ani_object info)
{
    const auto dialogAPI = reinterpret_cast<ArkUI_NativeDialogAPI_1 *>(
        OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_DIALOG, "ArkUI_NativeDialogAPI_1"));
    auto ret = dialogAPI->close(nullptr);
