...
static ani_double TestCustomDialogCustomStyle001(ani_env* env, ani_object info)
{
    ...
    auto ret = dialogAPI->enableCustomStyle(customDialog, true);
    dialogAPI->show(customDialog, true);
    dialogAPI->close(customDialog);
    ASSERT_EQ(ret, SUCCESS);
    NAPI_END;
}
static ani_double TestCustomDialogCustomStyle002(ani_env* env, ani_object info)
{
    const auto dialogAPI = reinterpret_cast<ArkUI_NativeDialogAPI_1 *>(
        OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_DIALOG, "ArkUI_NativeDialogAPI_1"));
    NAPI_START(column, ARKUI_NODE_COLUMN);
    auto customDialog = dialogAPI->create();
    dialogAPI->setContent(customDialog, column);
    auto ret = dialogAPI->enableCustomStyle(customDialog, false);
    dialogAPI->show(customDialog, true);
    dialogAPI->close(customDialog);
    ASSERT_EQ(ret, SUCCESS);
    NAPI_END;
