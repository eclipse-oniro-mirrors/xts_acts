...
static ani_double TestCustomDialogRemoveContent001(ani_env* env, ani_object info)
{
    ...
    auto ret = dialogAPI->removeContent(customDialog);
    ASSERT_EQ(ret, SUCCESS);
    dialogAPI->dispose(customDialog);
    NAPI_END;
}

static ani_double TestCustomDialogRemoveContent002(ani_env* env, ani_object info)
{
    const ArkUI_NativeDialogAPI_1* dialogAPI = reinterpret_cast<ArkUI_NativeDialogAPI_1 *>(
    OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_DIALOG, "ArkUI_NativeDialogAPI_1"));
    ArkUI_NativeDialogHandle customDialog = dialogAPI->create();
    auto ret = dialogAPI->removeContent(nullptr);
