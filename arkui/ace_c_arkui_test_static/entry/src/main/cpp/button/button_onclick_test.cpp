...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ButtonOnClickTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ButtonOnClickTest", "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeEvent_GetTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ButtonOnClickTest", "OnEventReceive eventId: %{public}d",
                 eventId);
...
void ButtonOnClickTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    id_item.string = "OnClickTestButton";
    nodeAPI->setAttribute(button, NODE_ID, &id_item);
    id_item.string = "OnClickTestButtonDisable";
