...
static ani_double TestCustomComponentGetNextSibling001(ani_env* env, ani_object info)
{
    ...
    auto previousButton = nodeAPI->getNextSibling(button);
    ASSERT_OBJ(previousButton, button3);
    NAPI_END;
}
static ani_double TestCustomComponentGetNextSibling002(ani_env* env, ani_object info)
{
    ArkUI_NativeNodeAPI_1* nodeAPI = nullptr;
    OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);
    auto column = nodeAPI->createNode(ARKUI_NODE_COLUMN);
    ...
    auto previousButton = nodeAPI->getNextSibling(button);
    ASSERT_OBJ(previousButton, button2);
    NAPI_END;
}
static ani_double TestCustomComponentGetNextSibling003(ani_env* env, ani_object info)
{
    ArkUI_NativeNodeAPI_1* nodeAPI = nullptr;
    OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);
    auto column = nodeAPI->createNode(ARKUI_NODE_COLUMN);
