...
#include "customcomponent_onblur_test.h"
#include <string>
#include "ani/ani.h"
#include "hilog/log.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CustomComponentOnBlurTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "CustomComponentOnBlurTest",
                     "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeEvent_GetTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CustomComponentOnBlurTest", "OnEventReceive eventId: %{public}d",
                 eventId);
...
void CustomComponentOnBlurTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    id_item.string = "OnBlurTestCustomComponent";
    nodeAPI->setAttribute(custom, NODE_ID, &id_item);
    id_item.string = "OnBlurTestCustomComponentCompare";
    nodeAPI->setAttribute(customCompare, NODE_ID, &id_item);
    id_item.string = "OnBlurTestCustomComponentDisable";
