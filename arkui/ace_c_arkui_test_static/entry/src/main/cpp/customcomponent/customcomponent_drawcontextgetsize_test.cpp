...
#include "customcomponent_drawcontextgetsize_test.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "CustomComponentDrawContextGetSizeTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "CustomComponentDrawContextGetSizeTest",
            "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeCustomEvent_GetEventTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CustomComponentDrawContextGetSizeTest",
    ...
    if (eventId == ON_DRAW_EVENT_ID) {
        ...
        ArkUI_NumberValue width_value2[] = { { .f32 = (float)width } };
        ArkUI_AttributeItem width_item2 = { width_value2, sizeof(width_value2) / sizeof(ArkUI_NumberValue) };
        nodeAPI->setAttribute(custom, NODE_WIDTH, &width_item2);

        auto height = size.height;
        ArkUI_NumberValue height_value2[] = { { .f32 = (float)height } };
        ArkUI_AttributeItem height_item2 = { height_value2, sizeof(height_value2) / sizeof(ArkUI_NumberValue) };
        nodeAPI->setAttribute(custom, NODE_HEIGHT, &height_item2);
        nodeAPI->addChild(column, custom);
        OH_Drawing_CanvasDrawColor(canvas, COLOR_RED, BLEND_MODE_SCREEN);
...
void CustomComponentDrawContextGetSizeTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
