...
#include "customcomponent_getcontextdrawcanvas_test.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CustomComponentGetContextDrawCanvasTest", "OnEventReceive");
    if (event == nullptr) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CustomComponentGetContextDrawCanvasTest",
    ...
    if (eventId == ON_DRAW_EVENT_ID) {
        ...
        OH_Drawing_PenSetColor(pen, COLOR_GREEN);
...
void CustomComponentGetContextDrawCanvasTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
