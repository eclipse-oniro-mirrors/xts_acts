...
#include "customcomponent_ontouch_test.h"
#include <string>
#include "ani/ani.h"
#include "hilog/log.h"
...
void CustomComponentOnTouchTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    auto customOnTouch = createChildNode(nodeAPI, true);
    auto customOnTouchDisable = createChildNode(nodeAPI, false);

    ArkUI_AttributeItem id_item = {};
    id_item.string = "OnTouchTest";
    nodeAPI->setAttribute(customOnTouch, NODE_ID, &id_item);
    id_item.string = "OnTouchTestDisable";
    nodeAPI->setAttribute(customOnTouchDisable, NODE_ID, &id_item);

    nodeAPI->addChild(column, customOnTouch);
    nodeAPI->addChild(column, customOnTouchDisable);
