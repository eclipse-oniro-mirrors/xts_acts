...
static void OnEventReceive(ArkUI_NodeEvent *event)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "CommonEventOnClickTest", "OnEventReceive eventId: %{public}d",
                 eventId);
...
void CommonEventOnClickTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    auto image = createChildNode(nodeAPI, true);
    auto imageDisable = createChildNode(nodeAPI, false);

    ArkUI_AttributeItem id_item = {};
    id_item.string = "OnClickTestImage";
    nodeAPI->setAttribute(image, NODE_ID, &id_item);
    id_item.string = "OnClickTestImageDisable";
    nodeAPI->setAttribute(imageDisable, NODE_ID, &id_item);

    nodeAPI->addChild(column, image);
    nodeAPI->addChild(column, imageDisable);
