...
static ani_double TestImageAutoResize001(ani_env* env, ani_object info)
{
...
static ani_double TestImageAutoResize002(ani_env* env, ani_object info)
{
    NAPI_START(image, ARKUI_NODE_IMAGE);
    int32_t autoResizeValue = true;
    ArkUI_NumberValue value[] = {{.i32 = autoResizeValue}};
    ArkUI_AttributeItem valueItem = {value, sizeof(value) / sizeof(ArkUI_NumberValue)};
    auto ret = nodeAPI->setAttribute(image, NODE_IMAGE_AUTO_RESIZE, &valueItem);
    ASSERT_EQ(ret, SUCCESS);
