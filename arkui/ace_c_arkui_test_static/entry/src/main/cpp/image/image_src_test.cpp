...
static ani_double TestImageSrc001(ani_env* env, ani_object info)
{
...
static ani_double TestImageSrc002(ani_env* env, ani_object info)
{
    NAPI_START(image, ARKUI_NODE_IMAGE);
...
static ani_double TestImageSrc003(ani_env* env, ani_object info)
{
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> 1");
    NAPI_START(image, ARKUI_NODE_IMAGE);
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> 2");
    OH_PixelmapNativeHandle pixelMap = nullptr;
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> 3");
    
    size_t dataSize = PARAM_960000;
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> 4");
    
    uint8_t data[dataSize];
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> uint8_t data");

    for (int i = PARAM_0; i < dataSize; i++) {
        data[i] = i + PARAM_1;
        OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> for ");  // 循环太多次
    }
    OH_LOG_INFO(LOG_APP, " [cf]TestImageSrc003 start -> 5");
    ...
    auto ret = nodeAPI->setAttribute(image, NODE_IMAGE_SRC, &item);
    ASSERT_EQ(ret, SUCCESS);
    ASSERT_OBJ(nodeAPI->getAttribute(image, NODE_IMAGE_SRC)->object, descriptors);
    NAPI_END;
}

static ani_double TestImageSrc004(ani_env* env, ani_object info)
{
    NAPI_START(image, ARKUI_NODE_IMAGE);
    OH_PixelmapNativeHandle pixelMap = nullptr;
    ...
    array[PARAM_0] = pixelMap1;
    array[PARAM_1] = pixelMap;
    ...
    auto ret = nodeAPI->setAttribute(image, NODE_IMAGE_SRC, &item);
    ASSERT_EQ(ret, SUCCESS);
    ASSERT_OBJ(nodeAPI->getAttribute(image, NODE_IMAGE_SRC)->object, descriptorAnimated);

    NAPI_END;
}

static ani_double TestImageSrc005(ani_env* env, ani_object info)
{
    NAPI_START(image, ARKUI_NODE_IMAGE);
    ArkUI_AttributeItem valueItem = {};
