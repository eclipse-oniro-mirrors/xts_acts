...
#define ON_ERROR_EVENT_ID 8001

namespace ArkUIAniTest {

static bool g_hasOnCompleteInit = false;

void ImageOnErrorTest::OnEventReceive(ArkUI_NodeEvent *event)
{
    ...
    if (eventId == ON_TOUCH_EVENT_ID) {
        ...
        src_item.string = "resource://media/icon1.png";
        nodeAPI->setAttribute(nodeHandler, NODE_IMAGE_SRC, &src_item);
    }

    if ((g_hasOnCompleteInit == true) || (eventId == ON_ERROR_EVENT_ID)) {
...
void ImageOnErrorTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    // bind image onTouch event
    nodeAPI->registerNodeEvent(image, NODE_IMAGE_ON_COMPLETE, ON_ERROR_EVENT_ID, nullptr);
