...
#include "list_scroll_page_test.h"
...
using OnScrollPageCallBack = std::function<void(std::shared_ptr<ListComponent>)>;
std::vector<int32_t> ListScrollPageTest::retData = {};
std::vector<int32_t> ListScrollPageTest::firstScrollIndexData = {};
std::vector<int32_t> ListScrollPageTest::lastScrollIndexData = {};
std::vector<int32_t> ListScrollPageTest::subcomponentScrollIndexData = {};
...
static std::shared_ptr<ColumnComponent> CreateListButton(std::shared_ptr<ColumnComponent> col)
{
    ...
    btn1->RegisterOnClick([col, nodeAPI]() {
        ...
        auto ret = nodeAPI->setAttribute(list->GetComponent(), NODE_SCROLL_PAGE, &item);
        ListScrollPageTest::retData.push_back(ret);
    });
...
static OnScrollPageCallBack scrollPage = [](std::shared_ptr<ListComponent> list) {
    list->SetOnScrollIndex([list](int32_t first, int32_t last, int32_t subcomponent) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "ListScrollPageTest",
            "OnScrollIndex first: %{public}d, last: %{public}d, subcomponent: %{public}d", first, last, subcomponent);
        ListScrollPageTest::firstScrollIndexData.push_back(first);
        ListScrollPageTest::lastScrollIndexData.push_back(last);
        ListScrollPageTest::subcomponentScrollIndexData.push_back(subcomponent);
    });
};

ani_object ListScrollPageTest::GetScrollPageData(ani_env* env, ani_object info)
{
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "ListScrollPageTest", "GetOnScrollIndexData");
    ani_object String_array1 = SetArrayAniData(ListScrollPageTest::retData, env);
    ani_object String_array2 = SetArrayAniData(ListScrollPageTest::firstScrollIndexData, env);
    ani_object String_array3 = SetArrayAniData(ListScrollPageTest::lastScrollIndexData, env);
    ani_object String_array4 = SetArrayAniData(ListScrollPageTest::subcomponentScrollIndexData, env);
    ListScrollPageTest::firstScrollIndexData.clear();
    ListScrollPageTest::lastScrollIndexData.clear();
    ListScrollPageTest::subcomponentScrollIndexData.clear();
    ListScrollPageTest::retData.clear();
...
void ListScrollPageTest::TestListScrollPage(ani_env* env, ani_object info, ani_object NodeContent)
{
    ...
    auto col1 = CreateList<SIZE_40>(scrollPage);
