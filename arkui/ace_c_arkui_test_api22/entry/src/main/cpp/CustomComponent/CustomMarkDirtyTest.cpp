...
ArkUI_NodeHandle CreateCustomMarkDirtyNodeHandle(const std::string &tag)
{
    ...
    value[0].f32 = 100;
    nodeAPI->setAttribute(custom, NODE_WIDTH, &item);
    nodeAPI->setAttribute(custom, NODE_HEIGHT, &item);

    value[0].u32 = 0xFFffc0cb; // Color.Pink
    nodeAPI->setAttribute(custom, NODE_BACKGROUND_COLOR, &item);

    item.string = "CustomMarkDirty1";
    nodeAPI->setAttribute(custom, NODE_ID, &item);

    nodeAPI->markDirty(custom, NODE_NEED_LAYOUT);
    nodeAPI->markDirty(custom1, NODE_NEED_MEASURE);
    nodeAPI->markDirty(custom2, NODE_NEED_RENDER);
...
napi_value CustomMarkDirtyTest::CreateCustomMarkDirtyNativeRoot(napi_env env, napi_callback_info info)
{
    ...
    if (nodeAPI != nullptr && nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
        ...
        auto nodeContentEvent = [](ArkUI_NodeContentEvent *event) {
            ...
            if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_ATTACH_TO_WINDOW) {
                ...
                if (userDate) {
                    testNode = CreateCustomMarkDirtyNodeHandle(*userDate);
                    delete userDate;
                    userDate = nullptr;
                } else {
                    testNode = CreateCustomMarkDirtyNodeHandle("noUserData");
                }
