...
void SwiperComponent::SetSwiperIndex(int32_t index, int32_t animationMode)
{
    ArkUI_NumberValue value[] = { { .i32 = index }, { .i32 = animationMode } };
...
void SwiperComponent::RegisterOnSelect(const std::function<void(ArkUI_NodeEvent* event)>& onSelect)
{
    onSelect_ = onSelect;
    _nodeAPI->registerNodeEvent(_component, NODE_SWIPER_EVENT_ON_SELECTED, 0, nullptr);
}
...
void SwiperComponent::OnNodeEvent(ArkUI_NodeEvent* event)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN,
        "SwiperOnSelectTest", "OnNodeEvent eventType:%{public}d", eventType);
    switch (eventType) {
