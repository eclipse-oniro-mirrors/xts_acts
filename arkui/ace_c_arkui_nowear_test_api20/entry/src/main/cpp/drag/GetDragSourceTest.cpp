...
namespace ArkUICapiTest {
    static void OnEventReceive(ArkUI_NodeEvent *event)
    {
        ...
        switch (eventType) {
            case NODE_ON_DRAG_START: {
                ...
                bool isRemoteBool;
                ArkUI_ErrorCode errorCode1 = OH_ArkUI_DragEvent_GetDragSource(dragEvent, bundleNameChar, 200);
                ArkUI_ErrorCode errorCode2 = OH_ArkUI_DragEvent_IsRemote(dragEvent, &isRemoteBool);
                OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "bundleName:%{public}s;", bundleNameChar);
                OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager", "isRemoteBool:%{public}d;", isRemoteBool);
    ...
    napi_value GetDragSourceTest::CreateNativeNode(napi_env env, napi_callback_info info)
    {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "dragEventGetDragSourceTest", "CreateNativeNode");
        ...
        if ((env == nullptr) || (info == nullptr)) {
            OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "dragEventGetDragSourceTest", "GetContext is null");
            return nullptr;
        }
