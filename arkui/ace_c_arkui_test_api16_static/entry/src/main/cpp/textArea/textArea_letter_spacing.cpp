...
void TextAreaLetterSpacing::textAreaLetterSpacing001(ani_env* env, ani_object info, ani_object NodeContent)
{
...
//                    nodeAPI->getAttribute(textarea, NODE_TEXT_AREA_LETTER_SPACING)->value[0].f32);
    ASSERT_EQ2(nodeAPI->getAttribute(textarea, NODE_TEXT_AREA_LETTER_SPACING)->value[0].f32, 1);
...
//                    "textAreaLetterSpacing001 NODE_TEXT_AREA_LETTER_SPACING content:%{public}d",
//                    nodeAPI->getAttribute(textarea, NODE_TEXT_AREA_LETTER_SPACING)->value[0].f32);
    ASSERT_EQ2(nodeAPI->getAttribute(textarea, NODE_TEXT_AREA_LETTER_SPACING)->value[0].f32, -5);
    ...
    ASSERT_EQ2(nodeAPI->getAttribute(textarea, NODE_TEXT_AREA_ENABLE_PREVIEW_TEXT)->value[0].u32, 1);
...
void TextAreaLetterSpacing::textInputLetterSpacing002(ani_env* env, ani_object info, ani_object NodeContent)
{
    OH_LOG_INFO(LOG_APP, " [cf]CustomComponentOnDrawBehindTest start -> 66");
    //获取节点的前置框架代码，直接写就行
    ArkUI_NativeNodeAPI_1 *nodeAPI = nullptr;
    ...
    ASSERT_EQ2(nodeAPI->getAttribute(textinput, NODE_TEXT_INPUT_LETTER_SPACING)->value[0].u32, 0);
    ...
    ASSERT_EQ2(nodeAPI->getAttribute(textinput, NODE_TEXT_INPUT_ENABLE_PREVIEW_TEXT)->value[0].u32, 1);
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "Manager",
                    "textAreaLetterSpacing001 NODE_TEXT_INPUT_ENABLE_PREVIEW_TEXT content:%{public}d",
                    nodeAPI->getAttribute(textinput, NODE_TEXT_INPUT_ENABLE_PREVIEW_TEXT)->value[0].i32);
    ASSERT_EQ2(nodeAPI->getAttribute(textinput, NODE_TEXT_INPUT_ENABLE_PREVIEW_TEXT)->value[0].u32, 0);
