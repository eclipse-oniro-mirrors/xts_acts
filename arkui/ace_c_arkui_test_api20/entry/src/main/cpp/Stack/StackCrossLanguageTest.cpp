...
#include "StackCrossLanguageTest.h"
...
ArkUI_NodeHandle CreateStackNodeHandle(const std::string &tag)
{
    ArkUI_NodeHandle capiNode = nodeAPI->createNode(ARKUI_NODE_STACK);
    ArkUI_AttributeItem itemStringStyle = {.string = "StackCNodePage_capiNode"};
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Stack", "OH_ArkUI_NodeUtils_SetCrossLanguageOption=%{public}d");
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Stack", "OH_ArkUI_CrossLanguageOption_GetAttributeSettingStatus=%{public}d",
...
napi_value StackCrossLanguageTest::createCrossLanguageStackNode(napi_env env, napi_callback_info info)
{
    ...
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "StackCrossLanguageTest", "napi_get_cb_info success");
    ArkUI_NodeContentHandle nodeContentHandle_ = nullptr;
    OH_ArkUI_GetNodeContentFromNapiValue(env, args[0], &nodeContentHandle_);
    OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "Stack", "OH_ArkUI_GetNodeHandleFromNapiValue %{public}d");
    ...
    if (nodeAPI != nullptr && nodeAPI->createNode != nullptr && nodeAPI->addChild != nullptr) {
        ...
        auto nodeContentEvent = [](ArkUI_NodeContentEvent *event) {
            ...
            if (OH_ArkUI_NodeContentEvent_GetEventType(event) == NODE_CONTENT_EVENT_ON_ATTACH_TO_WINDOW) {
                ...
                if (userDate) {
                    testNode = CreateStackNodeHandle(*userDate);
                    delete userDate;
                    userDate = nullptr;
                } else {
                    testNode = CreateStackNodeHandle("noUserData");
                }
