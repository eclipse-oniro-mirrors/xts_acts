...
napi_value CreateFrameNode(napi_env env, napi_callback_info info)
{
    ...
    g_nodenapiresult1 = OH_ArkUI_GetNodeHandleFromNapiValue(env, args[0], &rowHandle);
...
napi_value GetNodeHandleFromNapiValueTest_001(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    ArkUI_NodeHandle rowHandle;
    int result1 =  OH_ArkUI_GetNodeHandleFromNapiValue(env, result, &rowHandle);
    napi_value retValue;
    napi_create_int32(env, g_nodenapiresult1 == ARKUI_ERROR_CODE_NO_ERROR ? 0 : 1, &retValue);
    return retValue;
}
