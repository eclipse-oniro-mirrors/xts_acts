...
#include "button_axisevent_test.h"
...
namespace ArkUICapiTest {
    auto g_setPropagation = -1;
    auto g_getScrollStep = -1;
    ...
    static auto createChildNode(ArkUI_NativeNodeAPI_1 *nodeAPI, bool enabled)
    {
        ...
        nodeAPI->registerNodeEvent(nodeHandle, NODE_ON_AXIS, ON_AXIS_EVENT_ID, nullptr);
    ...
    static void OnEventReceive(ArkUI_NodeEvent *event)
    {
        ...
        g_setPropagation = OH_ArkUI_AxisEvent_SetPropagation(uiInputEvent, true);
        g_getScrollStep = OH_ArkUI_AxisEvent_GetScrollStep(uiInputEvent);
        if (eventId == ON_AXIS_EVENT_ID && g_setPropagation != -1 && g_getScrollStep != -1) {
    ...
    void ButtonAxisEventTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
    {
        OH_LOG_INFO(LOG_APP, " [cf]ButtonAxisEventTest start -> 66");
        //获取节点的前置框架代码，直接写就行
        ArkUI_NativeNodeAPI_1 *nodeAPI = nullptr;
        OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);
        ...
        id_item.string = "OnAxisTestButton";
