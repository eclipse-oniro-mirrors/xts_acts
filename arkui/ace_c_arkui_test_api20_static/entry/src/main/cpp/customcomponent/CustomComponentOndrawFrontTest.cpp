...
#include "customcomponent_ondraw_front_test.h"
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, ArkUIAniTest::LOG_PRINT_DOMAIN, "CustomComponentOnDrawTest", "OnEventReceive");
    if (event == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, ArkUIAniTest::LOG_PRINT_DOMAIN, "CustomComponentOnDrawTest",
                     "OnEventReceive: event is null");
        return;
    }

    int32_t eventId = OH_ArkUI_NodeCustomEvent_GetEventTargetId(event);
    OH_LOG_Print(LOG_APP, LOG_INFO, ArkUIAniTest::LOG_PRINT_DOMAIN, "CustomComponentOnDrawTest", "OnEventReceive eventId: %{public}d",
                 eventId);
...
void CustomComponentOnDrawFrontTest::CreateNativeNode(ani_env* env, ani_object info, ani_object NodeContent)
{
