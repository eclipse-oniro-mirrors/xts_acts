/*
 * Copyright (C) 2024 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import { describe, beforeAll, beforeEach, afterEach, afterAll, it, expect, TestType, Size, Level } from '@ohos/hypium'

import distributedAccount from '@ohos.account.distributedAccount'

export default function ActsDANoPermissionTest() {
    describe('ActsDANoPermissionTest', function () {

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0100
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0100
         * @tc.desc   no ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0100', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0100 start====");
            let accountAbility = distributedAccount.getDistributedAccountAbility();
            try {
                accountAbility.getOsAccountDistributedInfo((err, data) => {
                    console.info('====>getOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                    try {
                        expect(err.code).assertEqual(201);
                    } catch (err) {
                        console.info('====>Assert err: ' + JSON.stringify(err));
                    }
                    done();
                });
            } catch (err) {
                console.info('====>getOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                expect(err.code).assertEqual(201);
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0200
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0200
         * @tc.desc   no ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0200', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0200 start====");
            const accountAbility = distributedAccount.getDistributedAccountAbility();
            try {
                await accountAbility.getOsAccountDistributedInfo();
                expect(true).assertFalse()
                done()
            } catch (err) {
                console.info('====>checkDataSyncEnabled err: ' + JSON.stringify(err));
                expect(err.code).assertEqual(201);
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0300
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0300
         * @tc.desc   no ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0300', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0300 start====");
            const accountAbility = distributedAccount.getDistributedAccountAbility();
            let accountInfo = { id: '12345', name: 'ZhangSan', event: 'Ohos.account.event.LOGIN' };
            try {
                accountAbility.setOsAccountDistributedInfo(accountInfo, (err) => {
                    console.info('====>setOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                    try {
                        expect(err.code).assertEqual(201);
                    } catch (err) {
                        console.info('====>Assert err: ' + JSON.stringify(err));
                    }
                    done();
                });
            } catch (err) {
                console.info('====>setOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                expect(err.code).assertEqual(201);
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0400
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0400
         * @tc.desc   no ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0400', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0400 start====");
            let accountAbility = distributedAccount.getDistributedAccountAbility();
            let accountInfo = { id: '12345', name: 'ZhangSan', event: 'Ohos.account.event.LOGIN' };
            try {
                await accountAbility.setOsAccountDistributedInfo(accountInfo);
                expect(true).assertFalse()
                done()
            } catch (err) {
                console.info('====>checkDataSyncEnabled err: ' + JSON.stringify(err));
                expect(err.code).assertEqual(201);
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0500
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0500
         * @tc.desc   no all permission.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0500', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0500 start====");
            const accountAbility = distributedAccount.getDistributedAccountAbility();
            try {
                accountAbility.queryOsAccountDistributedInfo((err, data) => {
                    console.info('====>err: ' + JSON.stringify(err));
                    console.info('====>data: ' + JSON.stringify(data));
                    done();
                });
            } catch (err) {
                console.info('====>queryOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                expect().assertFail();
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0600
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0600
         * @tc.desc   no all permission.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0600', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            console.info("====>SUB_Account_DistributedAccount_NoPermission_0600 start====");
            let accountAbility = distributedAccount.getDistributedAccountAbility();
            try {
                await accountAbility.queryOsAccountDistributedInfo();
                console.info("====>run err");
                expect().assertFail();
                done()
            } catch (err) {
                console.info('====>queryOsAccountDistributedInfo err: ' + JSON.stringify(err));
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0700
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0700
         * @tc.desc   no ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0700', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            let tag = "====>SUB_Account_DistributedAccount_NoPermission_0700 "
            console.info(tag + "start====");
            let distributedAccountAbility = distributedAccount.getDistributedAccountAbility();
            let accountInfo = {
                name: 'testName',
                id: 'testId',
                event: 'testEvent',
                avatar: ''
            };
            try {
                distributedAccountAbility.updateOsAccountDistributedInfo(accountInfo, (err, data) => {
                console.info(tag + "err: " + JSON.stringify(err));
                console.info(tag + "data: " + JSON.stringify(data));
                done();
                });
            } catch (err) {
                console.info('====>queryOsAccountDistributedInfo exception: ' + JSON.stringify(err));
                expect().assertFail();
                done();
            }
        });

        /**
         * @tc.name   SUB_Account_DistributedAccount_NoPermission_0800
         * @tc.number SUB_Account_DistributedAccount_NoPermission_0800
         * @tc.desc   updateOsAccountDistributedInfo promise with empty nickname.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it('SUB_Account_DistributedAccount_NoPermission_0800', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async (done) => {
            let tag = "====>SUB_Account_DistributedAccount_NoPermission_0800 "
            console.info(tag + "start====");
            let distributedAccountAbility = distributedAccount.getDistributedAccountAbility();
            let accountInfo = {
                name: 'testName',
                id: 'testId',
                event: 'testEvent',
                nickname: ''
            };
            try {
                await distributedAccountAbility.updateOsAccountDistributedInfo(accountInfo);
                console.info("====>run err");
                expect().assertFail();
                done();
            } catch (err) {
                console.info(tag + "catch err: " + JSON.stringify(err));
                done();
            }
        });
    });
}