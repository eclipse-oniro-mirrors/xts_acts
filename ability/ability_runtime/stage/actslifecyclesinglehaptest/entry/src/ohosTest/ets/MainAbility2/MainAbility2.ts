...
export default class MainAbility2 extends Ability {
    onCreate(want, launchParam) {
        ...
        commonEvent.publish('MainAbility2_onCreate', (err, data) => {
            console.log('MainAbility2 onCreate publish succeed' + JSON.stringify(err) + JSON.stringify(data));
        });
    }

    async onDestroy():Promise<void> {
        console.log('[Demo] MainAbility2 onDestroy');
        commonEvent.publish('MainAbility2_onDestroy', (err, data) => {
            console.log('MainAbility2 onDestroy publish succeed' + JSON.stringify(err) + JSON.stringify(data));
        });
    }
