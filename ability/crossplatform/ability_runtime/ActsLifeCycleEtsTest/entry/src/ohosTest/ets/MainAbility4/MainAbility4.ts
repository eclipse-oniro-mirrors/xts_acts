...
export default class MainAbility4 extends Ability {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    let lifecycleid = globalThis.applicationContext4.on('abilityLifecycle', AbilityLifecycleCallback);
  ...
  async onDestroy():Promise<void> {
    ...
    commonEventManager.publish('MainAbility4_onDestroy', (err, data) => {
      console.log('MainAbility4 onDestroy publish succeed' + JSON.stringify(err) + JSON.stringify(data));
    });
  }
  ...
  onWindowStageCreate(windowStage: window.WindowStage) {
    ...
    windowStage.loadContent('TestAbility/pages/index4', (err: BusinessError) => {
      if (err.code) {
        console.log('MainAbility4 loadContent error');
        return;
      }
      console.log('MainAbility4 loadContent success');
    });
  }
  ...
  onForeground() {
    ...
    commonEventManager.publish('MainAbility4_onForeground', (err, data) => {
