...
export default class MainAbility1 extends Ability {
  onCreate(want: Want, launchParam: AbilityConstant.LaunchParam) {
    ...
    let AbilityLifecycleCallback = {
      ...
      onAbilityDestroy(ability) {
        ...
        commonEventManager.publish(`${abilityname}In1_onAbilityDestroy`, (err, data) => {
          console.log(`${abilityname}In1 onAbilityDestroy publish succeed` + JSON.stringify(err) + JSON.stringify(data));
        });
      },
  ...
  async onDestroy():Promise<void> {
    ...
    commonEventManager.publish('MainAbility1_onDestroy', (err, data) => {
  ...
  onForeground() {
    ...
    commonEventManager.publish('MainAbility1_onForeground', (err, data) => {
