...
async function requestPermission() {
    try {
        ...
        await context.requestPermissionsFromUser(['ohos.permission.READ_HEALTH_DATA'], 0, (data) => {
...
async function driveFn() {
    try {
        ...
        if (button == null) {
            console.info(`====>sensors permission is ${permission}`);
        } else {
            permission = true;
            console.info(`====>sensors permission is ${permission}`);
            await button.click();
    ...
    it("SensorOnOff_ErrCode_Test_0023", TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
        try{
            sensor.getSingleSensor(sensor.SensorId.HEART_RATE,(error, data) => {
                if (error) {
                    console.info('SensorOnOff_ErrCode_Test_0023 getSingleSensor fail:' +JSON.stringify(error));
                    done();
                } else {
                    try {
                        ...
                        console.info('SensorOnOff_ErrCode_Test_0023 catch error:' +JSON.stringify(error));
                        if (permission == false) {
