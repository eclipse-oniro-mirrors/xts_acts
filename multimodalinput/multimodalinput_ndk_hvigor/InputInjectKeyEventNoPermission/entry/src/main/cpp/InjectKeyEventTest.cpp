...
napi_value AddKeyEventMonitorException(napi_env env, napi_callback_info info)
{
    ...
    napi_create_int32(env, (retResult == INPUT_PERMISSION_DENIED) ? 0 : -1, &result);
...
napi_value RemoveKeyEventInterceptorDenied(napi_env env, napi_callback_info info)
{
    ...
    if (retResult == INPUT_SERVICE_EXCEPTION) {
        napi_create_int32(env, (retResult == INPUT_SERVICE_EXCEPTION) ? 0 : -1, &result);
    } else {
        napi_create_int32(env, (retResult == INPUT_PERMISSION_DENIED) ? 0 : -1, &result);
    }
    return result;
...
napi_value UnregisterDeviceListenersException(napi_env env, napi_callback_info info)
{
    ...
    napi_create_int32(env, (errorCode== INPUT_SUCCESS) ? 0 : -1, &result);
    delete listener1;
    delete listener2;
    return result;
}
