...
static void ServiceInit(void)
{
    ...
    if (result2 == FALSE) {
        (&g_service2)->serviceStatus = FALSE;
    }
}
...
{
    BOOL result1 = SAMGR_GetInstance()->RegisterDefaultFeatureApi(SERVICE_NAME, GET_IUNKNOWN(g_service));
    if (result1 == FALSE) {
        (&g_service)->serviceStatus = FALSE;
    }
    BOOL result2 = SAMGR_GetInstance()->RegisterDefaultFeatureApi(SERVICE_NAME, GET_IUNKNOWN(g_service2));
    if (result2 == FALSE) {
        (&g_service2)->serviceStatus = FALSE;
    }
}
 ...
 * @tc.level  LEVEL1
 */
HWTEST_F(LiteIPCServiceTest, testIPCService0010, Function | MediumTest | Level1)
{
    ASSERT_EQ((&g_service)->serviceStatus, TRUE);
    ASSERT_EQ((&g_service)->identity.serviceId != -1, TRUE);
    ASSERT_EQ((&g_service2)->serviceStatus, TRUE);
};

/**
 * @tc.name   testIPCService0020
 ...
 * @tc.level  LEVEL2
 */
HWTEST_F(LiteIPCServiceTest, testIPCService0020, Function | MediumTest | Level2)
{
 ...
 * @tc.level  LEVEL2
 */
HWTEST_F(LiteIPCServiceTest, testIPCService0030, Function | MediumTest | Level2)
{
 ...
 * @tc.level  LEVEL2
 */
HWTEST_F(LiteIPCServiceTest, testIPCService0040, Function | MediumTest | Level2)
{
 ...
 * @tc.level  LEVEL2
 */
HWTEST_F(LiteIPCServiceTest, testIPCService0050, Function | MediumTest | Level2)
{
