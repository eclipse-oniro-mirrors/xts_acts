/*
 * Copyright (C) 2021 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the 'License');
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an 'AS IS' BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import {describe, it, expect, isIntNum, statfs} from './Common';
import featureAbility from '@ohos.ability.featureAbility';
import { Level } from '@ohos/hypium';

export default function statfs_test() {
describe('statfs_test', function () {

  /**
   * @tc.name   statfs_test_get_free_bytes_async_000
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0000
   * @tc.desc   Test getFreeBytes() interfaces, When the path is a sandbox path, promise way back.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_000', Level.LEVEL0, async function (done) {
    try {
      let context = featureAbility.getContext();
      let data = await context.getFilesDir();
      let number = await statfs.getFreeBytes(data);
      console.info("getFreeBytes getFilesDir===>" + (number / 1024));
      expect(isIntNum(number)).assertTrue();
      done();
    } catch (e) {
      console.info('statfs_test_get_free_bytes_async_000 has failed for ' + e);
      expect(null).assertFail();
    }
  });

  /**
   * @tc.name   statfs_test_get_free_bytes_async_001
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0010
   * @tc.desc   Test getFreeBytes() interfaces, When the path is dev, callback way back.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_001', Level.LEVEL0, async function (done) {
    try {
      statfs.getFreeBytes("/dev",(err,number)=>{
        console.info("getFreeBytesNumber dev===>:" + (number / 1024));
        expect(isIntNum(number)).assertTrue();
        done();
      });
    } catch (e) {
      console.info('statfs_test_get_free_bytes_async_001 has failed for ' + e);
      expect(null).assertFail();
    }
  });

  /**
   * @tc.name   statfs_test_get_free_bytes_async_002
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0020
   * @tc.desc   Test getFreeBytes() interfaces, When the parameter type is wrong.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_002', Level.LEVEL0, async function (done) {
    try {
      await statfs.getFreeBytes(1);
    } catch (err) {
      console.info('statfs_test_get_free_bytes_async_002 has failed for ' + err);
      expect(err.message == "Invalid path").assertTrue();
      done();
    }
  });

  /**
   * @tc.name   statfs_test_get_free_bytes_async_003
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0030
   * @tc.desc   Test getFreeBytes() interfaces, When there are no parameters.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_003', Level.LEVEL0, async function (done) {
    try {
      await statfs.getFreeBytes();
    } catch (err) {
      console.info('statfs_test_get_free_bytes_async_003 has failed for ' + err);
      expect(err.message == "Number of arguments unmatched").assertTrue();
      done();
    }
  });

  /**
   * @tc.name   statfs_test_get_free_bytes_async_004
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0040
   * @tc.desc   Test getFreeBytes() interfaces, When the path is empty.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_004', Level.LEVEL0, async function (done) {
    try {
      await statfs.getFreeBytes("");
    } catch (err) {
      console.info('statfs_test_get_free_bytes_async_004 has failed for ' + err);
      expect(err.message == "No such file or directory").assertTrue();
      done();
    }
  });

  /**
   * @tc.name   statfs_test_get_free_bytes_async_005
   * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0050
   * @tc.desc   Test getFreeBytes() interfaces, When the path is dev, callback way back.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('statfs_test_get_free_bytes_async_005', Level.LEVEL0, async function (done) {
    try {
      statfs.getFreeBytes("",(err,number)=>{
        if (err) {
          console.info('statfs_test_get_free_bytes_async_005 err: ' + err);
          expect(err.message == "No such file or directory").assertTrue();
          done();
        }
      });
    } catch (e) {
      console.info('statfs_test_get_free_bytes_async_005 has failed for ' + e);
      expect(null).assertFail();
      done();
    }
  });

     /**
      * @tc.name   statfs_test_get_total_bytes_async_000
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0000
      * @tc.desc   Test getTotalBytes() interfaces, When the path is a sandbox path, promise way back.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
     it('statfs_test_get_total_bytes_async_000', Level.LEVEL0, async function (done) {
      try {
        let context = featureAbility.getContext();
        let data = await context.getFilesDir();
        let number = await statfs.getTotalBytes(data);
        console.info("getTotalBytesNumber GetFilesDir====>:" + (number / 1024));
        expect(isIntNum(number)).assertTrue();
        done();
      } catch (e) {
        console.info('statfs_test_get_total_bytes_async_000 has failed for ' + e);
        expect(null).assertFail();
      }
    });
  
    /**
     * @tc.name   statfs_test_get_total_bytes_async_001
     * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0010
     * @tc.desc   Test getTotalBytes() interfaces, When the path is dev, callback way back.
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL0
     */
    it('statfs_test_get_total_bytes_async_001', Level.LEVEL0, async function (done) {
      try {
        statfs.getTotalBytes("/dev",(err,number)=>{
          console.info("getTotalBytesNumber dev===>:" + (number / 1024));
          expect(isIntNum(number)).assertTrue();
          done();
        });
      } catch (e) {
        console.info('statfs_test_get_total_bytes_async_001 has failed for ' + e);
        expect(null).assertFail();
      }
    });
  
    /**
     * @tc.name   statfs_test_get_total_bytes_async_002
     * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0020
     * @tc.desc   Test getTotalBytes() interfaces, When the parameter type is wrong.
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL0
     */
    it('statfs_test_get_total_bytes_async_002', Level.LEVEL0, async function (done) {
      try {
        await statfs.getTotalBytes(1);
      } catch (err) {
        console.info('statfs_test_get_total_bytes_async_002 has failed for ' + err);
        expect(err.message == "Invalid path").assertTrue();
        done();
      }
    });
  
    /**
     * @tc.name   statfs_test_get_total_bytes_async_003
     * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0030
     * @tc.desc   Test getTotalBytes() interfaces, When there are no parameters.
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL0
     */
    it('statfs_test_get_total_bytes_async_003', Level.LEVEL0, async function (done) {
      try {
        await statfs.getTotalBytes();
      } catch (err) {
        console.info('statfs_test_get_total_bytes_async_003 has failed for ' + err);
        expect(err.message == "Number of arguments unmatched").assertTrue();
        done();
      }
    });
  
    /**
     * @tc.name   statfs_test_get_total_bytes_async_004
     * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0040
     * @tc.desc   Test getTotalBytes() interfaces, When the path is empty.
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL0
     */
    it('statfs_test_get_total_bytes_async_004', Level.LEVEL0, async function (done) {
      try {
        await statfs.getTotalBytes("");
      } catch (err) {
        console.info('statfs_test_get_total_bytes_async_004 has failed for ' + err);
        expect(err.message == "No such file or directory").assertTrue();
        done();
      }
    });

    /**
      * @tc.name   statfs_test_get_free_bytes_callback_005
      * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_CALLBACK_0050
      * @tc.desc   Test getFreeBytes() callback interfaces, When the path is null.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_free_bytes_callback_005', Level.LEVEL0, async function (done) {
        try {
          statfs.getFreeBytes(null, (err, number) => {
            expect(err.message == "Invalid path").assertTrue();
            done();
          });
        } catch (e) {
          console.info('statfs_test_get_free_bytes_callback_005 has failed for ' + e);
          expect(e.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_free_bytes_callback_006
      * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0060
      * @tc.desc   Test getFreeBytes() callback interfaces, When the path is undefined.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_free_bytes_callback_006', Level.LEVEL0, async function (done) {
        try {
          statfs.getFreeBytes(undefined, (err, number) => {
            expect(err.message == "Invalid path").assertTrue();
            done();
          });
        } catch (e) {
          console.info('statfs_test_get_free_bytes_callback_006 has failed for ' + e);
          expect(e.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_free_bytes_promise_005
      * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0051
      * @tc.desc   Test getFreeBytes() promise interfaces, When the path is null.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_free_bytes_promise_005', Level.LEVEL0, async function (done) {
        try {
          await statfs.getFreeBytes(null);
        } catch (err) {
          console.info('statfs_test_get_free_bytes_promise_005 has failed for ' + err);
          expect(err.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_free_bytes_promise_006
      * @tc.number SUB_DF_STATFS_GET_FREE_BYTES_0061
      * @tc.desc   Test getFreeBytes() promise interfaces, When the path is undefined.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_free_bytes_promise_006', Level.LEVEL0, async function (done) {
        try {
          await statfs.getFreeBytes(undefined);
        } catch (err) {
          console.info('statfs_test_get_free_bytes_promise_006 has failed for ' + err);
          expect(err.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_total_bytes_callback_005
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0050
      * @tc.desc   Test getTotalBytes() callback interfaces, When the path is null.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_total_bytes_callback_005', Level.LEVEL0, async function (done) {
        try {
          statfs.getTotalBytes(null, (err, number) => {
            expect(err.message == "Invalid path").assertTrue();
            done();
          });
        } catch (e) {
          console.info('statfs_test_get_total_bytes_callback_005 has failed for ' + e);
          expect(e.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_total_bytes_callback_006
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0060
      * @tc.desc   Test getTotalBytes() callback interfaces, When the path is undefined.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_total_bytes_callback_006', Level.LEVEL0, async function (done) {
        try {
          statfs.getTotalBytes(undefined, (err, number) => {
            expect(err.message == "Invalid path").assertTrue();
            done();
          });
        } catch (e) {
          console.info('statfs_test_get_total_bytes_callback_006 has failed for ' + e);
          expect(e.message == "Invalid path").assertTrue();
          done();
        }
      });

      /**
      * @tc.name   statfs_test_get_total_bytes_callback_007
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0070
      * @tc.desc   Test getTotalBytes() callback interfaces, When the path is undefined.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_total_bytes_callback_007', Level.LEVEL0, async function (done) {
        try {
          statfs.getTotalBytes("", (err, number) => {
            console.info('statfs_test_get_total_bytes_callback_007 err:' + err);
            expect(err.message == "No such file or directory").assertTrue();
            done();
          });
        } catch (e) {
          console.info('statfs_test_get_total_bytes_callback_007 has failed for ' + e);
          expect(e.message == "No such file or directory").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_total_bytes_promise_005
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0051
      * @tc.desc   Test getTotalBytes() promise interfaces, When the path is null.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_total_bytes_promise_005', Level.LEVEL0, async function (done) {
        try {
          await statfs.getTotalBytes(null);
        } catch (err) {
          console.info('statfs_test_get_total_bytes_promise_005 has failed for ' + err);
          expect(err.message == "Invalid path").assertTrue();
          done();
        }
      });
  
      /**
      * @tc.name   statfs_test_get_total_bytes_promise_006
      * @tc.number SUB_DF_STATFS_GET_TOTAL_BYTES_0061
      * @tc.desc   Test getTotalBytes() promise interfaces, When the path is undefined.
      * @tc.type   FUNCTION
      * @tc.size   MEDIUMTEST
      * @tc.level  LEVEL0
      */
      it('statfs_test_get_total_bytes_promise_006', Level.LEVEL0, async function (done) {
        try {
          await statfs.getTotalBytes(undefined);
        } catch (err) {
          console.info('statfs_test_get_total_bytes_promise_006 has failed for ' + err);
          expect(err.message == "Invalid path").assertTrue();
          done();
        }
      });
});
}
