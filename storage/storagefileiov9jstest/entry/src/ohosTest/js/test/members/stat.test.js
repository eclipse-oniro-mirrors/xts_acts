...
describe('fileIO_fs_stat', function () {
  ...
  /**
   * @tc.name   fileIO_stat_sync_004
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0400
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_004', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    let fpath = await nextFileName('fileIO_stat_sync_004');
    expect(prepareFile(fpath, FILE_CONTENT)).assertTrue();
    ...
    try {
      ...
      expect(stat !== null).assertTrue();
      console.log('fileIO_stat_sync_004 stat ' + stat.size);
      fileIO.unlinkSync(fpath);
    } catch (e) {
      console.log('fileIO_stat_sync_004 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_005
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0500
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_005', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    let dpath = await nextFileName('fileIO_stat_sync_005');
    ...
    try {
      let stat = fileIO.statSync(uri);
      expect(stat !== null).assertTrue();
      console.log('fileIO_stat_sync_005 stat ' + stat.size);
      fileIO.rmdirSync(dpath);
    } catch (e) {
      console.log('fileIO_stat_sync_005 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_006
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0600
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_006', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    let fpath = await nextFileName('fileIO_stat_sync_006');
    let uri = fileUri.getUriFromPath(fpath);
    ...
    try {
      ...
      console.log('fileIO_stat_sync_006 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900002 && e.message == 'No such file or directory').assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_007
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0700
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_007', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    try {
      fileIO.statSync(null);
      expect(false).assertTrue();
    } catch (e) {
      console.log('fileIO_stat_sync_007 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900020 && e.message == 'Invalid argument').assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_008
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0800
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_008', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    try {
      fileIO.statSync(undefined);
      expect(false).assertTrue();
    } catch (e) {
      console.log('fileIO_stat_sync_008 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900020 && e.message == 'Invalid argument').assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_009
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_0900
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_009', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    try {
      fileIO.statSync("");
      expect(false).assertTrue();
    } catch (e) {
      console.log('fileIO_stat_sync_009 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900002 && e.message == 'No such file or directory').assertTrue();
    }
  });

  /**
   * @tc.name   fileIO_stat_sync_010
   * @tc.number SUB_DF_FILEIO_STAT_SYNC_1000
   * @tc.desc   Test statSync() for uri interfaces.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_sync_010', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL0, async function () {
    let fpath = await nextFileName('fileIO_stat_sync_004');
    expect(prepareFile(fpath, FILE_CONTENT)).assertTrue();
    let file = fileIO.openSync(fpath);
    try {
      ...
      expect(stat !== null).assertTrue();
      console.log('fileIO_stat_sync_010 stat ' + stat.size);
      fileIO.unlinkSync(fpath);
    } catch (e) {
      console.log('fileIO_stat_sync_010 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
    }
  });
  ...
  /**
   * @tc.name   fileIO_stat_async_009
   * @tc.number SUB_DF_FILEIO_STAT_ASYNC_0900
   * @tc.desc   Test stat() interface for uri. Promise.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_async_009', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    let fpath = await nextFileName('fileIO_stat_async_009');
    expect(prepareFile(fpath, FILE_CONTENT)).assertTrue();
    let uri = fileUri.getUriFromPath(fpath);

    try {
      let stat = await fileIO.stat(uri);
      expect(stat !== null).assertTrue();
      fileIO.unlinkSync(fpath);
      done();
    } catch (e) {
      console.log('fileIO_stat_async_009 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
      done();
    }
  });
   ...
   /**
   * @tc.name   fileIO_stat_async_010
   * @tc.number SUB_DF_FILEIO_STAT_ASYNC_1000
   * @tc.desc   Test stat() interface for uri. Callback.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
   it('fileIO_stat_async_010', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    let dpath = await nextFileName('fileIO_stat_async_010');
    fileIO.mkdirSync(dpath);
    let uri = fileUri.getUriFromPath(dpath);

    try {
      fileIO.stat(uri, (err, stat) => {
        if(err) {
          console.log('fileIO_stat_async_010 error package1: ' + JSON.stringify(err));
          expect(false).assertTrue();
        }
        expect(stat !== null).assertTrue();
        fileIO.rmdirSync(dpath);
      ...
      console.log('fileIO_stat_async_010 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
      done();
    }
  });
  ...
  /**
   * @tc.name   fileIO_stat_async_011
   * @tc.number SUB_DF_FILEIO_STAT_ASYNC_1100
   * @tc.desc   Test stat() interface for uri. Promise.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
  it('fileIO_stat_async_011', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    let fpath = await nextFileName('fileIO_stat_async_011');
    let uri = fileUri.getUriFromPath(fpath);
    ...
    try {
      ...
      console.log('fileIO_stat_async_011 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900002 && e.message == 'No such file or directory').assertTrue();
      done();
    }
  });
  ...
  /**
   * @tc.name   fileIO_stat_async_012
   * @tc.number SUB_DF_FILEIO_STAT_ASYNC_1200
   * @tc.desc   Test stat() interface for uri. Callback.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
   it('fileIO_stat_async_012', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    try {
      fileIO.stat(null, (err, stat) => {
        if(err) {
          console.log('fileIO_stat_async_012 error package1: ' + JSON.stringify(err));
          expect(false).assertTrue();
          done();
        }
      });
    } catch (e) {
      console.log('fileIO_stat_async_012 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900020 && e.message == 'Invalid argument').assertTrue();
      done();
    }
  });
   ...
   * @tc.level  LEVEL0
   */
   it('fileIO_stat_async_013', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    try {
      fileIO.stat(undefined, (err, stat) => {
        if(err) {
          console.log('fileIO_stat_async_013 error package1: ' + JSON.stringify(err));
          expect(false).assertTrue();
          done();
        }
      });
    } catch (e) {
      console.log('fileIO_stat_async_013 has failed for ' + e.message + ', code: ' + e.code);
      expect(e.code === 13900020 && e.message == 'Invalid argument').assertTrue();
      done();
    }
  });
  ...
  /**
   * @tc.name   fileIO_stat_async_014
   * @tc.number SUB_DF_FILEIO_STAT_ASYNC_1400
   * @tc.desc   Test stat() interface for uri. Callback.
   * @tc.type   FUNCTION
   * @tc.size   MEDIUMTEST
   * @tc.level  LEVEL0
   */
   it('fileIO_stat_async_014', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, async function (done) {
    try {
      fileIO.stat("", (err, stat) => {
        if(err) {
          console.log('fileIO_stat_async_014 error package1: ' + JSON.stringify(err));
          expect(err.code === 13900002 && err.message == 'No such file or directory').assertTrue();
          done();
        }
      });
    } catch (e) {
      console.log('fileIO_stat_async_014 has failed for ' + e.message + ', code: ' + e.code);
      expect(false).assertTrue();
      done();
    }
  });
  ...
  /**
   * @tc.name   fileIO_test_append_file_sync_001
