...
ArkUI_AccessibilityProviderCallbacksWithInstance AccessibilityDialog::accessibilityProviderCallbacksWithInstance_;
...
void AccessibilityDialog::CloseCustomDialog()
{
    OH_ArkUI_CustomDialog_CloseDialog(g_id);
}

void AccessibilityDialog::MainViewMethod(ArkUI_NodeContentHandle handle)
{
    ...
    nodeAPI->addNodeEventReceiver(buttonNode, AccessibilityDialog::OnButtonClicked);
    nodeAPI->addChild(column, buttonNode);
    OH_ArkUI_NodeContent_AddNode(handle, column);
}

void AccessibilityDialog::OnButtonClicked(ArkUI_NodeEvent *event)
{
    if (!event || !buttonNode) {
        return;
    }
    ...
    if (eventId == BUTTON_CLICK_ID) {
        ...
        if (g_isShown) {
            ...
            nodeAPI->setAttribute(buttonNode, NODE_BUTTON_LABEL, &labelItem);
            AccessibilityDialog::CloseCustomDialog();
        } else {
            g_isShown = true;
            ArkUI_AttributeItem labelItem = {.string = "关闭弹窗"};
            nodeAPI->setAttribute(buttonNode, NODE_BUTTON_LABEL, &labelItem);
            AccessibilityDialog::CreateNativeNodeDialog();
        }
    }
}

ArkUI_NodeHandle AccessibilityDialog::CreateNativeNodeDialog()
{
    ArkUI_NativeNodeAPI_1 *nodeApi = reinterpret_cast<ArkUI_NativeNodeAPI_1 *>(
        OH_ArkUI_QueryModuleInterfaceByName(ARKUI_NATIVE_NODE, "ArkUI_NativeNodeAPI_1"));
    if (nodeApi == nullptr) {
...
void AccessibilityDialog::CreateAccessibilityProviderSection_dialog(ArkUI_NativeNodeAPI_1* nodeApi)
{
    ...
    if (nodeApi == nullptr || dialogApi == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "nodeApi || dialogApi is NULL");
        return;
    }
    
    ArkUI_NativeDialogHandle dialogHandle = dialogApi->create();
    if (dialogHandle == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "dialogHandle is NULL");
        return;
    }
    
    auto customNode = nodeApi->createNode(ARKUI_NODE_CUSTOM);
    if (customNode == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "customNode is NULL");
        return;
    }
    ...
    if (dialogApi->setContent != nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "dialogApi->setContent not nullptr");
        dialogApi->setContent(dialogHandle, customNode);
    }
    
    char uniqueNodeId[64] = {0};
    AccessibilityDialog::GetAccessibilityProvider(&customNode, uniqueNodeId);
    if (dialogApi->show != nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "dialogApi->show not nullptr");
        dialogApi->show(dialogHandle, true);
    }
}

int32_t AccessibilityDialog::GetAccessibilityProvider(ArkUI_NodeHandle* customNode, const char* id)
{
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.findAccessibilityNodeInfosById =
        FindAccessibilityNodeInfosById;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.findAccessibilityNodeInfosByText =
        FindAccessibilityNodeInfosByText;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.findFocusedAccessibilityNode =
        FindFocusedAccessibilityNode;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.findNextFocusAccessibilityNode =
        FindNextFocusAccessibilityNode;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.executeAccessibilityAction =
        ExecuteAccessibilityAction;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.clearFocusedFocusAccessibilityNode =
        ClearFocusedFocusAccessibilityNode;
    AccessibilityDialog::accessibilityProviderCallbacksWithInstance_.getAccessibilityNodeCursorPosition =
    ...
    if (accessibilityProvider_ == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "accessibilityProvider_ is null");
        return 0;
    }

    int32_t ret = OH_ArkUI_AccessibilityProviderRegisterCallbackWithInstance(id, accessibilityProvider_,
        &AccessibilityDialog::accessibilityProviderCallbacksWithInstance_);
    if (ret != 0) {
        return 0;
    }
    return 0;
...
void AccessibilityDialog::FillElementRect(int64_t elementId, ArkUI_AccessibleRect& rect)
{
    if (elementId == 0) {
        rect.leftTopX = 0;
        rect.leftTopY = 0;
...
void AccessibilityDialog::InitRootNode(ArkUI_AccessibilityElementInfo* rootNode, int parentOfRoot)
{
    ...
    AccessibilityDialog::FillElementRect(0, rect);
    OH_ArkUI_AccessibilityElementInfoSetScreenRect(rootNode, &rect);
}

void AccessibilityDialog::AddChildNodesForRoot(const char* instanceId, ArkUI_AccessibilityElementInfoList* elementList,
                                               ArkUI_AccessibilityElementInfo* rootNode)
{
    ...
    for (int i = 0; i < objects.size(); i++) {
        ...
        ArkUI_AccessibleRect rect;
        AccessibilityDialog::FillElementRect(elementId, rect);
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog",
                 "FindAccessibilityNodeInfosById child count: %{public}ld %{public}d",
                 objects.size(), ret);
}

void AccessibilityDialog::HandlePrefetchCurrentMode(const char* instanceId, int64_t elementId,
                                                    ArkUI_AccessibilityElementInfoList* elementList, int parentOfRoot)
{
    ...
    if (elementId == 0) {
        AccessibilityDialog::AddChildNodesForRoot(instanceId, elementList, node);
    }
}

int32_t AccessibilityDialog::FindAccessibilityNodeInfosById(const char* instanceId, int64_t elementId,
                                                            ...
                                                            ArkUI_AccessibilityElementInfoList* elementList)
{
    ...
    if (elementList == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "AccessibilityDialog",
...
int32_t AccessibilityDialog::FindAccessibilityNodeInfosByText(const char* instanceId, int64_t elementId,
...
int32_t AccessibilityDialog::FindFocusedAccessibilityNode(const char* instanceId, int64_t elementId,
...
int32_t AccessibilityDialog::FindNextFocusAccessibilityNode(const char* instanceId, int64_t elementId,
...
int32_t AccessibilityDialog::ExecuteAccessibilityAction(const char* instanceId, int64_t elementId,
...
int32_t AccessibilityDialog::ClearFocusedFocusAccessibilityNode(const char* instanceId)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "ClearFocusedFocusAccessibilityNode");
    return OH_NATIVEXCOMPONENT_RESULT_SUCCESS;
}

int32_t AccessibilityDialog::GetAccessibilityNodeCursorPosition(const char* instanceId, int64_t elementId,
                                                                int32_t requestId, int32_t *index)
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "AccessibilityDialog", "GetAccessibilityNodeCursorPosition");
    return OH_NATIVEXCOMPONENT_RESULT_SUCCESS;
}