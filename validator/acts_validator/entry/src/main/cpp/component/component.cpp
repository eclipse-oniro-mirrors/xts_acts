...
void Component::SetSelectedStates(std::shared_ptr<Component>& component)
{
    if (!component) {
        return;
    }
    int32_t selectedState = ArkUI_UIState::UI_STATE_SELECTED;
    std::weak_ptr<Component> weakComponent = component;
    // 定义静态回调函数
    static auto selectedStateCallback = [](int32_t currentStates, void* userData) {
        auto compPtr = static_cast<Component*>(userData);
            if (!compPtr) {
                return;
            }
    ...
    ArkUI_ErrorCode result = OH_ArkUI_AddSupportedUIStates(
