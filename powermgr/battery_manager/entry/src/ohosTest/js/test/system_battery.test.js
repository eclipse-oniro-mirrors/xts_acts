/*
 * Copyright (C) 2022 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *   http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import battery from '@system.battery';
import batteryInfo from '@ohos.batteryInfo';
import { describe, beforeAll, beforeEach, afterEach, afterAll, it, expect, TestType, Size, Level } from '@ohos/hypium';

function successFunc(data, tag) {
  console.log(tag + ": level: " + data.level + ", charging: " + data.charging);
  let soc = (batteryInfo.batterySOC * 0.01);
  expect(fabs(soc - data.level) <= 1e-9).assertTrue();
  if (batteryInfo.chargingStatus === batteryInfo.BatteryChargeState.ENABLE ||
    batteryInfo.chargingStatus === batteryInfo.BatteryChargeState.FULL) {
    expect(data.charging).assertTrue();
  } else {
    expect(data.charging).assertFalse();
  }
}

function failFunc(data, code, tag) {
  console.log(tag + ": data: " + data + ", code: " + code);
}

function completeFunc(tag) {
  console.log(tag + ": The device information is obtained successfully.");
}

export default function SystemBatteryTest() {
  describe('SystemBatteryTest', function () {
    console.log("*************System Battery Unit Test Begin*************");

    /**
     * @tc.name   get_status_success_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0010
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_success_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: (data) => {
          successFunc(data, successTest);
        },
        fail: (data, code) => {
          failFunc(data, code, successTest);
          expect().assertFail();
        },
        complete: () => {
          completeFunc(successTest);
        }
      });
    });

    /**
     * @tc.name   get_status_success_null_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0020
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_success_null_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: null,
        fail: (data, code) => {
          failFunc(data, code, successNullTest);
          expect().assertFail();
        },
        complete: () => {
          completeFunc(successNullTest);
        }
      });
    });

    /**
     * @tc.name   get_status_success_empty_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0030
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_success_empty_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        fail: (data, code) => {
          failFunc(data, code, successEmptyTest);
          expect().assertFail();
        },
        complete: () => {
          completeFunc(successEmptyTest);
        }
      });
    });

    /**
     * @tc.name   get_status_fail_null_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0040
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_fail_null_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: (data) => {
          expect(data).assertNotNull();
          successFunc(data, failNullTest);
        },
        fail: null,
        complete: () => {
          completeFunc(failNullTest);
        }
      });
    });

    /**
     * @tc.name   get_status_fail_empty_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0050
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_fail_empty_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: () => {
          successFunc(data, failNullTest);
          expect(data).assertNull();
        },
        complete: () => {
          completeFunc(failEmptyTest);
        }
      });
    });

    /**
     * @tc.name   get_status_complete_null_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0060
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_complete_null_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: (data) => {
          successFunc(data, completeNullTest);
        },
        fail: (data, code) => {
          failFunc(data, code, completeNullTest);
          expect().assertFail();
        },
        complete: null
      });
    });

    /**
     * @tc.name   get_status_complete_empty_test
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0070
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_complete_empty_test', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      battery.getStatus({
        success: (data) => {
          successFunc(data, completeEmptyTest);
        },
        fail: (data, code) => {
          failFunc(data, code, completeEmptyTest);
          expect().assertFail();
        }
      });
    });

    /**
     * @tc.name   get_status_all_null
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0080
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_all_null', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      let allNull = false;
      battery.getStatus({
        success: null,
        fail: null,
        complete: null,
      });
      expect(!allNull).assertTrue();
    });

    /**
     * @tc.name   get_status_all_empty
     * @tc.number SUB_PowerSystem_SystemBattery_JSTest_0090
     * @tc.desc   Battery acquisition kit
     * @tc.type   FUNCTION
     * @tc.size   MEDIUMTEST
     * @tc.level  LEVEL3
     */
    it('get_status_all_empty', TestType.FUNCTION | Size.MEDIUMTEST | Level.LEVEL3, function () {
      let allNull = false;
      battery.getStatus();
      expect(!allNull).assertTrue();
    });
  })
}