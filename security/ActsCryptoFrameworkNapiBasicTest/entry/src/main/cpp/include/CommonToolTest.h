...
template <typename T>
inline void AssertTrueImpl(const T& v, const char* expr, const AssertLocation& loc)
{
    if (!v) {
...
template <typename T>
inline void AssertFalseImpl(const T& v, const char* expr, const AssertLocation& loc)
{
    if (v) {
...
#define EXPECT_TRUE(v) /* NOLINT(G.PRE.02-CPP) */ \
    AssertTrueImpl((v), #v, AssertLocation({ __FILE__, __LINE__ }))

#define EXPECT_FALSE(v) /* NOLINT(G.PRE.02-CPP) */ \
    AssertFalseImpl((v), #v, AssertLocation({ __FILE__, __LINE__ }))
...
int SafeMemset(void* dst, size_t dstSize, const void* src, size_t len);

void HcfFree(void *addr);

void HcfBlobDataClearAndFree(Crypto_DataBlob *dataBlob);
