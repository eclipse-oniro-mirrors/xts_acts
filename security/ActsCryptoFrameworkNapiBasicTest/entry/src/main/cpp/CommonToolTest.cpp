...
const std::string DEFAULT_TAG = "ActsCryptoFrameworkNapiBasicTestTag";
}
int32_t GetInt32ArgImpl(napi_env env, napi_callback_info info, size_t index, int32_t defaultValue)
{
    size_t argc = index + 1;
    std::vector<napi_value> argsStorage(argc);
    napi_value* args = argsStorage.data();
...
int SafeMemset(void* dst, size_t dstSize, const void* src, size_t len)
{
    ...
    unsigned char value = *(static_cast<const unsigned char *>(src));
    for (size_t i = 0; i < len; i++) {
        ptr[i] = value;
    }
...
{
    if ((dataBlob == nullptr) || (dataBlob->data == nullptr)) {
        return;
    }
    (void)SafeMemset(dataBlob->data, dataBlob->len, 0, dataBlob->len);
    HcfFree(dataBlob->data);
    dataBlob->data = nullptr;
    dataBlob->len = 0;
}
