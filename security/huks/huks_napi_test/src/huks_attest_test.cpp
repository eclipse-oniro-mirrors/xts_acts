...
void HuksAttestKeyNoIdsTest::SetUpTestCase(void) { useSoftware = checkUseSoftware(); }
...
/**
 * @tc.name   Security_HUKS_NAPI_Attest_0100
...
HWTEST_F(HuksAttestKeyNoIdsTest, Security_HUKS_NAPI_Attest_0100, TestSize.Level0)
{
    if (useSoftware) {
        ...
        certChain = NULL;
        OH_Huks_Result ret1 = OH_Huks_DeleteKeyItem(&g_keyAlias, paramSet);
