...
static void AcoshTestOne(struct DD* p, int* err)
{
    double y;
    float d;
    int e;
    ...
    if (!Checkexcept(e, p->e, p->r)) {
        ...
        printf(
            "%s:%d: bad fp exception: %s acosh(%a)=%a, want %s", p->file, p->line, Rstr(p->r), excX, excY, Estr(p->e));
        printf(" got %s\n", Estr(e));
        (*err)++;
    }
    d = Ulperr(y, p->y, p->dy);
    if (!Checkulp(d, p->r)) {
        ...
        printf("%s:%d: %s acosh(%a) want %a got %a Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
