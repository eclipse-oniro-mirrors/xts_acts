...
/**
 * @tc.name      : GetgrnamR0100
 * @tc.desc      : Get group file entry
 * @tc.level     : Level 0
 */
void GetgrnamR0100(void)
{
    errno = 0;
    char buf[512];
    gid_t gid = 0;
    ...
    int result = getgrnam_r(groupName, &grpStorage, buf, sizeof(buf), &grp);
    if (result != 0) {
...
/**
 * @tc.name      : GetgrnamR0200
 * @tc.desc      : Invalid parameter test
 * @tc.level     : Level 2
 */
void GetgrnamR0200(void)
{
    char buf[512];
    struct group* grp;
    struct group grpStorage;

    int result = getgrnam_r("invalid_name", &grpStorage, buf, sizeof(buf), &grp);
    if (result) {
        t_error("%s getgrnam_r should be failed\n", __func__);
    }
}

static int GetgrnamRTestImpl(int argc, char* argv[])
{
    GetgrnamR0100();
    GetgrnamR0200();
    return T_STATUS;
...
int GetgrnamRTest(void)
{
    static char libcProgStub[] = "libc_test";
    char* libcArgvStub[] = { libcProgStub, nullptr };
    return GetgrnamRTestImpl(1, libcArgvStub);
}
