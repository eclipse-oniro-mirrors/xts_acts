...
int MkdtempFailureTest()
{
    ...
    r = mkdtemp(p);
    if (r) {
        LibcTestError(&status, "mkdtemp(" S ") did not fail\n");
    }
    if (memcmp(p, S, sizeof p) != 0) {
        LibcTestError(&status, "mkdtemp(" S ") modified the template: %s\n", p);
    }
    if (r == nullptr && errno != EINVAL) {
        LibcTestError(&status,
                      "mkdtemp(" S ") failed with %d [%s] instead of %d [%s]\n",
                      errno,
                      strerror(errno),
                      EINVAL,
                      strerror(EINVAL));
