...
static void Exp2lTestOne(struct LL* p, int* err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkulp(d, p->r)) {
        ...
        printf("%s:%d: %s exp2l(%La) want %La got %La Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
               ulpLaX,
...
static int Exp2lTestImpl(void)
{
#pragma STDC FENV_ACCESS ON
    int i;
    int err = 0;
    for (i = 0; i < sizeof t / sizeof *t; i++) {
        Exp2lTestOne(t + i, &err);
    }
    return !!err;
}
