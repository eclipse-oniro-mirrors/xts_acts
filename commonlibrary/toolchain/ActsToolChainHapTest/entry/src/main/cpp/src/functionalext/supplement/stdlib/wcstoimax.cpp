 ...
 * @tc.level     : Level 2
 */
void Wcstoimax0200(void)
{
    signal(SIGSEGV, DealAberrant);
    intmax_t result = wcstoimax(nullptr, nullptr, 0);
}

static int WcstoimaxTestImpl(int argc, char *argv[])
{
    Wcstoimax0100();
    Wcstoimax0200();
    return g_tStatus;
...
int WcstoimaxTest(void)
{
    ...
    return WcstoimaxTestImpl(1, libcArgv);
}
