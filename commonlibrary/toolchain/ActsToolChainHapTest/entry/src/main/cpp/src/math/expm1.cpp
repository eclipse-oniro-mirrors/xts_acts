...
static void Expm1TestOne(struct DD *p, int *err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkulp(d, p->r)) {
        ...
        printf("%s:%d: %s expm1(%a) want %a got %a Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
...
static int Expm1TestImpl(void)
{
#pragma STDC FENV_ACCESS ON
    int i;
    int err = 0;
    for (i = 0; i < sizeof t / sizeof *t; i++) {
        Expm1TestOne(t + i, &err);
    }
    return !!err;
}
