...
void Readv0100(void)
{
    ...
    const ssize_t expected = static_cast<ssize_t>(strlen(buf1) + strlen(buf2));
...
{
    errno = 0;
    ssize_t result = readv(-1, nullptr, -1);
    if (result == 0) {
        t_error("%s failed: result = %ld\n", __func__, result);
    }
...
static int ReadvTestImpl(int argc, char* argv[])
{
    Readv0100();
    Readv0200();
...
int ReadvTest(void)
{
    ...
    return ReadvTestImpl(1, libcArgv);
}
