...
static void AsinhTestOne(struct DD* p, int* err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkexcept(e, p->e, p->r)) {
        ...
        printf(
            "%s:%d: bad fp exception: %s asinh(%a)=%a, want %s", p->file, p->line, Rstr(p->r), excX, excY, Estr(p->e));
        printf(" got %s\n", Estr(e));
        (*err)++;
    }
    d = Ulperr(y, p->y, p->dy);
    if (!Checkulp(d, p->r)) {
        ...
        printf("%s:%d: %s asinh(%a) want %a got %a Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
