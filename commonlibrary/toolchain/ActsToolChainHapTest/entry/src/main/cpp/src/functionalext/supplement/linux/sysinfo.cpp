...
/**
 * @tc.name      : Sysinfo0200
 * @tc.desc      : return system information with nullptr
 * @tc.level     : Level 2
 */
void Sysinfo0200(void)
{
    int result = sysinfo(nullptr);
    if (result == 0) {
        t_error("%s failed: result = %d\n", __func__, result);
        return;
    }
...
static int SysinfoTestImpl(int argc, char* argv[])
{
    Sysinfo0100();
    Sysinfo0200();
...
int SysinfoTest(void)
{
    static char libcProgStub[] = "libc_test";
    char* libcArgvStub[] = { libcProgStub, nullptr };
    return SysinfoTestImpl(1, libcArgvStub);
}
