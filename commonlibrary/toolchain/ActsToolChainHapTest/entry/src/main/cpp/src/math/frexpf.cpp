...
static void FrexpfTestOne(struct FFi* p, int* err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkcr(y, p->y, p->r) || (isfinite(p->x) && yi != p->i)) {
        ...
        printf("%s:%d: %s frexpf(%a) want %a,%lld got %a,%d Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
               static_cast<double>(p->x),
...
static int FrexpfTestImpl(void)
{
#pragma STDC FENV_ACCESS ON
    int i;
    int err = 0;
    for (i = 0; i < sizeof t / sizeof *t; i++) {
        FrexpfTestOne(t + i, &err);
    }
    return !!err;
}
