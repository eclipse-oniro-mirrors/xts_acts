...
static bool HasRemquolQuotientMismatch(const struct LlLi* p, int quotient)
{
    return (!isnan(p->y) &&
            ((quotient & REMQUO_LOW_BITS_MASK) != (p->i & REMQUO_LOW_BITS_MASK) || (quotient < 0) != (p->i < 0)));
}

static int RemquolTestImpl(void)
{
#pragma STDC FENV_ACCESS ON
    ...
    for (i = 0; i < sizeof t / sizeof *t; i++) {
        ...
        d = Ulperr(y, p->y, p->dy);
        if (!Checkcr(y, p->y, p->r) || HasRemquolQuotientMismatch(p, yi)) {
            printf("%s:%d: %s remquol(%La,%La) want %La,%lld got %La,%d ulperr %.3f = %.3f + %.3f\n",
                   p->file,
                   p->line,
                   Rstr(p->r),
                   p->x,
...
int RemquolTest(void)
{
    return RemquolTestImpl();
}
