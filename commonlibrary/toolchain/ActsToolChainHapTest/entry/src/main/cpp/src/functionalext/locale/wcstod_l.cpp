...
static void EXPECT_DOUBLE_EQ(double a, double b)
{
...
static struct {
    ...
    // 2^-1022 + 2^-1075 + eps
    { L".2225073858507201630123055637955676152503612414573018013083228724049586647606760e-307",
    ...
    { L"17976931348623158079372897140530341507993413271003782693617377898044"
      ...
      L"2711559699508093042880177904174497791.999999999999999999999999999999",
    ...
    { L"17976931348623158079372897140530341507993413271003782693617377898044"
      ...
      L"02630657488671505820681908902000708383676273854845817711531764475730"
      L"27006985557136695962284291481986083493647529271907416844436551070434"
      L"2711559699508093042880177904174497792",
      INFINITY },
    // some random numbers
 ...
 * @tc.level     : Level 0
 */
void WcstodL0100(void)
{
    LocaleGuard locGuard(newlocale(LC_ALL_MASK, "zh_CN.UTF-8", nullptr));
    locale_t loc = locGuard.Get();
    int i;
    double x;
    wchar_t* p;

    for (i = 0; i < static_cast<int>(ArrayLength(g_t)); i++) {
 ...
 * @tc.level     : Level 0
 */
void WcstodL0200(void)
{
    LocaleGuard locGuard(newlocale(LC_ALL_MASK, "zh_CN", nullptr));
    locale_t loc = locGuard.Get();
 ...
 * @tc.level     : Level 0
 */
void WcstodL1700(void)
{
    LocaleGuard locGuard(newlocale(LC_ALL_MASK, "de_DE", nullptr));
    locale_t loc = locGuard.Get();
 ...
 * @tc.level     : Level 0
 */
void WcstodL1800(void)
{
    LocaleGuard locGuard(newlocale(LC_CTYPE_MASK, "zh_CN", nullptr));
    locale_t loc = locGuard.Get();
    wchar_t* str = L"     ";
    const double target = 0;
    wchar_t str1[] = L"     ";
...
/**
 * @tc.name      : WcstodL1900
 * @tc.desc      : set locale as zh_CN.UTF-8, set endptr as nullptr
 * @tc.level     : Level 0
 */
void WcstodL1900(void)
{
    LocaleGuard locGuard(newlocale(LC_ALL_MASK, "zh_CN.UTF-8", nullptr));
    locale_t loc = locGuard.Get();
    wchar_t* str = L"123.45";
    const double target = 123.45;
    double num = wcstod_l(str, nullptr, loc);
    EXPECT_DOUBLE_EQ(num, target);
}

static int WcstodLTestImpl(void)
{
    WcstodL0100();
    WcstodL0200();
    WcstodL0300();
...
int WcstodLTest(void)
{
    return WcstodLTestImpl();
}
