 ...
 * @tc.level     : Level 2
 */
void Wcstoumax0200(void)
{
    signal(SIGSEGV, DealAberrant);
    uintmax_t result = wcstoumax(nullptr, nullptr, 0);
}

static int WcstoumaxTestImpl(int argc, char *argv[])
{
    Wcstoumax0100();
    Wcstoumax0200();
    return g_tStatus;
...
int WcstoumaxTest(void)
{
    ...
    return WcstoumaxTestImpl(1, libcArgv);
}
