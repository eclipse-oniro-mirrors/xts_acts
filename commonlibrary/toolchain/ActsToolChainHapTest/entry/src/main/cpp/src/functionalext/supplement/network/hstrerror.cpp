...
void Hstrerror0100(void)
{
    ...
    for (i = 0; i < TEST_MSG_COUNT; i++) {
        ...
        EXPECT_PTRNE("Hstrerror0100", ret, nullptr);
        if (ret) {
            EXPECT_STREQ("Hstrerror0100", ret, msg[i]);
        }
    }
}

static int HstrerrorTestImpl(void)
{
    Hstrerror0100();
    return T_STATUS;
}

int HstrerrorTest(void)
{
    return HstrerrorTestImpl();
}
