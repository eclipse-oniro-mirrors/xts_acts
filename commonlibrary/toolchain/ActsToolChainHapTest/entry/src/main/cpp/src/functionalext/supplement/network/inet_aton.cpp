...
/**
 * @tc.name      : InetAton0600
 * @tc.desc      : When the parameter is invalid, test the return value of the function
 * @tc.level     : Level 2
 */
void InetAton0600()
{
    int ret = inet_aton("", nullptr);
    if (ret != 0) {
        t_error("%s inet_aton failed\n", __func__);
    }
}

static int InetAtonTestImpl(int argc, char* argv[])
{
    InetAton0100();
    InetAton0200();
    InetAton0300();
...
int InetAtonTest(void)
{
    static char libcProgStub[] = "libc_test";
    char* libcArgvStub[] = { libcProgStub, nullptr };
    return InetAtonTestImpl(1, libcArgvStub);
}
