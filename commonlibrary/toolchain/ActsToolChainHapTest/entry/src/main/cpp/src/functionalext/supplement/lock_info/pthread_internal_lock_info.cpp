...
void WaitUntilSignal(int signum)
{
    ...
    (void)sigtimedwait(&set, nullptr, &timeout);
}
...
#define FAIL_CLOSE (-1)
    ...
    int res;
    res = snprintf_s(finalCommand, COMMAND_SIZE, READ_SIZE, "%s%s", command, needToMatch);
    if (res == NEGATIVE_ONE) {
...
void PthreadInternalLockInfoTestCase(void)
{
    // 发送信号35和40到自己
...
int PthreadInternalLockInfoTest(void)
{
    return PthreadInternalLockInfoTestImpl();
}
