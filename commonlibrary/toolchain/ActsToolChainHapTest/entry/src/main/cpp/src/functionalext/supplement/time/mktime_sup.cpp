...
static const struct MktimeStruct G_RESULT_DATA[] = {
...
void Mktime0100(void)
{
    for (int32_t i = 0; i < static_cast<int32_t>(sizeof(G_RESULT_DATA) / sizeof(G_RESULT_DATA[0])); i++) {
        ...
        if (!timeptr) {
            EXPECT_PTRNE("Mktime0100", timeptr, nullptr);
            return;
        }
        time_t mk = mktime(timeptr);
        EXPECT_EQ("Mktime0100", mk, G_RESULT_DATA[i].result);
    }
}

static int MktimeSupTestImpl(void)
{
    Mktime0100();
    return T_STATUS;
}

int MktimeSupTest(void)
{
    return MktimeSupTestImpl();
}
