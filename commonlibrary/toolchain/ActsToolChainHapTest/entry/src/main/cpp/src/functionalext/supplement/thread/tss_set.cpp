...
#define TSS_SET_VALUE (reinterpret_cast<void*>(static_cast<uintptr_t>(0xFF)))
static tss_t g_key;

int Tssfunc(void* arg)
{
    int ret = tss_create(&g_key, nullptr);
    if (ret != thrd_success) {
        t_error("%s tss_create failed", __func__);
    }
    ret = tss_set(g_key, TSS_SET_VALUE);
    if (ret != thrd_success) {
        t_error("%s tss_set failed", __func__);
    }
    void* value = tss_get(g_key);
    if (value == nullptr) {
 ...
 * @tc.level     : Level 0
 */
void TssSet0100(void)
{
    thrd_t id;
    int result = tss_create(&g_key, nullptr);
    if (result != thrd_success) {
        t_error("%s tss_create failed", __func__);
    }

    result = thrd_create(&id, Tssfunc, nullptr);
    if (result != thrd_success) {
...
static int TssSetTestImpl(int argc, char* argv[])
{
    TssSet0100();
    return T_STATUS;
}
...
int TssSetTest(void)
{
    ...
    return TssSetTestImpl(1, libcArgvStub);
}
