 ...
 * @tc.level     : Level 0
 */
void Vdprintf0100(void)
{
    char *value = "asdf";
    char buffer[100] = {0};
    int fp = open("/data/storage/el2/base/cache/temp_vdprintf.txt", O_RDWR | O_CREAT, TEST_MODE);
    if (fp < 0) {
    ...
    close(fp);
    fp = open("/data/storage/el2/base/cache/temp_vdprintf.txt", O_RDWR);
    read(fp, buffer, strlen(value));
    if (strcmp(buffer, "asdf") != 0) {
        t_error("%s vdprintf get is '%s' are not 'asdf'\n", __func__, buffer);
    }
    close(fp);
    unlink("/data/storage/el2/base/cache/temp_vdprintf.txt");
}
...
static int VdprintfTestImpl(int argc, char *argv[])
{
    Vdprintf0100();
    Vdprintf0200();
    return g_tStatus;
...
int VdprintfTest(void)
{
    ...
    return VdprintfTestImpl(1, libcArgv);
}
