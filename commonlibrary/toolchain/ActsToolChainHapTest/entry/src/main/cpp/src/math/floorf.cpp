...
static void FloorfTestOne(struct FF* p, int* err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkcr(y, p->y, p->r)) {
        ...
        printf("%s:%d: %s floorf(%a) want %a got %a Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
               static_cast<double>(p->x),
...
static int FloorfTestImpl(void)
{
#pragma STDC FENV_ACCESS ON
    int i;
    int err = 0;
    for (i = 0; i < sizeof t / sizeof *t; i++) {
        FloorfTestOne(t + i, &err);
    }
    return !!err;
}
