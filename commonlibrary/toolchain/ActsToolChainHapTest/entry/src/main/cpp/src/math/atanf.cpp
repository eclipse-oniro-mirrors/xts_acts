...
static void AtanfTestOne(struct FF* p, int* err)
{
#pragma STDC FENV_ACCESS ON
    ...
    if (!Checkexcept(e, p->e, p->r)) {
        ...
        printf(
            "%s:%d: bad fp exception: %s atanf(%a)=%a, want %s", p->file, p->line, Rstr(p->r), excX, excY, Estr(p->e));
        printf(" got %s\n", Estr(e));
        (*err)++;
    }
    ...
    if (!Checkulp(d, p->r)) {
        ...
        printf("%s:%d: %s atanf(%a) want %a got %a Ulperr %.3f = %.3f + %.3f\n",
               p->file,
               p->line,
               Rstr(p->r),
