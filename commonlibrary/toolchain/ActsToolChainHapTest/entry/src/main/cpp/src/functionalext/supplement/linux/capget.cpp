 ...
 * @tc.level     : Level 2
 */
void Capget0200(void)
{
    struct __user_cap_data_struct cap_data_data;
    cap_user_data_t cap_data = &cap_data_data;

    int result = capget(nullptr, cap_data);
    if (result != -1) {
        t_error("%s capget should be failed\n", __func__);
    }
}

static int CapgetTestWithArgs(int argc, char* argv[])
{
    Capget0100();
    Capget0200();
    return T_STATUS;
...
int CapgetTest(void)
{
    ...
    return CapgetTestWithArgs(1, libcArgvStub);
}
