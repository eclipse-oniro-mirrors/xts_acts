...
/**
 * @tc.name      : Sigwaitinfo0100
 * @tc.desc      : wait for queued signals
 * @tc.level     : Level 0
 */
void Sigwaitinfo0100(void)
{
    // block a signal
    sigset_t set;
    sigemptyset(&set);
...
static int SigwaitinfoTestImpl(int argc, char* argv[])
{
    Sigwaitinfo0100();
...
int SigwaitinfoTest(void)
{
    static char libcProgStub[] = "libc_test";
    char* libcArgvStub[] = { libcProgStub, nullptr };
    return SigwaitinfoTestImpl(1, libcArgvStub);
}
