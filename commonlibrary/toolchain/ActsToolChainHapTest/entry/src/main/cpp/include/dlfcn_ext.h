 ...
 *        If DL_EXT_WRITE_RELRO is set in extinfo, the GNU RELRO section will be written to relroFd and allowed to
 *        reused by other process loading the same library at the same address.
 *        IF DL_EXT_USE_RELRO is set in extinfo, the GNU RELRO section written in relroFd will be reused.
 * @param file Equivalent to the argument of dlopen.
 * @param mode Equivalent to the argument of dlopen.
 * @param extinfo Indicates the DlExtinfo struct.
 * @return Returns a non-NULL handle for the loaded object on success. On error returns NULL.
 */
void* DlopenExt(const char* file, int mode, const DlExtinfo* extinfo);

/**
 * @brief open dso in given namespace which has own lib search paths,
 *        when namespace is null, it's same to DlopenExt().
 ...
 *    -- RTLD_GLOBAL.
 *    -- RTLD_LOCAL.
 * @param DlExtinfo * indicates the DlExtinfo struct,include flag and relroFd.
 * @return success: dynamic library handleoid,failed: NULL.
 * @retval none.
 */
void* dlopen_ns_ext(Dl_namespace*, const char*, int, const DlExtinfo*);
