...
static JSVM_Value GetHeapStatistics(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    if (result.numberOfNativeContexts < 0) {
        OH_JSVM_ThrowError(env, nullptr, "Test JSVM OH_JSVM_GetHeapStatistics failed");
    }
...
static JSVM_Value SetTypeTagToObject(JSVM_Env env, JSVM_CallbackInfo info)
{
    ...
    if (TagsData[0].upper == 0) {
        OH_JSVM_ThrowError(env, "Reconnect error", "A destroyed TagsData");
    }
    if (TagsData[0].lower == 0) {
        OH_JSVM_ThrowError(env, "Reconnect error", "A destroyed TagsData");
    }
    JSVM_Value result = nullptr;
    OH_JSVM_GetBoolean(env, true, &result);
...
static JSVM_CallbackStruct param[] = {
    ...
    {.callback = TestCpuProfilerCase01, .data = nullptr},
...
static JSVM_PropertyDescriptor jsDescriptor[] = {
    ...
    {"testCpuProfilerCase01", nullptr, nullptr, nullptr, nullptr, nullptr, JSVM_DEFAULT},
...
[[maybe_unused]] static napi_value ThresholdTest(napi_env env, napi_callback_info info) {
    (void)RunTestsWithPrefix("test_threshold.cpp");
    napi_value result;
    napi_create_int32(env, 0, &result);
    return result;
}
...
static napi_value Init(napi_env env, napi_value exports)
{
    ...
    napi_property_descriptor properties[] = {
        ...
        DECLARE_NAPI_FUNCTION("testThreshold", ThresholdTest),
    };
