...
import TestTsAopAdd from "./TestTsAopAdd"
import { NameSpaceAspect } from "./Aspect"
...
export function testAddBeforeInit() {
  NameSpaceAspect.addBefore(TestTsAopAdd, 'addForAddBefore', (a, b) => {
    addBeforeArgA = a;
    addBeforeArgB = b;
  });
}
...
export function testAddAfterInit() {
  NameSpaceAspect.addAfter(TestTsAopAdd, 'addForAddAfter', (that, ret, a, b) => {
    addAfterOriRet = ret;
    addAfterArgA = a;
    addAfterArgB = b;
    return ret + 1;
...
export function testReplaceInit() {
  NameSpaceAspect.replace(TestTsAopAdd, 'addForReplace', (a, b) => {
    replaceArgA = a;
    replaceArgB = b;
    return a * b;
  });
...
export function testReplaceKitUseDeclareNapiStaticFunction() {
  NameSpaceAspect.replace(cameraPicker, 'pick', (context, mediaTypes, pickerProfile) => {
    return `args: mediaTypes:${JSON.stringify(mediaTypes)},pickerProfile:${JSON.stringify(pickerProfile)}`
  });
}
...
export function testReplaceKitUseDeclareNapiFunction() {
  NameSpaceAspect.replace(ble, 'startBLEScan', (filters: Array<ble.ScanFilter>, options?: ble.ScanOptions) => {
...
export function testReplaceKitUseDeclareNapiFunctionWithData() {
  NameSpaceAspect.replace(relationalStore, 'getRdbStore', (context, config, callback) => {
    getRdbStoreArgs = `args: config:${JSON.stringify(config)}}`
  });
}
