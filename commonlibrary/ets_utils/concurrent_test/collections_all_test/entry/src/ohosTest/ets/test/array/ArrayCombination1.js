...
export function ArrayConstructor1Test036() {
    const arr = [[1], [2]];
    const r1 = arr.flatMap(x => x).fill(null, 1);
    const typedArr = [new Float32Array([1]), new Float32Array([2])];
    const r2 = Array.from(typedArr.flatMap(x => Array.from(x))).fill(null, 1);
    return [r1, r2]
}
