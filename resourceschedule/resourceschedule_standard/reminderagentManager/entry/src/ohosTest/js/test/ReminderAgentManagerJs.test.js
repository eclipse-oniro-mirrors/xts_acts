/*
 * Copyright (C) 2021-2025 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// @ts-nocheck
import { describe, beforeAll, beforeEach, afterEach, afterAll, it, expect, Level } from '@ohos/hypium'
import reminderAgent from '@ohos.reminderAgentManager'
import notificationManager from '@ohos.notificationManager';
import { Driver, ON } from '@ohos.UiTest';

export default function ReminderAgentManagerTest() {
    describe('ReminderAgentManagerTest', function () {
        function sleep(ms) {
            console.info(`ReminderAgentManagerTest SUB_NOTIFICATION_ANS_Publish_TEST ===>====>sleep is success`);
            return new Promise(resolve => setTimeout(resolve, ms));
        }
        // 格式化时间函数（返回对象，包含年月日时分秒）
        function getFormattedParts(timestamp) {
            console.info(`ReminderAgentManagerTest ===>====>getFormattedParts is success`);
            const date = new Date(timestamp);
            const year = date.getFullYear();
            const month = date.getMonth() + 1;
            const day = date.getDate();
            const hour = date.getHours();
            const minute = date.getMinutes();
            const second = date.getSeconds();
        
            return {
              year,
              month,
              day,
              hour,
              minute,
              second
            };
        }
        beforeAll(async function (done) {

            /*
             * @tc.setup: setup invoked before all testcases
             */
            console.info(`ReminderAgentManagerTest beforeAll called click usb button`)
            await sleep(500);
            let usb = Driver.create();
            await sleep(500)
            let usbButton = await usb.findComponent(ON.type('Button').text('确定').within(ON.type('Dialog')));
            console.info(`usb button is ${JSON.stringify(usbButton)}`);
            if (usbButton !== null) {
                await sleep(1000);
                await usbButton.click();
                await sleep(500);
            }

            await sleep(500);
            console.info(`ReminderAgentManagerTest beforeAll called requestEnableNotification`)
            notificationManager.requestEnableNotification(async (err) => {
            console.info(`ReminderAgentManagerTest come in requestEnableNotification`);
              if (err) {
                console.info(`ReminderAgentManagerTest requestEnableNotification failed, code is ${err.code}, message is ${err.message}`);
                expect(false).assertTrue();
                done();
              } else {
                console.info("ReminderAgentManagerTest requestEnableNotification success");
                expect(true).assertTrue();
                await sleep(1000);
                done();
              }
            });
        

            await sleep(1000);
            let driver = await Driver.create();
            console.info(`ReminderAgentManagerTest come in driveFn`);
            await sleep(1000);
            console.info(`ReminderAgentManagerTest driver is ${JSON.stringify(driver)}`);
            let button = await driver.findComponent(ON.text('允许').within(ON.type('Dialog')));
            console.info(`ReminderAgentManagerTest button is ${JSON.stringify(button)}`);
            if(button != null){
                await button.click();
                console.info(`ReminderAgentManagerTest button is click`);
                await sleep(3000);
                done();
            }else{
                console.info(`ReminderAgentManagerTest button is null`);
                await sleep(1000);
                done(); 
            } 
        })

        afterAll(function () {

            /*
             * @tc.teardown: teardown invoked after all testcases
             */
            console.info('afterAll caled')
        })

        beforeEach(function () {

            /*
             * @tc.setup: setup invoked before each testcases
             */
            console.info('beforeEach caled')
        })

        afterEach(function () {

            /*
             * @tc.teardown: teardown invoked after each testcases
             */
            console.info('afterEach caled')
        })

        console.info('start################################start');

        /**
         * @tc.name   reminderRequestAttribute_0001
         * @tc.number reminderRequestAttribute_0001
         * @tc.desc   test ReminderRequest Attribute tapDismissed and autoDeletedTime
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0001", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0001---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                tapDismissed: true,
                autoDeletedTime: 20000000000
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0001 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0001 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("reminderRequestAttribute_0001 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0002
         * @tc.number reminderRequestAttribute_0002
         * @tc.desc   test ReminderRequest Attribute groupId
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0002", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0002---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 20,
                groupId: ''
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0002 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0002 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("reminderRequestAttribute_0002 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0003
         * @tc.number reminderRequestAttribute_0003
         * @tc.desc   test ReminderRequest ActionButton titleResource
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0003", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0003---------------------------');
            const currentYear = new Date().getFullYear();
            const nextYear = currentYear + 1
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: nextYear,
                    month: 1,
                    day: 11,
                    hour: 11,
                    minute: 11,
                    second: 11
                },
                actionButton: [
                    {
                        title: 'close',
                        type: 0,
                    }
                ]
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0003 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0003 callback reminderId = ' + reminderId);
                        try {
                            reminderAgent.getValidReminders((err, reminder) => {
                                if (reminder.length = 0) {
                                    reminder = [{ titleResource: '' }]
                                }
                                console.info('reminderRequestAttribute_0003 callback reminder = ' + reminder.titleResource);
                                expect(reminderId).assertLarger(0);
                                done();
                            })
                        } catch (error) {
                            console.log("reminderRequestAttribute_0003 getValidReminders error.code:" + error.code);
                        }
                    }
                })
            } catch (error) {
                console.log("reminderRequestAttribute_0003 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0005
         * @tc.number reminderRequestAttribute_0005
         * @tc.desc   test ReminderRequest snoozeSlotType
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0005", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0005---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                slotType: 2,
                snoozeSlotType: 2
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0005 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0005 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("reminderRequestAttribute_0005 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0006
         * @tc.number reminderRequestAttribute_0006
         * @tc.desc   test ReminderRequest customRingUri
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0006", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0006---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                customRingUri: "#"
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0006 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0006 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("reminderRequestAttribute_0006 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0004
         * @tc.number reminderRequestAttribute_0004
         * @tc.desc   test timer normal parameter,return with promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0004", Level.LEVEL0, async function (done) {
            console.info('----------------------testPublishReminderTimer_030---------------------------');
            try {
                reminderAgent.cancelAllReminders().then((err, data) => {
                    console.info('reminderRequestAttribute_0004 cancelAllReminders success');
                    expect(true).assertTrue();
                    done();
                });
            } catch (error) {
                console.log("reminderRequestAttribute_0004 cancelAllReminders error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0007
         * @tc.number reminderRequestAttribute_0007
         * @tc.desc   test publishReminder callbacl REMINDER_TYPE_CALENDAR
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0007", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0007---------------------------');
            let calendar = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: 2024,
                    month: 1,
                    day: 31,
                    hour: 10,
                    minute: 30,
                    second: 10
                },
                repeatMonths: [3],
                repeatDays: [6, 7],
                actionButton: [
                    {
                        title: "close",
                        titleResource: 'English',
                        type: 0,
                    },
                    {
                        title: "snooze",
                        titleResource: 'English',
                        type: 1
                    }
                ],
                wantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                maxScreenWantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                ringDuration: 2,
                snoozeTimes: 1,
                timeInterval: 3,
                title: "this is title",
                content: "this is content",
                expiredContent: "this reminder has expired",
                snoozeContent: "remind later",
                notificationId: 500,
                groupId: 'this is groupId',
            }
            try {
                reminderAgent.publishReminder(calendar, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0007 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0007 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0007 publishReminder error.code:" + error.code);
            }
        })


        /**
         * @tc.name   reminderRequestAttribute_0008
         * @tc.number reminderRequestAttribute_0008
         * @tc.desc   test publishReminder promise REMINDER_TYPE_CALENDAR
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0008", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0008---------------------------');
            let calendar = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: 2024,
                    month: 1,
                    day: 28,
                    hour: 13,
                    minute: 14,
                    second: 32
                },
                repeatMonths: [5],
                repeatDays: [1, 2, 3, 4, 5],
                actionButton: [
                    {
                        title: "close",
                        titleResource: 'English',
                        type: 0,
                    },
                    {
                        title: "snooze",
                        titleResource: 'English',
                        type: 1
                    }
                ],
                wantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                maxScreenWantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                ringDuration: 10,
                snoozeTimes: 3,
                timeInterval: 30,
                title: "#",
                content: "#",
                expiredContent: "#",
                snoozeContent: "#",
                notificationId: 1000,
                groupId: '#',
            }
            try {
                reminderAgent.publishReminder(calendar).then((reminderId) => {
                    console.info('reminderRequestAttribute_0008 promise reminderId = ' + reminderId);
                    expect(reminderId).assertLarger(0);
                    done();
                }).catch((err) => {
                    console.info('reminderRequestAttribute_0008 promise err.code is :' + err.code);
                });
            } catch (error) {
                console.info("reminderRequestAttribute_0008 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0009
         * @tc.number reminderRequestAttribute_0009
         * @tc.desc   test getValidReminders callback
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0009", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0009---------------------------');
            try {
                reminderAgent.getValidReminders((err, reminders) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0009 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0009 callback reminders.length = ' + reminders.length);
                        expect(reminders.length).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0009 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0010
         * @tc.number reminderRequestAttribute_0010
         * @tc.desc   test getValidReminders promise
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0010", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0010---------------------------');
            try {
                reminderAgent.getValidReminders().then((reminders) => {
                    console.info('reminderRequestAttribute_0010 promise reminders.length = ' + reminders.length);
                    expect(reminders.length).assertLarger(0);
                    done();
                }).catch((err) => {
                    console.info('reminderRequestAttribute_0010 promise err.code is :' + err.code);
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0010 getValidReminders error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0011
         * @tc.number reminderRequestAttribute_0011
         * @tc.desc   test cancelAllReminders callback.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0011", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0011---------------------------');
            try {
                reminderAgent.cancelAllReminders((err, data) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0011 cancelAllReminders callback er.code:' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0011 cancelAllReminders callback date =' + data);
                        expect(true).assertTrue();
                        done();
                    }
                });
            } catch (error) {
                console.info("reminderRequestAttribute_0011 cancelAllReminders error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0012
         * @tc.number reminderRequestAttribute_0012
         * @tc.desc   test addNotificationSlot promise mySlot type 3
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it('reminderRequestAttribute_0012', Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0012---------------------------');
            let mySlot = {
                type: 5
            }
            try {
                reminderAgent.addNotificationSlot(mySlot).then(() => {
                    console.info('reminderRequestAttribute_0012  promise mySliot.type is :' + mySlot.type)
                    expect(mySlot.type).assertEqual(5);
                    done();
                }, (err) => {
                    console.info('reminderRequestAttribute_0012 promise err.code is :' + err.code)
                });
            } catch (error) {
                console.info('reminderRequestAttribute_0012 addNotificationSlot error.code is :' + error.code)
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0013
         * @tc.number reminderRequestAttribute_0013
         * @tc.desc   test removeNotificationSlot promise
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it('reminderRequestAttribute_0013', Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0013---------------------------');
            try {
                reminderAgent.removeNotificationSlot(5).then(() => {
                    console.info("reminderRequestAttribute_0013 promise success");
                    expect(true).assertTrue();
                    done();
                }).catch((err) => {
                    console.info("reminderRequestAttribute_0013 promise err.code:" + err.code);
                });
            } catch (error) {
                console.info("removeNotificationSlot promise error.code:" + error.code)
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0014
         * @tc.number reminderRequestAttribute_0014
         * @tc.desc   test addNotificationSlot callback mySlot type 2
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it('reminderRequestAttribute_0014', Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0014---------------------------');
            let mySlot = {
                type: 10086
            }
            try {
                reminderAgent.addNotificationSlot(mySlot, (err) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0014 callback err.code is :' + err.code)
                    } else {
                        console.info('reminderRequestAttribute_0014 callback success ')
                    }
                });
            } catch (error) {
                console.info('reminderRequestAttribute_0014 addNotificationSlot error.code is :' + error.code)
                expect(error.code).assertEqual('401');
                done();
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0015
         * @tc.number reminderRequestAttribute_0015
         * @tc.desc   test removeNotificationSlot callback
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it('reminderRequestAttribute_0015', Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0015---------------------------');
            try {
                reminderAgent.removeNotificationSlot(10086, (err) => {
                    if (err) {
                        console.info("reminderRequestAttribute_0015 callback err.code:" + err.code);
                    } else {
                        console.info("reminderRequestAttribute_0015 callback success");
                    }
                });
            } catch (error) {
                console.info("reminderRequestAttribute_0015 removeNotificationSlot error.code:" + error.code)
                expect(error.code).assertEqual(401);
                done();
            }
        })


        /**
         * @tc.name   reminderRequestAttribute_0016
         * @tc.number reminderRequestAttribute_0016
         * @tc.desc   test UNKNOWN_TYPE default parameter with 0.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0016", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0016---------------------------');
            let type = notificationManager.SlotType.UNKNOWN_TYPE;
            expect(type).assertEqual(0);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0017
         * @tc.number reminderRequestAttribute_0017
         * @tc.desc   test SOCIAL_COMMUNICATION default parameter with 1.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0017", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0017---------------------------');
            let type = notificationManager.SlotType.SOCIAL_COMMUNICATION;
            expect(type).assertEqual(1);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0018
         * @tc.number reminderRequestAttribute_0018
         * @tc.desc   test SERVICE.INFORMATION default parameter with 2.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0018", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0018---------------------------');
            let type = notificationManager.SlotType.SERVICE_INFORMATION;
            expect(type).assertEqual(2);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0019
         * @tc.number reminderRequestAttribute_0019
         * @tc.desc   test CONTENT_INFORMATION default parameter with 3.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0019", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0019---------------------------');
            let type = notificationManager.SlotType.CONTENT_INFORMATION;
            expect(type).assertEqual(3);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0020
         * @tc.number reminderRequestAttribute_0020
         * @tc.desc   test CONTENT_INFORMATION default parameter with 3.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0020", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0020---------------------------');
            let type = notificationManager.SlotType.CUSTOMER_SERVICE;
            expect(type).assertEqual(5);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0021
         * @tc.number reminderRequestAttribute_0021
         * @tc.desc   test OTHER_TYPES default parameter with 65535.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0021", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0021`---------------------------');
            let type = notificationManager.SlotType.OTHER_TYPES;
            expect(type).assertEqual(65535);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0022
         * @tc.number reminderRequestAttribute_0022
         * @tc.desc   test button close default parameter with 0.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0022", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0022---------------------------');
            let button = reminderAgent.ActionButtonType.ACTION_BUTTON_TYPE_CLOSE;
            expect(button).assertEqual(0);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0023
         * @tc.number reminderRequestAttribute_0023
         * @tc.desc   test button snooze default parameter with 1.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0023", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0023---------------------------');
            let button = reminderAgent.ActionButtonType.ACTION_BUTTON_TYPE_SNOOZE;
            expect(button).assertEqual(1);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0024
         * @tc.number reminderRequestAttribute_0024
         * @tc.desc   test publishReminder REMINDER_TYPE_ALARM
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0024", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0024---------------------------');
            try {
                let alarm = {
                    reminderType: reminderAgent.ReminderType.REMINDER_TYPE_ALARM,
                    hour: 13,
                    minute: 14,
                    daysOfWeek: [1, 2, 3, 4, 5]
                }
                reminderAgent.publishReminder(alarm).then((reminderId) => {
                    if (reminderId != -1) {
                        console.info("reminderRequestAttribute_0024 reminderId =" + reminderId);
                        expect(reminderId).assertLarger(0);
                        done()
                    }
                }).catch((err => {
                    console.info('reminderRequestAttribute_0024 promise err.code is :' + err.code);
                }))
            } catch (error) {
                console.info("reminderRequestAttribute_0024 publishReminder err.code:" + error.code);
            }

        })

        /**
         * @tc.name   reminderRequestAttribute_0025
         * @tc.number reminderRequestAttribute_0025
         * @tc.desc   test reminderRequest WantAgent parameters.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0025", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0025---------------------------');
            let timer = {
                reminderType:reminderAgent.ReminderType.REMINDER_TYPE_ALARM,
                hour:6,
                minute:30,
                daysOfWeek:[1],
                actionButton:[{
                    title:'close',
                    titleResource:'this is 0025 close button',
                    type:reminderAgent.ActionButtonType.ACTION_BUTTON_TYPE_CLOSE
                }],
                wantAgent:{
                    pkgName:"ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName:"ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility",
                    parameters:{"aa":"aaa"},
                    uri:"#"
                }
            }
            try{
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if(err){
                        console.info('reminderRequestAttribute_0025 callback err.code is : ' + err.code)
                    }else{
                        console.info('reminderRequestAttribute_0025 callback reminderId is : ' + reminderId)
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            }catch(error){
                console.info('reminderRequestAttribute_0025 publishReminder err.code is : ' + err.code)
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0026
         * @tc.number reminderRequestAttribute_0026
         * @tc.desc   test getValidReminders promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0026", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0026---------------------------');
            try{
                reminderAgent.getAllValidReminders().then((reminderInfo) => {
                    console.info('reminderRequestAttribute_0026 reminderInfo = ' + JSON.stringify(reminderInfo));
                    let reminderInfoArr = [];
                    reminderInfoArr = reminderInfo;
                    let reminderId = 0;
                    let reminderReq = {};
                    console.info('reminderRequestAttribute_0026 reminderInfoArr = ' + JSON.stringify(reminderInfoArr));
                    for(let i = 0; i < reminderInfoArr.length; i++){
                        console.info('reminderRequestAttribute_0026 for is ok');
                        if(reminderInfoArr[i].reminderReq.minute == 30){
                            console.info('reminderRequestAttribute_0026 if is ok');
                            reminderId = reminderInfoArr[i].reminderId;
                            Object.assign(reminderReq, reminderInfoArr[i].reminderReq);
                            break;
                        }
                    }
                    console.info('reminderRequestAttribute_0026 = ' + JSON.stringify(reminderReq));
                    expect(reminderInfoArr.length).assertLarger(0);
                    expect(reminderId).assertLarger(0);
                    expect(reminderReq.hour).assertEqual(6);
                    done();
                }).catch((err) =>{
                    console.info('reminderRequestAttribute_0026 promise err.code is : ' + err.code);
                })
            }catch(error){
                console.info('reminderRequestAttribute_0026 getValidReminders error.code is : ' + error.code);
            }
        })
        
        /**
         * @tc.name   reminderRequestAttribute_0027
         * @tc.number reminderRequestAttribute_0027
         * @tc.desc   test timer normal parameter, return with promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0027", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0027---------------------------');
            try{
                reminderAgent.cancelAllReminders().then((err, data) => {
                    console.info('reminderRequestAttribute_0027 cancelAllReminders success');
                    expect(true).assertTrue();
                    done();
                })
            }catch(error){
                console.info('reminderRequestAttribute_0027 cancelAllReminders error.code is : ' + error.code);
            }
        })
        
        /**
         * @tc.name   reminderRequestAttribute_0028
         * @tc.number reminderRequestAttribute_0028
         * @tc.desc   test addExcludeDate promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0028", Level.LEVEL0, async function (done) {
            let calendar = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: 2024,
                    month: 7,
                    day: 28,
                    hour: 13,
                    minute: 30,
                    second: 32
                },
                endDateTime: {
                    year: 2024,
                    month: 7,
                    day: 31,
                    hour: 10,
                    minute: 30,
                    second: 10
                },
                repeatMonths: [3],
                repeatDays: [6, 7],
                actionButton: [
                    {
                        title: "close",
                        titleResource: 'English',
                        type: 0,
                    },
                    {
                        title: "snooze",
                        titleResource: 'English',
                        type: 1
                    }
                ],
                wantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                maxScreenWantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                ringDuration: 2,
                snoozeTimes: 1,
                timeInterval: 3,
                title: "this is addExcludeDate title",
                content: "this is content",
                expiredContent: "this reminder has expired",
                snoozeContent: "remind later",
                notificationId: 500,
                groupId: 'this is groupId',
            }
            try {
                reminderAgent.publishReminder(calendar, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0028 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0028 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                    let reqReminderId = reminderId;
                    let date = new Date();
                    reminderAgent.addExcludeDate(reqReminderId, date).then(() => {
                        expect(true).assertTrue();
                        done();
                    }).catch((err) => {
                        console.error("promise err code:" + err.code + " message:" + err.message);
                        expect(false).assertTrue();
                        done();
                    });
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0028 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0029
         * @tc.number reminderRequestAttribute_0029
         * @tc.desc   test getExcludeDates promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0029", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0029---------------------------');
            try {
                reminderAgent.getAllValidReminders().then((reminderInfo) => {
                    console.info('reminderRequestAttribute_0029 reminderInfo = ' + JSON.stringify(reminderInfo));
                    let reminderInfoArr = [];
                    reminderInfoArr = reminderInfo;
                    let reminderId = 0;
                    console.info('reminderRequestAttribute_0029 reminderInfoArr = ' + JSON.stringify(reminderInfoArr));
                    for (let i = 0; i < reminderInfoArr.length; i++) {
                        console.info('reminderRequestAttribute_0029 for is ok');
                        if (reminderInfoArr[i].reminderReq.dateTime.month == 7) {
                            reminderId = reminderInfoArr[i].reminderId;
                            break;
                        }
                    }
                    console.info('reminderRequestAttribute_0029 reminderId' + reminderId)
                    reminderAgent.getExcludeDates(reminderId).then((dates) => {
                        console.info('reminderRequestAttribute_0029 dates: ' + JSON.stringify(dates));
                        expect(dates.length).assertLarger(0)
                        done();
                    }).catch((err) => {
                        console.error("promise err code:" + err.code + " message:" + err.message);
                        expect(false).assertTrue();
                        done();
                    });
                }).catch((err) => {
                    console.info('reminderRequestAttribute_0029 promise err.code is : ' + err.code);
                })
            } catch (error) {
                console.info('reminderRequestAttribute_0029 getValidReminders error.code is : ' + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0030
         * @tc.number reminderRequestAttribute_0030
         * @tc.desc   test deleteExcludeDates promise.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0030", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0030---------------------------');
            try {
                reminderAgent.getAllValidReminders().then((reminderInfo) => {
                    console.info('reminderRequestAttribute_0030 reminderInfo = ' + JSON.stringify(reminderInfo));
                    let reminderInfoArr = [];
                    reminderInfoArr = reminderInfo;
                    let reminderId = 0;
                    console.info('reminderRequestAttribute_0030 reminderInfoArr = ' + JSON.stringify(reminderInfoArr));
                    for (let i = 0; i < reminderInfoArr.length; i++) {
                        console.info('reminderRequestAttribute_0030 for is ok');
                        if (reminderInfoArr[i].reminderReq.dateTime.month == 7) {
                            console.info('reminderRequestAttribute_0030 if is ok');
                            reminderId = reminderInfoArr[i].reminderId;
                            break;
                        }
                    }
                    
                    reminderAgent.deleteExcludeDates(reminderId).then(() => {
                        expect(true).assertTrue();
                        done();
                    }).catch((err) => {
                        console.error("promise err code:" + err.code + " message:" + err.message);
                        expect(false).assertTrue();
                        done();
                    });
                }).catch((err) => {
                    console.info('reminderRequestAttribute_0030 promise err.code is : ' + err.code);
                })
            } catch (error) {
                console.info('reminderRequestAttribute_0030 getValidReminders error.code is : ' + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0031
         * @tc.number reminderRequestAttribute_0031
         * @tc.desc   test cancelReminder promise function
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0031", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0031---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0031 cancelAllReminders success');
                expect(true).assertTrue();
            });
            const currentYear = new Date().getFullYear();
            const nextYear = currentYear + 1
            let ReminderRequest = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: nextYear,
                    month: 1,
                    day: 11,
                    hour: 11,
                    minute: 11,
                    second: 11
                },
                actionButton: [
                    {
                        title: 'close',
                        type: 0,
                    }
                ],
                titleResourceId: 123
            }
            try {
                reminderAgent.publishReminder(ReminderRequest, (err, reminderId) => {
                    if (err) {                           
                        console.log('reminderRequestAttribute_0031 callback err.code is :' + err.code);
                    }
                    expect(reminderId).assertLarger(0);
                    console.log('reminderRequestAttribute_0031 callback reminderId = ' + reminderId);
                    reminderAgent.cancelReminder(reminderId).then(() => {
                        console.log("reminderRequestAttribute_0031 cancelReminder promise");
                        expect(true).assertTrue();
                        done();
                      }).catch((err) => {
                        console.log("reminderRequestAttribute_0031 promise err code:" + err.code + " message:" + err.message);
                        expect(false).assertTrue();
                        done();
                      });
                })             
            } catch (error) {
                console.log("reminderRequestAttribute_0031 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0032
         * @tc.number reminderRequestAttribute_0032
         * @tc.desc   test cancelReminder callback function
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0032", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0032---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0032 cancelAllReminders success');
                expect(true).assertTrue();
            });
            const currentYear = new Date().getFullYear();
            const nextYear = currentYear + 1
            let ReminderRequest = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: nextYear,
                    month: 1,
                    day: 11,
                    hour: 11,
                    minute: 11,
                    second: 11
                },
                actionButton: [
                    {
                        title: 'close',
                        type: 0,
                    }
                ],
                titleResourceId: 123
            }
            try {
                reminderAgent.publishReminder(ReminderRequest, (err, reminderId) => {
                    if (err) {                           
                        console.error('reminderRequestAttribute_0032 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0032 callback reminderId = ' + reminderId);
                        let id = reminderId
                        reminderAgent.cancelReminder(id, (err) => {
                            if (err) {
                              console.error("reminderRequestAttribute_0032 cancelReminder callback err code:" + err.code + " message:" + err.message);
                              expect(false).assertTrue();
                              done();
                            } else {
                              console.log("reminderRequestAttribute_0032 cancelReminder callback");
                              expect(true).assertTrue();
                              done();
                            }
                        });
                    }
                })             
            } catch (error) {
                console.log("reminderRequestAttribute_0032 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0033
         * @tc.number reminderRequestAttribute_0033
         * @tc.desc   test reminderAgent.RingChannel.RING_CHANNEL_ALARM is 0.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0033", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0033---------------------------');
            let ringType = reminderAgent.RingChannel.RING_CHANNEL_ALARM;
            expect(ringType).assertEqual(0);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0034
         * @tc.number reminderRequestAttribute_0034
         * @tc.desc   test reminderAgent.RingChannel.RING_CHANNEL_ALARM is 0.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0034", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0034---------------------------');
            let ringType = reminderAgent.RingChannel.RING_CHANNEL_MEDIA;
            expect(ringType).assertEqual(1);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0035
         * @tc.number reminderRequestAttribute_0035
         * @tc.desc   test updateReminder promise function
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0035", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0035---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0035 cancelAllReminders success');
            });
            const currentYear = new Date().getFullYear();
            const nextYear = currentYear + 1
            let ReminderRequest = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: nextYear,
                    month: 1,
                    day: 11,
                    hour: 11,
                    minute: 11,
                    second: 11
                },
                actionButton: [
                    {
                        title: 'close',
                        type: 0,
                    }
                ],
                ringChannel: reminderAgent.RingChannel.RING_CHANNEL_ALARM
            }
            let timer = {
              reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
              triggerTimeInSeconds: 10
            }
            try {
                reminderAgent.publishReminder(ReminderRequest, (err, reminderId) => {
                    if (err) {                           
                        console.error('reminderRequestAttribute_0035 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0035 callback reminderId = ' + reminderId);
                        let id = reminderId;
                        reminderAgent.updateReminder(id, timer).then(() => {
                            console.log("reminderRequestAttribute_0035 update reminder succeed");
                            expect(true).assertTrue();
                            done();
                        }).catch((err) => {
                            console.error("reminderRequestAttribute_0035 promise err code:" + err.code + " message:" + err.message);
                            expect(false).assertTrue();
                            done();
                        });
                    }
                })             
            } catch (error) {
                console.log("reminderRequestAttribute_0035 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0036
         * @tc.number reminderRequestAttribute_0036
         * @tc.desc   test getValidReminders promise function res is null
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0036", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0036---------------------------');
            reminderAgent.cancelAllReminders().then((err) => {
                console.info('reminderRequestAttribute_0036 cancelAllReminders success');
                try {
                    reminderAgent.getValidReminders().then((reminders) => {
                        console.info('reminderRequestAttribute_0036 promise reminders.length = ' + reminders.length);
                        expect(reminders.length).assertEqual(0);
                        done();
                    }).catch((err) => {
                        console.info('reminderRequestAttribute_0036 promise err.code is :' + err.code);
                    })
                } catch (error) {
                    console.info("reminderRequestAttribute_0036 getValidReminders error.code:" + error.code);
                }
            });
        })

        /**
         * @tc.name   reminderRequestAttribute_0037
         * @tc.number reminderRequestAttribute_0037
         * @tc.desc   test getValidReminders callback function res is null
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0037", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0037---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0037 cancelAllReminders success');
            });
            try {
                reminderAgent.getValidReminders((err, reminders) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0037 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0037 callback reminders.length = ' + reminders.length);
                        expect(reminders.length).assertEqual(0);
                        done();
                    }
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0037 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0038
         * @tc.number reminderRequestAttribute_0038
         * @tc.desc   test getAllValidReminders promise function res is null
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0038", Level.LEVEL0, async function (done) {
            console.info('----------------------reminderRequestAttribute_0038---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0038 cancelAllReminders success');
            });
            try {
                reminderAgent.getAllValidReminders().then((reminderInfo) => {
                    console.info('reminderRequestAttribute_0038 reminderInfo = ' + JSON.stringify(reminderInfo));
                    let reminderInfoArr = [];
                    reminderInfoArr = reminderInfo;
                    console.info('reminderRequestAttribute_0038 reminderInfoArr = ' + JSON.stringify(reminderInfoArr));
                    console.info('reminderRequestAttribute_0038 reminderInfoArr = ' + JSON.stringify(reminderInfoArr.length));
                    expect(reminderInfoArr.length).assertEqual(0);
                    done();
                }).catch((err) =>{
                    console.info('reminderRequestAttribute_0038 promise err.code is : ' + err.code);
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0038 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0039
         * @tc.number reminderRequestAttribute_0039
         * @tc.desc   test getExcludeDates promise function res is null
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL0
         */
        it("reminderRequestAttribute_0039", Level.LEVEL0, async function (done) {
            let calendar = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime: {
                    year: 2037,
                    month: 7,
                    day: 28,
                    hour: 13,
                    minute: 30,
                    second: 32
                },
                endDateTime: {
                    year: 2037,
                    month: 7,
                    day: 31,
                    hour: 10,
                    minute: 30,
                    second: 10
                },
                repeatMonths: [3],
                repeatDays: [6, 7],
                actionButton: [
                    {
                        title: "close",
                        titleResource: 'English',
                        type: 0,
                    },
                    {
                        title: "snooze",
                        titleResource: 'English',
                        type: 1
                    }
                ],
                wantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                maxScreenWantAgent: {
                    pkgName: "ohos.acts.resourceschedule.reminderagentMgr.js.function",
                    abilityName: "ohos.acts.resourceschedule.reminderagentMgr.js.function.MainAbility"
                },
                ringDuration: 2,
                snoozeTimes: 1,
                timeInterval: 3,
                title: "this is addExcludeDate title",
                content: "this is content",
                expiredContent: "this reminder has expired",
                snoozeContent: "remind later",
                notificationId: 500,
                groupId: 'this is groupId',
            }
            try {
                reminderAgent.publishReminder(calendar, (err, reminderId) => {
                    if (err) {
                        console.info('reminderRequestAttribute_0039 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0039 callback reminderId = ' + reminderId);
                        reminderAgent.getExcludeDates(reminderId).then((dates) => {
                            console.info('reminderRequestAttribute_0039 dates: ' + JSON.stringify(dates));
                            expect(dates.length).assertEqual(0)
                            done();
                        }).catch((err) => {
                            console.error("reminderRequestAttribute_0039 promise err code:" + err.code + " message:" + err.message);
                            expect(false).assertTrue();
                            done();
                        });
                    }
                })
            } catch (error) {
                console.info("reminderRequestAttribute_0039 publishReminder error.code:" + error.code);
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0040
         * @tc.number reminderRequestAttribute_0040
         * @tc.desc   test reminderAgent.RingChannel.RING_CHANNEL_ALARM is 2.
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0040", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0040---------------------------');
            let ringType = reminderAgent.RingChannel.RING_CHANNEL_NOTIFICATION;
            expect(ringType).assertEqual(2);
            done();
        })

        /**
         * @tc.name   reminderRequestAttribute_0041
         * @tc.number reminderRequestAttribute_0041
         * @tc.desc   test cancelReminderOnDisplay promise function
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0041", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0041---------------------------');
            reminderAgent.cancelAllReminders().then((err, data) => {
                console.info('reminderRequestAttribute_0041 cancelAllReminders success');

            // 获取当前时间戳（毫秒）
            const nowTimestamp = Date.now();
            // 获取当前时间一小时后的时间部分
            const endDateTime = getFormattedParts(nowTimestamp + 3600 * 1000);
            console.info("reminderRequestAttribute_0041 当前时间一小时后的时间部分:", JSON.stringify(endDateTime));
            // 获取当前时间一小时前的时间部分
            const dateTime = getFormattedParts(nowTimestamp - 3600 * 1000);
            console.info("reminderRequestAttribute_0041 当前时间一小时前的时间部分:", JSON.stringify(dateTime));
            let ReminderRequest = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                dateTime,
                endDateTime,
                actionButton: [
                    {
                        title: 'close',
                        type: 0,
                    }
                ],
                ringChannel: reminderAgent.RingChannel.RING_CHANNEL_NOTIFICATION
            }
            let callbakFlag = 0;
            try {
                reminderAgent.publishReminder(ReminderRequest, async (err, reminderId) => {
                    await sleep(5000)
                    if (err) {                           
                        console.error('reminderRequestAttribute_0041 callback err.code is :' + err.code);
                    } else {
                        console.info('reminderRequestAttribute_0041 callback reminderId = ' + reminderId);
                        let id = reminderId;
                        reminderAgent.cancelReminderOnDisplay(id).then(() => {
                            console.log("reminderRequestAttribute_0041 cancelReminderOnDisplay reminder succeed");
                            expect(callbakFlag).assertEqual(0);
                            done();
                        }).catch((err) => {
                            console.error("reminderRequestAttribute_0041 promise err code:" + err.code + " message:" + err.message);
                            expect(false).assertTrue();
                            done();
                        });
                    }
                })             
            } catch (error) {
                console.log("reminderRequestAttribute_0041 publishReminder error.code:" + error.code);
            }
            });
        })

        /**
         * @tc.name   reminderRequestAttribute_0042
         * @tc.number reminderRequestAttribute_0042
         * @tc.desc   test subscribeReminderState promise function and unsubscribeReminderState promise function
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0042", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0042---------------------------');
            let reminderStateCallback = (states) =>{
                console.info('reminderRequestAttribute_0042 length is : ' + states.length)
                if(states === undefined){
                    states=[
                        {
                            reminderId: 1,
                            buttonType: reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR,
                            isMessageResent: true
                        }
                    ];
                    console.info('reminderRequestAttribute_0042 states callback reminderId ' + states[0].reminderId)
                    console.info('reminderRequestAttribute_0042 states callback buttonType ' + states[0].buttonType)
                    console.info('reminderRequestAttribute_0042 states callback isMessageResent ' + states[0].isMessageResent)

                    expect(states[0].reminderId).assertInstanceOf('Number');
                    expect(states[0].buttonType == reminderAgent.ReminderType.REMINDER_TYPE_CALENDAR).assertTrue();
                    expect(states[0].isMessageResent == true).assertTrue();
                    done();
                }
            }

            try {   
                let callbakFlag1 = 1;
                await reminderAgent.subscribeReminderState(reminderStateCallback);
                console.info('reminderRequestAttribute_0042 subscribe succeed');
                expect(callbakFlag1 == 1).assertTrue();
                done();
            } catch (error) {
                console.log("reminderRequestAttribute_0042 subscribeReminderState error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }

            await sleep(1500);

            try {   
                let callbakFlag2 = 1;
                await reminderAgent.unsubscribeReminderState(reminderStateCallback);
                console.info('reminderRequestAttribute_0042 unsubscribe succeed');
                expect(callbakFlag2 == 1).assertTrue();
                done();
            } catch (error) {
                console.log("reminderRequestAttribute_0042 subscribeReminderState error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   reminderRequestAttribute_0043
         * @tc.number reminderRequestAttribute_0043
         * @tc.desc   test api23 unsubscribeReminderState promise function and input is empty
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("reminderRequestAttribute_0043", Level.LEVEL3, async function (done) {
            console.info('----------------------reminderRequestAttribute_0043---------------------------');

            try {   
                let callbakFlag = 1;
                await reminderAgent.unsubscribeReminderState();
                console.info('reminderRequestAttribute_0043 unsubscribe succeed');
                expect(callbakFlag == 1).assertTrue();
                done();
            } catch (error) {
                console.log("reminderRequestAttribute_0043 subscribeReminderState error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100
         * @tc.desc   test publishReminder timer with repeatInterval and repeatCount normal parameters (callback)
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: 5
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100 callback err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0100 publishReminder error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200
         * @tc.desc   test publishReminder timer with repeatInterval and repeatCount normal parameters (promise)
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: 3
            }
            try {
                reminderAgent.publishReminder(timer).then((reminderId) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200 promise reminderId = ' + reminderId);
                    expect(reminderId).assertLarger(0);
                    done();
                }).catch((err) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200 promise err.code is :' + err.code);
                    expect(false).assertTrue();
                    done();
                });
            } catch (error) {
                console.info("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0200 publishReminder error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300
         * @tc.desc   test publishReminder timer with repeatInterval boundary value 86400 (minimum valid value)
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: 1
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300 callback err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0300 publishReminder error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400
         * @tc.desc   test publishReminder timer with repeatCount boundary value 0 (infinite repeat)
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: 0
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400 callback err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0400 publishReminder error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500
         * @tc.desc   test publishReminder timer with repeatInterval invalid value (less than 86400) returns error code 401
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86399,
                repeatCount: 5
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500 callback err.code is :' + err.code);
                        expect(err.code).assertEqual("401");
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500 callback reminderId = ' + reminderId);
                        expect(false).assertTrue();
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0500 publishReminder error.code:" + error.code);
                expect(error.code).assertEqual("401");
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600
         * @tc.desc   test publishReminder timer with repeatCount invalid value (less than 0) returns error code 401
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: -1
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600 callback err.code is :' + err.code);
                        expect(err.code).assertEqual("401");
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600 callback reminderId = ' + reminderId);
                        expect(false).assertTrue();
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0600 publishReminder error.code:" + error.code);
                expect(error.code).assertEqual("401");
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700
         * @tc.desc   test publishReminder timer with repeatInterval large value and repeatCount large value
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 172800,
                repeatCount: 100
            }
            try {
                reminderAgent.publishReminder(timer, (err, reminderId) => {
                    if (err) {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700 callback err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    } else {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700 callback reminderId = ' + reminderId);
                        expect(reminderId).assertLarger(0);
                        done();
                    }
                })
            } catch (error) {
                console.log("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0700 publishReminder error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800
         * @tc.desc   test getAllValidReminders to verify repeatInterval and repeatCount values after publishReminder
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10,
                repeatInterval: 86400,
                repeatCount: 7
            }
            try {
                reminderAgent.publishReminder(timer).then((reminderId) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 publishReminder promise reminderId = ' + reminderId);
                    expect(reminderId).assertLarger(0);
                    reminderAgent.getAllValidReminders().then((reminders) => {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 getAllValidReminders reminders.length = ' + reminders.length);
                        expect(reminders.length).assertLarger(0);
                        let found = false;
                        for (let i = 0; i < reminders.length; i++) {
                            console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 checking reminder: ' + JSON.stringify(reminders[i]) + ' (type: ' + typeof reminders[i] + ')');
                            if (reminders[i].reminderId === reminderId) {
                                console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 found reminder, reminderReq = ' + JSON.stringify(reminders[i].reminderReq));
                                if (reminders[i].reminderReq.repeatInterval !== undefined && reminders[i].reminderReq.repeatCount !== undefined) {
                                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 found reminder, repeatInterval = ' + reminders[i].reminderReq.repeatInterval);
                                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 found reminder, repeatCount = ' + reminders[i].reminderReq.repeatCount);
                                    expect(reminders[i].reminderReq.repeatInterval).assertEqual(86400);
                                    expect(reminders[i].reminderReq.repeatCount).assertEqual(7);
                                }
                                found = true;
                                break;
                            }
                        }
                        expect(found).assertTrue();
                        done();
                    }).catch((err) => {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 getAllValidReminders err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    });
                }).catch((err) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 publishReminder err.code is :' + err.code);
                    expect(false).assertTrue();
                    done();
                });
            } catch (error) {
                console.info("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0800 error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })

        /**
         * @tc.name   SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900
         * @tc.number SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900
         * @tc.desc   test updateReminder with repeatInterval and repeatCount normal parameters (promise)
         * @tc.type   FUNCTION
         * @tc.size   MEDIUMTEST
         * @tc.level  LEVEL3
         */
        it("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900", Level.LEVEL3, async function (done) {
            console.info('----------------------SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900---------------------------');
            let timer = {
                reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                triggerTimeInSeconds: 10
            }
            try {
                reminderAgent.publishReminder(timer).then((reminderId) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 publishReminder promise reminderId = ' + reminderId);
                    expect(reminderId).assertLarger(0);
                    let updateTimer = {
                        reminderType: reminderAgent.ReminderType.REMINDER_TYPE_TIMER,
                        triggerTimeInSeconds: 20,
                        repeatInterval: 86400,
                        repeatCount: 5
                    }
                    reminderAgent.updateReminder(reminderId, updateTimer).then(() => {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 updateReminder promise success');
                        reminderAgent.getAllValidReminders().then((reminders) => {
                            console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 getAllValidReminders reminders.length = ' + reminders.length);
                            let found = false;
                            for (let i = 0; i < reminders.length; i++) {
                                if (reminders[i].reminderId === reminderId) {
                                    if (reminders[i].reminderReq.repeatInterval !== undefined && reminders[i].reminderReq.repeatCount !== undefined) {
                                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 found reminder, repeatInterval = ' + reminders[i].reminderReq.repeatInterval);
                                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 found reminder, repeatCount = ' + reminders[i].reminderReq.repeatCount);
                                        expect(reminders[i].reminderReq.repeatInterval).assertEqual(86400);
                                        expect(reminders[i].reminderReq.repeatCount).assertEqual(5);
                                    }
                                    found = true;
                                    break;
                                }
                            }
                            expect(found).assertTrue();
                            done();
                        }).catch((err) => {
                            console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 getAllValidReminders err.code is :' + err.code);
                            expect(false).assertTrue();
                            done();
                        });
                    }).catch((err) => {
                        console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 updateReminder err.code is :' + err.code);
                        expect(false).assertTrue();
                        done();
                    });
                }).catch((err) => {
                    console.info('SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 publishReminder err.code is :' + err.code);
                    expect(false).assertTrue();
                    done();
                });
            } catch (error) {
                console.info("SUB_RESOURCESCHEDULE_REMINDERAGENT_TASK_XTS_0900 error.code:" + error.code);
                expect(false).assertTrue();
                done();
            }
        })
        
    })
}
