...
struct Fiber : detail::NonCopyable {
    ...
    static Fiber* Init(std::function<void()>&& f, void* stack, size_t stackSize)
    {
        if (stack == nullptr || stackSize < sizeof(Fiber) + minStackSize) {
            return nullptr;
        }
        auto c = new (stack) Fiber(std::forward<std::function<void()>>(f));
        if (ffrt_fiber_init(&c->fb, reinterpret_cast<void(*)(void*)>(FiberEntry), c,
            static_cast<char*>(stack) + sizeof(Fiber), stackSize - sizeof(Fiber))) {
    ...
    explicit Fiber(std::function<void()>&& f)
    {
