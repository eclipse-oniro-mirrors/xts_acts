...
const int ERR_CODE_10 = 10;
const int ERR_CODE_11 = 11;
...
const uint64_t SLEEP_TIME = 2000;
const uint64_t UNIT_TASK_DELAY_500 = 500000;
const uint64_t COST_TIME_200 = 200;
const uint64_t COST_TIME_300 = 300;
const uint64_t COST_TIME_500 = 500;
const uint64_t COST_TIME_600 = 600;
const int TASK_STORAGE_SIZE = 128;
const int STORAGE_SIZE = 64;
const int TEST_VALUE_1 = 100;
const int TEST_VALUE_2 = 5;
#if defined(__aarch64__)
const int FIBER_STORAGE_SIZE = 22;
#elif defined(__arm__)
const int FIBER_STORAGE_SIZE = 64;
#elif defined(__x86_64__)
const int FIBER_STORAGE_SIZE = 8;
#endif
const int FUNCTION_STORAGE_SIZE = 96;
const int ONE_SECOND_TO_MICRO = 1000 * 1000;
...
{
    struct timespec tm;
    tm.tv_sec = ns / NS_PER_SEC;
    tm.tv_nsec = ns - (tm.tv_sec * NS_PER_SEC);
    return tm;
}

static struct timespec timeoutms_to_tm(int timeout_ms)
{
    struct timespec start_tm;
    struct timespec end_tm;
    clock_gettime(CLOCK_MONOTONIC, &start_tm);
    end_tm = ns_to_tm(tm_to_ns(start_tm) + timeout_ms * NS_PER_MS);
    return end_tm;
}
...
static napi_value ConditionVariableTest005(napi_env env, napi_callback_info info)
{
    int a = 0;
    int resultEnd = 0;
    ffrt_cond_t cond;
    ...
    std::function<void()>&& FfrtFunc1 = [&]() {
        int timeout = 2000;
        struct timespec tm = timeoutms_to_tm(timeout);
        ffrt_mutex_lock(&lock_);
        int ret = ffrt_cond_timedwait(nullptr, &lock_, &tm);
        if (ret != ffrt_error_inval) {
            resultEnd = ERR_CODE_1;
        }
        ret = ffrt_cond_timedwait(&cond, nullptr, &tm);
        if (ret != ffrt_error_inval) {
            resultEnd = ERR_CODE_2;
        }
        ret = ffrt_cond_timedwait(&cond, &lock_, nullptr);
        if (ret != ffrt_error_inval) {
            resultEnd = ERR_CODE_3;
        }
        ret = ffrt_cond_timedwait(&cond, &lock_, &tm);
        if (ret != ffrt_success) {
    ...
    std::function<void()>&& FfrtFunc2 = [&]() {
        ffrt_usleep(ONE_SECOND_TO_MICRO);
        ffrt_mutex_lock(&lock_);
        a = TEST_VALUE_2;
        ffrt_cond_signal(&cond);
        ffrt_mutex_unlock(&lock_);
    };
    ffrt_function_header_t* ffrt_func2 = create_function_wrapper((FfrtFunc2));
    ffrt_submit_base(ffrt_func2, nullptr, nullptr, nullptr);
...
static napi_value SharedMutexAbnormalParamTest001(napi_env env, napi_callback_info info)
{
    int result = 0;
    int type = 0;
    ffrt_rwlockattr_t attr = {0};
    ffrt_rwlock_t mtx = {0};
    int ret = ffrt_rwlock_init(&mtx, nullptr);
    if (ret != ffrt_success) {
        result = ERR_CODE_1;
    }
    ret = ffrt_mutexattr_settype(NULL, ffrt_mutex_normal);
    ret = ffrt_mutexattr_settype(NULL, ffrt_mutex_default);
    if (ret != ffrt_error_inval) {
        result = ERR_CODE_2;
    }
    napi_value flag = nullptr;
...
static napi_value QueueApiTest005(napi_env env, napi_callback_info info)
{
    ...
    int a = 0;
    ffrt_task_handle_t handle = ffrt_queue_submit_h(queue_handle, ffrt_create_function_wrapper(OnePlusForTest,
        nullptr, &a, ffrt_function_kind_queue), nullptr);
...
static napi_value QueueApiTest016(napi_env env, napi_callback_info info)
{
    int result = 0;
    ffrt_queue_attr_t queue_attr;
    (void)ffrt_queue_attr_init(&queue_attr);

    ffrt_queue_attr_set_thread_mode(&queue_attr, true);
    ffrt_queue_t queue_handle = ffrt_queue_create(ffrt_queue_serial, "test_queue", &queue_attr);
    if (queue_handle == nullptr) {
        result += 1;
    }
    int a = 0;
    std::function<void()>&& basicFunc = [&a]() {
        a += 1;
    };
    ffrt_queue_submit(queue_handle, create_function_wrapper(basicFunc, ffrt_function_kind_queue), nullptr);
    if (!ffrt_queue_attr_get_thread_mode(&queue_attr)) {
        result += 1;
    }
...
static napi_value QueueApiTest017(napi_env env, napi_callback_info info)
{
    int result = 0;
    ffrt_queue_attr_t queue_attr;
    (void)ffrt_queue_attr_init(&queue_attr);

    ffrt_queue_attr_set_thread_mode(&queue_attr, false);
    ffrt_queue_t queue_handle = ffrt_queue_create(ffrt_queue_serial, "test_queue", &queue_attr);
    if (queue_handle == nullptr) {
        result += 1;
    }
    ...
    ffrt_task_handle_t handle = ffrt_queue_submit_h_f(nullptr, OnePlusForTest, &a, nullptr);
    if (ffrt_queue_attr_get_thread_mode(&queue_attr)) {
        result += 1;
    }
...
static napi_value QueueDelayTest001(napi_env env, napi_callback_info info)
{
    ...
    ffrt_task_attr_set_delay(&task_attr1, UNIT_TASK_DELAY_500);
    ffrt_task_attr_set_delay(&task_attr2, UNIT_TASK_DELAY);
    ...
    if (t <= COST_TIME_200 || t >= COST_TIME_300) {
        resultEnd = 2;
    }
    ...
    if (t <= COST_TIME_500 || t >= COST_TIME_600) {
        resultEnd = 4;
    }
...
static napi_value QueueDelayTest005(napi_env env, napi_callback_info info)
{
    ...
    ffrt_task_attr_t task_attr;
    (void)ffrt_task_attr_init(&task_attr);
    ffrt_task_attr_set_delay(&task_attr, UNIT_TASK_DELAY_500);
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, 1, "FFRT QUEUE", "test2 cost %{public}f ms", t);
    ffrt_queue_wait(task1);
    if (a != 10) {
        result = 3;
    }
    uint64_t delay = ffrt_task_attr_get_delay(&task_attr);
    if (delay != UNIT_TASK_DELAY_500) {
        result = 4;
    }
    ffrt_task_attr_destroy(&task_attr);
    ffrt_task_handle_destroy(task1);
...
static napi_value QueueTest001(napi_env env, napi_callback_info info)
{
    ...
    ffrt_queue_submit(queue_handle, ffrt_create_function_wrapper(MultipleForTest, nullptr, &result,
        ffrt_function_kind_queue), nullptr);
    ffrt_task_handle_t handle = ffrt_queue_submit_h(queue_handle,
        ffrt_create_function_wrapper(SubForTest, nullptr, &result, ffrt_function_kind_queue), nullptr);
    ffrt_queue_wait(handle);
    ffrt_task_handle_destroy(handle);
    napi_value flag = nullptr;
    napi_create_double(env, result, &flag);
...
static napi_value SubmitAbnormalTest(napi_env env, napi_callback_info info)
{
    ...
    ffrt_task_handle_t handle = ffrt_submit_h_base(nullptr, &in, &out, nullptr);
    if (handle != nullptr) {
        result = 1;
    }
...
static napi_value ffrt_loop_0001(napi_env env, napi_callback_info info)
{
    ...
    std::function<void()> &&SleepFunc = [] () {usleep(SLEEP_TIME);};
...
static bool SuspendFunc([[maybe_unused]] void* p)
{
    return true;
}

static napi_value ffrt_fiber_001(napi_env env, napi_callback_info info)
{
    constexpr uint64_t stackSize = 16 * 1024;
    std::array<char, stackSize> stack;
    using fiber_t = ffrt::Fiber<>;
    std::atomic<int> a = {0};
    auto t = fiber_t::Init([&a] {
        for (int i = 0; i < 10; i++) {
            a++;
            fiber_t::Suspend(SuspendFunc);
        }
    }, &stack, stackSize);
    t->Start();

    napi_value result;
    napi_create_double(env, a.load(), &result);
    return result;
}

static napi_value ffrt_alloc_function_0001(napi_env env, napi_callback_info info)
{
    int result = 0;
    ffrt_function_kind_t kind = ffrt_function_kind_general;
    auto p = ffrt_alloc_auto_managed_function_storage_base(kind);
...
static napi_value ffrtStorageSizeCheck(napi_env env, napi_callback_info info)
{
    int result = 0;
    ffrt_storage_size_t ret = ffrt_task_attr_storage_size;
    if (ret != TASK_STORAGE_SIZE) {
        result = ERR_CODE_1;
    }
    ret = ffrt_auto_managed_function_storage_size;
    if (ret != STORAGE_SIZE + sizeof(ffrt_function_header_t)) {
        result = ERR_CODE_2;
    }
    ret = ffrt_mutex_storage_size;
    if (ret != STORAGE_SIZE) {
        result = ERR_CODE_3;
    }
    ret = ffrt_cond_storage_size;
    if (ret != STORAGE_SIZE) {
        result = ERR_CODE_4;
    }
    ret = ffrt_queue_attr_storage_size;
    if (ret != TASK_STORAGE_SIZE) {
        result = ERR_CODE_5;
    }
    ret = ffrt_rwlock_storage_size;
    if (ret != STORAGE_SIZE) {
        result = ERR_CODE_6;
    }
    ret = ffrt_fiber_storage_size;
    if (ret != FIBER_STORAGE_SIZE) {
        result = ERR_CODE_7;
    }

    ffrt_error_t ero = ffrt_error_timedout;
    if (ero != ETIMEDOUT) {
        result = ERR_CODE_8;
    }
    ero = ffrt_error;
...
static napi_value ffrtQueueQosCheck(napi_env env, napi_callback_info info)
{
    int result = 0;
    ffrt_task_attr_t* attr = (ffrt_task_attr_t *) malloc(sizeof(ffrt_task_attr_t));
    ffrt_queue_attr_t queue_attr;
    (void)ffrt_queue_attr_init(&queue_attr);
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_inherit));
    ffrt_qos_t queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_inherit)) {
        result = ERR_CODE_1;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_background));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_background)) {
        result = ERR_CODE_2;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_utility));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_utility)) {
        result = ERR_CODE_3;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_user_initiated));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_user_initiated)) {
        result = ERR_CODE_4;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_default));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_default)) {
        result = ERR_CODE_5;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_deadline_request));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_deadline_request)) {
        result = ERR_CODE_6;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_user_interactive));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_user_interactive)) {
        result = ERR_CODE_7;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<ffrt_qos_t>(ffrt::qos_max));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt::qos_max)) {
        result = ERR_CODE_8;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<int>(ffrt_qos_deadline_request));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt_qos_deadline_request)) {
        result = ERR_CODE_9;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<int>(ffrt_qos_user_interactive));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt_qos_user_interactive)) {
        result = ERR_CODE_10;
    }
    ffrt_queue_attr_set_qos(&queue_attr, static_cast<int>(ffrt_qos_max));
    queueqos = ffrt_queue_attr_get_qos(&queue_attr);
    if (queueqos != static_cast<int>(ffrt_qos_max)) {
        result = ERR_CODE_11;
    }
    ffrt_mutex_t mutex = {0};
    ffrt_fiber_t fiber = {0};
    ffrt_cond_t cond = {0};
    free(attr);
...
napi_property_descriptor desc[] = {
    ...
    {"conditionVariableTest004", nullptr, ConditionVariableTest004, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"conditionVariableTest005", nullptr, ConditionVariableTest005, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    ...
    {"sharedMutexAbnormalParamTest001", nullptr, SharedMutexAbnormalParamTest001, nullptr,
        nullptr, nullptr, napi_default, nullptr },
    {"recursiveMutexAbnormalInvalidParamTest001", nullptr, RecursiveMutexAbnormalInvalidParamTest001, nullptr,
        nullptr, nullptr, napi_default, nullptr },
    {"mutexAbnormalParamTest001", nullptr, MutexAbnormalParamTest001, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"mutexAbnormalParamTest002", nullptr, MutexAbnormalParamTest002, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    ...
    {"queueApiTest016", nullptr, QueueApiTest016, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"queueApiTest017", nullptr, QueueApiTest017, nullptr, nullptr, nullptr, napi_default, nullptr },
    ...
    {"submitInDependEmptyTest", nullptr, SubmitInDependEmptyTest, nullptr, nullptr, nullptr, napi_default, nullptr },
    ...
    {"submitOuDependNullptrTest", nullptr, SubmitOuDependNullptrTest, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"submitIODependTest001", nullptr, SubmitIODependTest001, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"submitIODependTest002", nullptr, SubmitIODependTest002, nullptr, nullptr, nullptr, napi_default, nullptr },
    ...
    {"submitHInDependEmptyTest", nullptr, SubmitHInDependEmptyTest, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"submitHInDependNullptrTest", nullptr, SubmitHInDependNullptrTest, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"submitHOuDependEmptyTest", nullptr, SubmitHOuDependEmptyTest, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"submitHOuDependNullptrTest", nullptr, SubmitHOuDependNullptrTest, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"submitHIODependTest001", nullptr, SubmitHIODependTest001, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"submitHIODependTest002", nullptr, SubmitHIODependTest002, nullptr, nullptr, nullptr, napi_default, nullptr },
    ...
    {"ffrt_loop_abnormal_0003", nullptr, ffrt_loop_abnormal_0003, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"ffrt_loop_normal_0003", nullptr, ffrt_loop_normal_0003, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"ffrt_get_main_queue_0001", nullptr, ffrt_get_main_queue_0001, nullptr, nullptr, nullptr, napi_default, nullptr },
    ...
    {"ffrt_this_task_get_qos_0001", nullptr, ffrt_this_task_get_qos_0001, nullptr, nullptr,
        nullptr, napi_default, nullptr },
    {"ffrt_attr_stack_size_0001", nullptr, ffrt_attr_stack_size_0001, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    {"ffrt_task_handle_ref_0001", nullptr, ffrt_task_handle_ref_0001, nullptr, nullptr, nullptr,
        napi_default, nullptr },
    ...
    {"ffrt_fiber_001", nullptr, ffrt_fiber_001, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"ffrt_alloc_function_0001", nullptr, ffrt_alloc_function_0001, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"ffrtStorageSizeCheck", nullptr, ffrtStorageSizeCheck, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"ffrtQueueQosCheck", nullptr, ffrtQueueQosCheck, nullptr, nullptr, nullptr, napi_default, nullptr },
};
