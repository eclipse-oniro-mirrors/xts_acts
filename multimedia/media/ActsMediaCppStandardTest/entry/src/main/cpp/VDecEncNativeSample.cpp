...
/** @file Video decode/encode sample implementation (native media API). NAPI_* macros denote native API check-and-return
 * helpers. */
...
constexpr uint32_t ES[] = { 2106, 11465, 321,  72,   472,  68,  76,   79,   509,  90,  677,  88,   956,  99,  347,
...
void VdecAsyncError(OH_AVCodec* codec, int32_t errorCode, void* userData)
{
    ...
    VDecEncSignal* vcodecSignal = static_cast<VDecEncSignal*>(userData);
    vcodecSignal->errorNum_ += 1;
}
...
{
    VDecEncSignal* vcodecSignal = static_cast<VDecEncSignal*>(userData);
    unique_lock<mutex> lock(vcodecSignal->inMutexDec_);
    if (vcodecSignal->isVdecFlushing_.load()) {
        cout << "VdecAsyncNeedInputData isVdecFlushing_ is true, return" << endl;
        return;
    }
    vcodecSignal->inQueueDec_.push(index);
    vcodecSignal->inBufferQueueDec_.push(data);
    vcodecSignal->inCondDec_.notify_all();
}

void VdecAsyncNewOutputData(OH_AVCodec* codec,
                            uint32_t index,
                            OH_AVMemory* data,
...
{
    VDecEncSignal* vcodecSignal = static_cast<VDecEncSignal*>(userData);
    unique_lock<mutex> lock(vcodecSignal->outMutexDec_);
    if (vcodecSignal->isVdecFlushing_.load()) {
        cout << "VdecAsyncNeedInputData isVdecFlushing_ is true, return" << endl;
        return;
    }
    vcodecSignal->outQueueDec_.push(index);
    vcodecSignal->flagQueueDec_.push(attr->flags);
    vcodecSignal->outCondDec_.notify_all();
}
...
void VencAsyncNewOutputData(OH_AVCodec* codec,
                            uint32_t index,
                            OH_AVMemory* data,
                            OH_AVCodecBufferAttr* attr,
                            void* userData)
{
    VDecEncSignal* vcodecSignal = static_cast<VDecEncSignal*>(userData);
    unique_lock<mutex> lock(vcodecSignal->outMutexEnc_);
    if (vcodecSignal->isVencFlushing_.load()) {
        cout << "VdecAsyncNeedInputData isVencFlushing_ is true, return" << endl;
        return;
    }
    vcodecSignal->outQueueEnc_.push(index);
    vcodecSignal->sizeQueueEnc_.push(attr->size);
    vcodecSignal->flagQueueEnc_.push(attr->flags);
    vcodecSignal->outBufferQueueEnc_.push(data);
    vcodecSignal->outCondEnc_.notify_all();
}
...
VDecEncNativeSample::~VDecEncNativeSample()
{
...
void VDecEncNativeSample::SetReadPath(std::string filepath)
{
    inFile_ = filepath;
...
void VDecEncNativeSample::SetSavePath(std::string filepath)
{
    outFile_ = filepath;
}

void VDecEncNativeSample::SetEosState(bool needSetEos)
{
    setEos = needSetEos;
}

void VDecEncNativeSample::ReRead()
{
    if (testFile_ != nullptr) {
        testFile_->close();
...
void VDecEncNativeSample::ResetDecParam()
{
    decInCnt_ = 0;
    decOutCnt_ = 0;
    isDecInputEOS = false;
...
void VDecEncNativeSample::ResetEncParam()
{
    encOutCnt_ = 0;
    isEncInputEOS = false;
...
struct OH_AVCodec* VDecEncNativeSample::CreateVideoDecoderByMime(std::string mimetype)
{
    if (mimetype == "video/avc") {
        ...
        mimeBuf.push_back('\0');
        vdec_ = OH_VideoDecoder_CreateByMime(mimeBuf.data());
    }
    NAPI_CHECK_AND_RETURN_RET_LOG(vdec_ != nullptr, nullptr, "Fatal: OH_VideoDecoder_CreateByMime");
    if (vcodecSignal_ == nullptr) {
        vcodecSignal_ = new VDecEncSignal();
        NAPI_CHECK_AND_RETURN_RET_LOG(vcodecSignal_ != nullptr, nullptr, "Fatal: No Memory");
    }
    ...
    NAPI_CHECK_AND_RETURN_RET_LOG(ret == AV_ERR_OK, nullptr, "Fatal: OH_VideoDecoder_SetCallback");
    return vdec_;
}

struct OH_AVCodec* VDecEncNativeSample::CreateVideoDecoderByName(std::string name)
{
    std::vector<char> nameBuf(name.begin(), name.end());
    nameBuf.push_back('\0');
    vdec_ = OH_VideoDecoder_CreateByName(nameBuf.data());
    NAPI_CHECK_AND_RETURN_RET_LOG(vdec_ != nullptr, nullptr, "Fatal: OH_VideoDecoder_CreateByName");
    if (vcodecSignal_ == nullptr) {
        vcodecSignal_ = new VDecEncSignal();
        NAPI_CHECK_AND_RETURN_RET_LOG(vcodecSignal_ != nullptr, nullptr, "Fatal: No Memory");
    }
...
int32_t VDecEncNativeSample::ConfigureDec(struct OH_AVFormat* format)
{
    return OH_VideoDecoder_Configure(vdec_, format);
}

int32_t VDecEncNativeSample::SetParameterDec(struct OH_AVFormat* format)
{
    return OH_VideoDecoder_SetParameter(vdec_, format);
}

int32_t VDecEncNativeSample::PrepareDec()
{
    return OH_VideoDecoder_Prepare(vdec_);
}

int32_t VDecEncNativeSample::StartDec()
{
    cout << "Enter dec start" << endl;
    isDecRunning_.store(true);

    if (inputLoopDec_ == nullptr) {
        inputLoopDec_ = make_unique<thread>(&VDecEncNativeSample::InputFuncDec, this);
        NAPI_CHECK_AND_RETURN_RET_LOG(inputLoopDec_ != nullptr, AV_ERR_UNKNOWN, "Fatal: No memory");
    }
    if (outputLoopDec_ == nullptr) {
        outputLoopDec_ = make_unique<thread>(&VDecEncNativeSample::OutputFuncDec, this);
        NAPI_CHECK_AND_RETURN_RET_LOG(outputLoopDec_ != nullptr, AV_ERR_UNKNOWN, "Fatal: No memory");
    }
    cout << "Exit dec start" << endl;
    return OH_VideoDecoder_Start(vdec_);
}

int32_t VDecEncNativeSample::StopDec()
{
...
int32_t VDecEncNativeSample::FlushDec()
{
...
int32_t VDecEncNativeSample::ResetDec()
{
...
int32_t VDecEncNativeSample::ReleaseDec()
{
...
void VDecEncNativeSample::PopInqueueDec()
{
    if (vcodecSignal_ == nullptr) {
        return;
    }
...
int32_t VDecEncNativeSample::PushInbufferDec(uint32_t index, uint32_t bufferSize)
{
    if (vdec_ == nullptr) {
...
bool VDecEncNativeSample::ProcessOneInputDecFrame(uint32_t index, OH_AVMemory* buffer)
{
    ...
    if (testFile_ == nullptr || !testFile_->is_open()) {
        (void)printf("Fatal: open file fail\n");
        return false;
    }
...
void VDecEncNativeSample::InputFuncDec()
{
    while (isDecRunning_.load()) {
        cout << "ENTER DEC IN" << endl;
        unique_lock<mutex> lock(vcodecSignal_->inMutexDec_);
        bool woke = vcodecSignal_->inCondDec_.wait_for(lock, std::chrono::seconds(1), [this]() {
            return vcodecSignal_->inQueueDec_.size() > 0 || !isDecRunning_.load();
        });
        if (!woke) {
        ...
        if (!ProcessOneInputDecFrame(index, buffer)) {
            break; /* EOF */
        }
    }
}

void VDecEncNativeSample::PopOutqueueDec()
{
    if (vcodecSignal_ == nullptr) {
        return;
    }
...
void VDecEncNativeSample::SendEncEos()
{
...
void VDecEncNativeSample::OutputFuncDec()
{
    while (isDecRunning_.load()) {
        unique_lock<mutex> lock(vcodecSignal_->outMutexDec_);
        bool woke = vcodecSignal_->outCondDec_.wait_for(lock, std::chrono::seconds(1), [this]() {
            return vcodecSignal_->outQueueDec_.size() > 0 || !isDecRunning_.load();
        });
        if (!woke) {
            break; /* G.CTL.03: timeout, exit to avoid permanent block */
        }
...
struct OH_AVCodec* VDecEncNativeSample::CreateVideoEncoderByMime(std::string mimetype)
{
    if (mimetype == "video/avc") {
        ...
        std::vector<char> mimeBuf(mimetype.begin(), mimetype.end());
        mimeBuf.push_back('\0');
        venc_ = OH_VideoEncoder_CreateByMime(mimeBuf.data());
    }
    NAPI_CHECK_AND_RETURN_RET_LOG(venc_ != nullptr, nullptr, "Fatal: OH_VideoEncoder_CreateByMime");
    ...
    NAPI_CHECK_AND_RETURN_RET_LOG(ret == AV_ERR_OK, nullptr, "Fatal: OH_VideoEncoder_SetCallback");
    return venc_;
}

struct OH_AVCodec* VDecEncNativeSample::CreateVideoEncoderByName(std::string name)
{
    std::vector<char> nameBuf(name.begin(), name.end());
    nameBuf.push_back('\0');
    venc_ = OH_VideoEncoder_CreateByName(nameBuf.data());
    NAPI_CHECK_AND_RETURN_RET_LOG(venc_ != nullptr, nullptr, "Fatal: OH_VideoEncoder_CreateByName");
...
int32_t VDecEncNativeSample::ConfigureEnc(struct OH_AVFormat* format)
{
    return OH_VideoEncoder_Configure(venc_, format);
}

int32_t VDecEncNativeSample::SetParameterEnc(struct OH_AVFormat* format)
{
    return OH_VideoEncoder_SetParameter(venc_, format);
}

int32_t VDecEncNativeSample::GetSurface()
{
    return OH_VideoEncoder_GetSurface(venc_, &nativeWindow_);
}

int32_t VDecEncNativeSample::SetOutputSurface()
{
    return OH_VideoDecoder_SetSurface(vdec_, nativeWindow_);
}

int32_t VDecEncNativeSample::PrepareEnc()
{
    return OH_VideoEncoder_Prepare(venc_);
}

int32_t VDecEncNativeSample::StartEnc()
{
    cout << "Enter enc start" << endl;
    isEncRunning_.store(true);
    if (outputLoopEnc_ == nullptr) {
        outputLoopEnc_ = make_unique<thread>(&VDecEncNativeSample::OutputFuncEnc, this);
...
int32_t VDecEncNativeSample::StopEnc()
{
...
int32_t VDecEncNativeSample::FlushEnc()
{
...
int32_t VDecEncNativeSample::ResetEnc()
{
...
int32_t VDecEncNativeSample::ReleaseEnc()
{
...
void VDecEncNativeSample::PopOutqueueEnc()
{
    if (vcodecSignal_ == nullptr) {
        return;
    }
...
int32_t VDecEncNativeSample::WriteToFile()
{
    ...
    std::vector<char> pathBuf(outFile_.begin(), outFile_.end());
    pathBuf.push_back('\0');
    FILE* outFile = fopen(pathBuf.data(), "a");
    if (outFile == nullptr) {
...
void VDecEncNativeSample::OutputFuncEnc()
{
    while (isEncRunning_.load()) {
        unique_lock<mutex> lock(vcodecSignal_->outMutexEnc_);
        bool woke = vcodecSignal_->outCondEnc_.wait_for(lock, std::chrono::seconds(1), [this]() {
            return vcodecSignal_->outQueueEnc_.size() > 0 || !isEncRunning_.load();
        });
        if (!woke) {
            break; /* G.CTL.03: timeout, exit to avoid permanent block */
        }
...
int32_t VDecEncNativeSample::CalcuError()
{
...
int32_t VDecEncNativeSample::GetFrameCount()
{
    return encOutCnt_;
}
bool VDecEncNativeSample::GetEncEosState()
{
    return isEncOutputEOS;
}
bool VDecEncNativeSample::GetDecEosState()
{
