...
void seekAvPlayerStateTest(OH_AVPlayer *player, AVPlayerSeekMode seekMode, OH_AVErrCode errorCode)
{
    int32_t ret;
    LOG("AVPlayerState  OH_AVPlayer_Seek");
    std::this_thread::sleep_for(std::chrono::milliseconds(g_gPlaytime));
    ret = OH_AVPlayer_Seek(player, PARAM_DURATION_F2, seekMode);
    LOG("call OH_AVPlayer_Seek value:%{public}d  mode:AV_SEEK_CLOSEST  ret:%{public}d", PARAM_DURATION_F2, ret);
    if (ret != errorCode) {
        napi_throw_error((screenCaptureEnv), nullptr, "error : expect OH_AVPlayer_Seek is 3");
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(g_gPlaytime));
    ret = OH_AVPlayer_Seek(player, PARAM_DURATION_100, seekMode);
    LOG("call OH_AVPlayer_Seek value:%{public}d  mode:AV_SEEK_CLOSEST  ret:%{public}d", PARAM_DURATION_100, ret);
    if (ret != errorCode) {
        napi_throw_error((screenCaptureEnv), nullptr, "error : expect OH_AVPlayer_Seek is 3");
    }

    std::this_thread::sleep_for(std::chrono::milliseconds(g_gPlaytime));
    ret = OH_AVPlayer_Seek(player, g_duration, seekMode);
    LOG("call OH_AVPlayer_Seek value:%{public}d  mode:AV_SEEK_CLOSEST  ret:%{public}d", g_duration, ret);
    if (ret != errorCode) {
        napi_throw_error((screenCaptureEnv), nullptr, "error : expect OH_AVPlayer_Seek is 3");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(g_gPlaytime));
}
void handleSeekStateChange(OH_AVPlayer *player, AVPlayerState state)
{
    int32_t ret;
    LOG("handleSeekStateChange AVPlayerState state:%{public}d", state);
    switch (state) {
        case AV_INITIALIZED: {
            seekAvPlayerStateTest(player, AV_SEEK_CLOSEST, AV_ERR_INVALID_VAL);
            EGLDisplay display = nullptr;
            EGLContext context = nullptr;
            EGLSurface surface = nullptr;
            InitGLES(display, context, surface);
            ...
            OH_NativeImage *image = OH_NativeImage_Create(textureId, GL_TEXTURE_2D);
            OHNativeWindow *window = OH_NativeImage_AcquireNativeWindow(image);
            ret = OH_AVPlayer_SetVideoSurface(player, window);
            LOG("OH_AVPlayer_SetVideoSurface ret:%{public}d", ret);
            ret = OH_AVPlayer_Prepare(player);
            LOG("OH_AVPlayer_Prepare ret:%{public}d", ret);
        } break;
        case AV_PREPARED:
            seekAvPlayerStateTest(player, AV_SEEK_CLOSEST, AV_ERR_OK);
...
void OHAVPlayerOnInfoCallback(OH_AVPlayer *player, AVPlayerOnInfoType type, OH_AVFormat *infoBody, void *userData)
{
    ...
    case AV_INFO_TYPE_STATE_CHANGE:
        ...
        handleSeekStateChange(player, avState);
        break;
...
static napi_value NAPI_Global_Setup(napi_env env, napi_callback_info info)
{
    ...
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int pType;
    napi_get_value_int32(env, args[PARAM_1], &pType);
    LOG("fd type %{public}d", pType);
    int pOffset;
    napi_get_value_int32(env, args[PARAM_2], &pOffset);
    LOG("fd size %{public}d", pOffset);
    int pDurationSize;
    napi_get_value_int32(env, args[PARAM_3], &pDurationSize);
    LOG("fd size %{public}d", pDurationSize);
    g_duration = pDurationSize - PARAM_4;
    OH_AVPlayer *player = OH_AVPlayer_Create();
    g_player = player;
    seekAvPlayerStateTest(player, AV_SEEK_CLOSEST, AV_ERR_INVALID_VAL);
    // 设置回调，监听信息
    ...
    {
        ret = OH_AVPlayer_SetFDSource(player, pType, pOffset, pDurationSize);
    ...
    if (ret != AV_ERR_OK) {
        napi_throw_error((screenCaptureEnv), nullptr, "error : expect OH_AVPlayer_SetAudioRendererInfo is 0");
    }
    // 设置音频流打断模式
    ...
    if (ret != AV_ERR_OK) {
        napi_throw_error((screenCaptureEnv), nullptr, "error : expect OH_AVPlayer_SetAudioInterruptMode is 0");
    }
    napi_value value;
...
static napi_value NAPI_Global_Release(napi_env env, napi_callback_info info)
{
    int ret = -1;
    if (g_player != NULL) {
        ret = OH_AVPlayer_Release(g_player);
...
static napi_value NAPI_Global_setLoudnessGain(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    waitAvPlayerStateChange(player, AV_PREPARED);
    double loudnessGain;
    napi_get_value_double(env, args[0], &loudnessGain);
    OH_AVErrCode errCode = OH_AVPlayer_SetLoudnessGain(player, loudnessGain);
    if (errCode == AV_ERR_OK) {
        backParam = SUCCESS;
    }
    OH_AVPlayer_ReleaseSync(player);
...
static void GetIntValue(struct OH_AVFormat *format, napi_env env, napi_value &JsObject, const char* &key)
{
    const char* out = nullptr;
    bool ret = false;
    int32_t value = 0;
    ret = OH_AVFormat_GetIntValue(format, key, &value);
    LOG("GetIntValue key:%{public}s value:%{public}d", key, ret);
    ...
    if (ret) {
        ...
        napi_set_named_property(env, JsObject, key, valueNapi);
    }
}

static void GetUintValue(struct OH_AVFormat *format, napi_env env, napi_value &JsObject, const char* &key)
{
    const char* out = nullptr;
    bool ret = false;
    uint32_t value = 0;
    ret = OH_AVFormat_GetUintValue(format, key, &value);
    LOG("GetUintValue key:%{public}s value:%{public}d", key, ret);
    std::string result = std::string(key) + " : %{public}d";
    LOG(result.c_str(), value);
    if (ret) {
        out = std::to_string(value).c_str();
        napi_value valueNapi = nullptr;
        napi_create_uint32(env, value, &valueNapi);
        napi_set_named_property(env, JsObject, key, valueNapi);
    }
}

static void GetLongValue(struct OH_AVFormat *format, napi_env env, napi_value &JsObject, const char* &key)
{
    const char* out = nullptr;
    bool ret = false;
    int64_t value = 0;
    ret = OH_AVFormat_GetLongValue(format, key, &value);
    LOG("GetLongValue key:%{public}s value:%{public}d", key, ret);
    std::string result = std::string(key) + " : %{public}lld";
    LOG(result.c_str(), value);
    if (ret) {
        out = std::to_string(value).c_str();
        napi_value valueNapi = nullptr;
...
static napi_value NAPI_Global_GetPlaybackStatisticMetrics(napi_env env, napi_callback_info info)
{
    size_t argc = 3;
    napi_value args[3] = {nullptr};
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    int pType;
    napi_get_value_int32(env, args[PARAM_0], &pType);
    LOG("fd type %{public}d", pType);
    int pOffset;
    napi_get_value_int32(env, args[PARAM_1], &pOffset);
    LOG("pOffset size %{public}d", pOffset);
    int fileSize;
    napi_get_value_int32(env, args[PARAM_2], &fileSize);
    ...
    ndkAVPlayerUser->player = OH_AVPlayer_Create();
    OH_AVErrCode errCode = OH_AVPlayer_SetOnInfoCallback(player, AVPlayerOnInfoCallbackImpl, ndkAVPlayerUser);
    OH_AVPlayer_SetFDSource(player, pType, pOffset, fileSize);
    AVPlayerState currentStage;
    OH_AVErrCode avErrorCode = OH_AVPlayer_GetState(player, &currentStage);
    while (AV_PREPARED != currentStage) {
        OH_AVPlayer_GetState(player, &currentStage);
    }
    OH_AVFormat *metrics = nullptr;
    metrics = OH_AVPlayer_GetPlaybackStatisticMetrics(player);
    if (metrics == nullptr) {
        LOG("GetPlaybackStatisticMetrics failed");
        OH_AVPlayer_ReleaseSync(player);
        delete ndkAVPlayerUser;
        return result;
    }
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_PREPARE_DURATION);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_RESOURCE_CONNECTION_DURATION);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_FIRST_FRAME_DECAPSULATION_DURATION);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_TOTAL_PLAYING_TIME);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_DOWNLOAD_REQUEST_COUNT);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_DOWNLOAD_TOTAL_TIME);
    GetLongValue(metrics, env, result, OH_MEDIA_EVENT_INFO_DOWNLOAD_TOTAL_SIZE);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_STALLING_COUNT);
    GetUintValue(metrics, env, result, OH_MEDIA_EVENT_INFO_TOTAL_STALLING_TIME);
    OH_AVFormat_Destroy(metrics);
    OH_AVPlayer_ReleaseSync(player);
    delete ndkAVPlayerUser;
    return result;
}

static napi_value NAPI_Global_GetPlaybackStatisticMetricsAbnormal(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    OH_AVFormat *metrics = nullptr;
    metrics = OH_AVPlayer_GetPlaybackStatisticMetrics(nullptr);
    napi_create_int32(env, (metrics == nullptr) ? SUCCESS : FAIL, &result);
    return result;
}

static napi_value NAPI_Global_GetMediaDescription(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    int32_t hasSubtitle = false;
    int32_t hasVideo = false;
    int32_t hasAudio = false;
    int backParam = FAIL;
    OH_AVPlayer *player = OH_AVPlayer_Create();
    AVPlayerCallback cb = { &AVPlayerOnInfo, &AVPlayerOnError};
    OH_AVPlayer_SetPlayerCallback(player, cb);
    ...
    OH_AVPlayer_Prepare(player);
    waitAvPlayerStateChange(player, AV_PREPARED);
    OH_AVFormat* format = OH_AVPlayer_GetMediaDescription(player);
    if (format != nullptr) {
        backParam = SUCCESS;
        OH_AVFormat_GetIntValue(format, OH_PLAYER_MD_KEY_HAS_SUBTITLE, &hasSubtitle);
        OH_AVFormat_GetIntValue(format, OH_PLAYER_MD_KEY_HAS_VIDEO, &hasVideo);
        OH_AVFormat_GetIntValue(format, OH_PLAYER_MD_KEY_HAS_AUDIO, &hasAudio);
    }
    OH_AVPlayer_ReleaseSync(player);
    napi_create_int32(env, backParam, &result);
    return result;
...
static napi_value NAPI_Global_GetMediaDescriptionOne(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    OH_AVFormat* format = OH_AVPlayer_GetMediaDescription(nullptr);
    if (format == nullptr) {
...
static napi_value NAPI_Global_GetTrackDescription(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    ...
    OH_AVFormat* format = OH_AVPlayer_GetTrackDescription(player, index);
    if (format != nullptr) {
        backParam = SUCCESS;
        OH_AVFormat_GetIntValue(format, OH_PLAYER_MD_KEY_TRACK_INDEX, &trackCount);
    }
    OH_AVPlayer_ReleaseSync(player);
    napi_create_int32(env, backParam, &result);
    return result;
...
static napi_value NAPI_Global_GetTrackDescriptionOne(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    int index;
    napi_get_value_int32(env, args[0], &index);
    OH_AVFormat* format = OH_AVPlayer_GetTrackDescription(nullptr, index);
    if (format == nullptr) {
        napi_create_int32(env, FAIL, &result);
    } else {
        napi_create_int32(env, PARAM_0, &result);
...
static napi_value NAPI_Global_setLoudnessGain1(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    double loudnessGain;
    napi_get_value_double(env, args[0], &loudnessGain);
    OH_AVErrCode errCode = OH_AVPlayer_SetLoudnessGain(nullptr, loudnessGain);
...
static napi_value NAPI_Global_setLoudnessGain2(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    GetFDSourceInfo(player, WAV_PATH);
    OH_AVPlayer_Prepare(player);
    OH_AVPlayer_SetAudioRendererInfo(player, AUDIOSTREAM_USAGE_UNKNOWN);
    waitAvPlayerStateChange(player, AV_PREPARED);
    double loudnessGain;
    napi_get_value_double(env, args[0], &loudnessGain);
    OH_AVErrCode errCode = OH_AVPlayer_SetLoudnessGain(player, loudnessGain);
...
static napi_value NAPI_Global_AvFormatSetUint(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = PARAM_2;
    napi_value args[PARAM_2] = {nullptr};
    ...
    uint32_t value;
    napi_get_value_uint32(env, args[PARAM_1], &value);
    const char* key;
    size_t keyLen;
    napi_get_value_string_utf8(env, args[PARAM_0], nullptr, 0, &keyLen);
    char* keyChar = new char[keyLen + 1];
    napi_get_value_string_utf8(env, args[PARAM_0], keyChar, keyLen + 1, nullptr);
    bool ret = OH_AVFormat_SetUintValue(format, keyChar, value);
    if (ret) {
        uint32_t getValue = 0;
        OH_AVFormat_GetUintValue(format, keyChar, &getValue);
        LOG("NAPI_Global_AvFormatSetUint key:%{public}s getValue:%{public}d", keyChar, getValue);
        if (getValue == value) {
            backParam = SUCCESS;
        }
    }
...
static napi_value NAPI_Global_AvFormatSetUint1(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = PARAM_2;
    napi_value args[PARAM_2] = {nullptr};
    ...
    bool ret = OH_AVFormat_SetUintValue(format, keyChar, value);
    if (!ret) {
        LOG("OH_AVFormat_SetUintValue failed");
        backParam = ONEVAL;
    }
    OH_AVFormat_Destroy(format);
    delete[] keyChar;
...
static napi_value NAPI_Global_AVPlayerOnInfoTypeEnum(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    int32_t seekdone = 0;
    int32_t speeddone = 1;
    int32_t bitratedone = 2;
    int32_t eos = 3;
    int32_t state = 4;
    int32_t position = 5;
    int32_t message = 6;
    ...
    int32_t buffering = 9;
    int32_t res = 1;
    if (AVPlayerOnInfoType::AV_INFO_TYPE_SEEKDONE == seekdone &&
        AVPlayerOnInfoType::AV_INFO_TYPE_SPEEDDONE == speeddone &&
        AVPlayerOnInfoType::AV_INFO_TYPE_BITRATEDONE == bitratedone &&
        AVPlayerOnInfoType::AV_INFO_TYPE_EOS == eos &&
        AVPlayerOnInfoType::AV_INFO_TYPE_STATE_CHANGE == state &&
        AVPlayerOnInfoType::AV_INFO_TYPE_POSITION_UPDATE == position &&
        AVPlayerOnInfoType::AV_INFO_TYPE_MESSAGE == message &&
        AVPlayerOnInfoType::AV_INFO_TYPE_VOLUME_CHANGE == volume &&
        AVPlayerOnInfoType::AV_INFO_TYPE_RESOLUTION_CHANGE == resolution &&
        AVPlayerOnInfoType::AV_INFO_TYPE_BUFFERING_UPDATE == buffering) {
        res = 0;
    }
...
static napi_value NAPI_Global_AVPlayerOnInfoTypeEnum1(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    int32_t bitrate = 10;
    int32_t interrupt = 11;
    int32_t duration = 12;
    int32_t isLive = 13;
    int32_t trackChange = 14;
    int32_t trackInfo = 15;
    int32_t subtitle = 16;
    int32_t audioOutput = 17;
    int32_t playbackRate = 18;
    int32_t resolution = 19;
    int32_t res = 1;
    if (AVPlayerOnInfoType::AV_INFO_TYPE_BITRATE_COLLECT == bitrate &&
        AVPlayerOnInfoType::AV_INFO_TYPE_INTERRUPT_EVENT == interrupt &&
        AVPlayerOnInfoType::AV_INFO_TYPE_DURATION_UPDATE == duration &&
        AVPlayerOnInfoType::AV_INFO_TYPE_IS_LIVE_STREAM == isLive &&
        AVPlayerOnInfoType::AV_INFO_TYPE_TRACKCHANGE == trackChange &&
        AVPlayerOnInfoType::AV_INFO_TYPE_TRACK_INFO_UPDATE == trackInfo &&
        AVPlayerOnInfoType::AV_INFO_TYPE_SUBTITLE_UPDATE == subtitle &&
        AVPlayerOnInfoType::AV_INFO_TYPE_AUDIO_OUTPUT_DEVICE_CHANGE == audioOutput &&
        AVPlayerOnInfoType::AV_INFO_TYPE_PLAYBACK_RATE_DONE == playbackRate &&
        AVPlayerOnInfoType::AV_INFO_TYPE_SUPER_RESOLUTION_CHANGED == resolution) {
        res = 0;
    }
    napi_create_int32(env, res, &result);
    return result;
...
static napi_value NAPI_Global_AVPlayerBufferingTypeEnum(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    int32_t start = 1;
    int32_t end = 2;
    ...
    int32_t duration = 4;
    int32_t res = 1;
    if (AVPlayerBufferingType::AVPLAYER_BUFFERING_START == start &&
        AVPlayerBufferingType::AVPLAYER_BUFFERING_END == end &&
        AVPlayerBufferingType::AVPLAYER_BUFFERING_PERCENT == percent &&
        AVPlayerBufferingType::AVPLAYER_BUFFERING_CACHED_DURATION ==duration) {
        res = 0;
    }
    napi_create_int32(env, res, &result);
    return result;
...
static napi_value NAPI_Global_setLoudnessGain3(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    GetFDSourceInfo(player, WAV_PATH);
    OH_AVPlayer_Prepare(player);
    OH_AVPlayer_SetAudioRendererInfo(player, AUDIOSTREAM_USAGE_MOVIE);
    waitAvPlayerStateChange(player, AV_PREPARED);
    double loudnessGain;
    napi_get_value_double(env, args[0], &loudnessGain);
    OH_AVErrCode errCode = OH_AVPlayer_SetLoudnessGain(player, loudnessGain);
...
static napi_value NAPI_Global_setLoudnessGain4(napi_env env, napi_callback_info info)
{
    napi_value result = nullptr;
    size_t argc = 1;
    napi_value args[1] = {nullptr};
    ...
    GetFDSourceInfo(player, WAV_PATH);
    OH_AVPlayer_Prepare(player);
    OH_AVPlayer_SetAudioRendererInfo(player, AUDIOSTREAM_USAGE_AUDIOBOOK);
    waitAvPlayerStateChange(player, AV_PREPARED);
    double loudnessGain;
    napi_get_value_double(env, args[0], &loudnessGain);
    OH_AVErrCode errCode = OH_AVPlayer_SetLoudnessGain(player, loudnessGain);
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        {"setLoudnessGain", nullptr, NAPI_Global_setLoudnessGain, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"setLoudnessGain1", nullptr, NAPI_Global_setLoudnessGain1, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"setLoudnessGain2", nullptr, NAPI_Global_setLoudnessGain2, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"setLoudnessGain3", nullptr, NAPI_Global_setLoudnessGain3, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"setLoudnessGain4", nullptr, NAPI_Global_setLoudnessGain4, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"getMediaDescription", nullptr, NAPI_Global_GetMediaDescription, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"getMediaDescriptionOne", nullptr, NAPI_Global_GetMediaDescriptionOne, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"getTrackDescription", nullptr, NAPI_Global_GetTrackDescription, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"getTrackDescriptionOne", nullptr, NAPI_Global_GetTrackDescriptionOne, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"getPlaybackStatisticMetrics", nullptr, NAPI_Global_GetPlaybackStatisticMetrics, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"getPlaybackStatisticMetricsAbnormal", nullptr, NAPI_Global_GetPlaybackStatisticMetricsAbnormal,
            nullptr, nullptr, nullptr, napi_default, nullptr},
        {"avFormatSetUint", nullptr, NAPI_Global_AvFormatSetUint, nullptr, nullptr, nullptr, napi_default, nullptr},
        {"avFormatSetUintWithNull", nullptr, NAPI_Global_AvFormatSetUint1, nullptr, nullptr,
            nullptr, napi_default, nullptr},
        {"avPlayerOnInfoTypeEnum", nullptr, NAPI_Global_AVPlayerOnInfoTypeEnum, nullptr, nullptr, nullptr,
            napi_default, nullptr},
        {"avPlayerOnInfoTypeEnum1", nullptr, NAPI_Global_AVPlayerOnInfoTypeEnum1, nullptr, nullptr, nullptr,
            napi_default, nullptr},
        {"avPlayerBufferingTypeEnum", nullptr, NAPI_Global_AVPlayerBufferingTypeEnum, nullptr, nullptr, nullptr,
            napi_default, nullptr},
    };
