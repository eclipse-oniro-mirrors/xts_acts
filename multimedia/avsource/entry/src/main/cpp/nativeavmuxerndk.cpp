...
static napi_value OHAVMuxerCreateFlv(napi_env env, napi_callback_info info)
{
    OH_AVMuxer *muxer = nullptr;
    OH_AVOutputFormat format = AV_OUTPUT_FORMAT_FLV;
    int fileDescribe = open("/data/storage/el2/base/haps/entry_test/files/demo.flv",
...
static napi_value OHAVMuxerSetFormat(napi_env env, napi_callback_info info)
{
    ...
    OH_AVFormat_SetFloatValue(fileFormat, OH_MD_KEY_LATITUDE, 60.0f);
    OH_AVFormat_SetFloatValue(fileFormat, OH_MD_KEY_LONGITUDE, -30.0f);
    OH_AVFormat_SetFloatValue(fileFormat, OH_MD_KEY_ALTITUDE, 10000.0f);
...
static napi_value OHAVMuxerWriteSampleBuffer(napi_env env, napi_callback_info info) {
    ...
    OH_AVFormat_SetIntValue(trackFormat, OH_MD_KEY_AUD_SAMPLE_RATE, BIGVAL);
    OH_AVFormat_SetIntValue(trackFormat, OH_MD_KEY_AUD_CHANNEL_COUNT, TWOVAL);
    OH_AVFormat_SetIntValue(trackFormat, OH_MD_KEY_ENABLE_MOOV_FRONT, 0);
    OH_AVMuxer_AddTrack(muxer, &trackId, trackFormat);
    OH_AVMuxer_Start(muxer);
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        {"oHAVMuxerCreateFlv", nullptr, OHAVMuxerCreateFlv, nullptr, nullptr, nullptr, napi_default, nullptr},
