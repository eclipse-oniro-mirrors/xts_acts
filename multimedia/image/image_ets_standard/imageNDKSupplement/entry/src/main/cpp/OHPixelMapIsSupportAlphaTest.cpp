...
#define LOG_TAG "PixelMapIsSupportAlphaNDK"
...
napi_value PixelMapToAlphaTest(napi_env env, napi_callback_info info)
{
...
napi_value OHPixelMapIsSupportAlpha(napi_env env, napi_callback_info info)
{
    LOGI("OH_PixelMap_IsSupportAlpha start");
    napi_value thisVar = nullptr;
    napi_value argValue[1] = {0};
    size_t argCount = 1;
    if (napi_get_cb_info(env, info, &argCount, argValue, &thisVar, nullptr) != napi_ok || argCount < 1 ||
        argValue[0] == nullptr) {
        LOGI("OH_PixelMap_IsSupportAlpha error1");
        return nullptr;
    }
    int32_t index;
    ...
    NativePixelMap *native = OH_PixelMap_InitNativePixelMap(env, PixelMapToAlphaTest(env, info));
    if (native == nullptr) {
        LOGI("OH_PixelMap_IsSupportAlpha error2");
        return result;
    }
    int32_t supportAlpha;
    int32_t imageResult;
    switch (index) {
    case CASE_INDEX_1:
        imageResult = OH_PixelMap_IsSupportAlpha(native, &supportAlpha);
        if (imageResult == 0) {
            LOGI("支持Alpha的指针 值：%{public}d", imageResult);
        }
        break;
    case CASE_INDEX_2:
        imageResult = OH_PixelMap_IsSupportAlpha(nullptr, &supportAlpha);
        if (imageResult == -1) {
            LOGI("不支持Alpha的指针 值：%{public}d", imageResult);
        }
        break;
    default:
        break;
