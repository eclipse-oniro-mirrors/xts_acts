...
napi_value CreatePixelMapCreat(napi_env env, napi_callback_info info)
{
...
napi_value OHPixelMapGetBytesNumberPerRow(napi_env env, napi_callback_info info)
{
    ...
    NativePixelMap *native = OH_PixelMap_InitNativePixelMap(env, CreatePixelMapCreat(env, info));
    if (native == nullptr) {
        LOGI("OH_PixelMap_GetBytesNumberPerRow error2");
        return result;
    }
    int32_t rowBytes;
    int32_t imageResult;
    switch (index) {
    case CASE_INDEX_1:
        imageResult = OH_PixelMap_GetBytesNumberPerRow(native, &rowBytes);
        break;
    case CASE_INDEX_2:
        imageResult = OH_PixelMap_GetBytesNumberPerRow(nullptr, &rowBytes);
        break;
    default:
        break;
    }
    LOGI("OH_PixelMap_GetBytesNumberPerRow 值：%{public}d  %{public}d ", rowBytes, imageResult);
    napi_value ret;
