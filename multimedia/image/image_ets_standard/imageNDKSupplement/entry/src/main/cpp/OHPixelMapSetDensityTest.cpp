...
#define LOG_TAG "PixelMapSetDensityNDK"
...
napi_value PixelMapToSetDensity(napi_env env, napi_callback_info info)
{
...
napi_value OHPixelMapSetDensity(napi_env env, napi_callback_info info)
{
    LOGI("OH_PixelMap_SetDensity start");
    napi_value thisVar = nullptr;
    napi_value argValue[1] = {0};
    size_t argCount = 1;
    if (napi_get_cb_info(env, info, &argCount, argValue, &thisVar, nullptr) != napi_ok || argCount < 1 ||
        argValue[0] == nullptr) {
        LOGI("OH_PixelMap_SetDensity error1");
        return nullptr;
    }
    int32_t index;
    ...
    NativePixelMap *native = OH_PixelMap_InitNativePixelMap(env, PixelMapToSetDensity(env, info));
    if (native == nullptr) {
        LOGI("OH_PixelMap_SetDensity error2");
        return result;
    }
    ...
    switch (index) {
    case CASE_INDEX_1:
        imageResult = OH_PixelMap_SetDensity(native, densityS);
        LOGI("值：%{public}d", imageResult);
        break;
    case CASE_INDEX_2:
        imageResult = OH_PixelMap_SetDensity(nullptr, densityS);
        LOGI("值：%{public}d", imageResult);
        break;
    default:
        break;
