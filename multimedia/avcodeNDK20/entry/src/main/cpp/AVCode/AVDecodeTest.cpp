...
AVDecodeTest *g_decSample = nullptr;
...
bool AVDecodeTest::MdCompare(unsigned char buffer[], int len, const char *source[])
{
    bool result = true;
    for (int i = 0; i < len; i++) {
    }
...
int64_t AVDecodeTest::GetSystemTimeUs()
{
    struct timespec now;
...
int32_t AVDecodeTest::ConfigureVideoDecoder()
{
...
void AVDecodeTest::CreateSurface()
{
    return;
}

int32_t AVDecodeTest::RunVideoDecSurface(string codeName)
{
    sfOutput = true;
    int err = AV_ERR_OK;
    CreateSurface();
...
int32_t AVDecodeTest::RunVideoDec(string codeName)
{
    sfOutput = false;
...
int32_t AVDecodeTest::SetVideoDecoderCallback()
{
...
void AVDecodeTest::ReleaseInFile()
{
    if (inFile_ != nullptr) {
        if (inFile_->is_open()) {
            inFile_->close();
...
int32_t AVDecodeTest::CreateVideoDecoder(string codeName)
{
...
int32_t AVDecodeTest::StartVideoDecoder()
{
    ...
    inputLoop_ = make_unique<thread>(&AVDecodeTest::InputFuncTest, this);
    ...
    outputLoop_ = make_unique<thread>(&AVDecodeTest::OutputFuncTest, this);
...
int32_t AVDecodeTest::StartSyncVideoDecoder()
{
    isRunning_.store(true);
    int ret = OH_VideoDecoder_Start(vdec_);
    if (ret != AV_ERR_OK) {
    ...
    signal_ = new VDecAPI11Signal();
    inputLoop_ = make_unique<thread>(&AVDecodeTest::SyncInputFunc, this);
    if (inputLoop_ == nullptr) {
        cout << "Failed to create input loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
    ...
    outputLoop_ = make_unique<thread>(&AVDecodeTest::SyncOutputFunc, this);
    if (outputLoop_ == nullptr) {
        cout << "Failed to create output loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
...
void AVDecodeTest::InputFuncTest()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
    ...
    while (flag) {
        ...
        lock.unlock();
        if (!inFile_->eof()) {
            int pushResult = PushData(index, buffer);
            if (pushResult == 1) {
                flag = false;
                break;
            }
...
void AVDecodeTest::SyncInputFunc()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
    ...
    while (flag) {
        ...
        uint32_t index;
        int32_t ret = OH_VideoDecoder_QueryInputBuffer(vdec_, &index, syncInputWaitTime);
        if (ret == AV_ERR_OK) {
            cout << "OH_VideoDecoder_QueryInputBuffer success" << endl;
        } else {
            errCount = errCount + 1;
            continue;
        }
...
int32_t AVDecodeTest::PushData(uint32_t index, OH_AVBuffer *buffer)
{
    ...
    uint32_t tmp = (((ch[3] & 0xFF)) | ((ch[2] & 0xFF) << EIGHT) | ((ch[1] & 0xFF) << SIXTEEN) |
                                     ((ch[0] & 0xFF) << TWENTY_FOUR));
    uint32_t bufferSize = tmp;
    if (useHDRSource) {
        uint32_t zero = 0;
        uint32_t one = 1;
        uint32_t two = 2;
        uint32_t three = 3;
        tmp = ((ch[zero] & 0xFF)) | ((ch[one] & 0xFF) << EIGHT) | ((ch[two] & 0xFF) << SIXTEEN) |
...
int32_t AVDecodeTest::CheckAndReturnBufferSize(OH_AVBuffer *buffer)
{
...
void AVDecodeTest::SetAttr(OH_AVCodecBufferAttr &attr, uint32_t bufferSize)
{
...
uint32_t AVDecodeTest::SendData(uint32_t bufferSize, uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
    ...
    if (isRunning_.load()) {
        ...
        outCount = outCount + 1;
        int32_t dFrameRate = static_cast<int32_t>(defaultFrameRate);
        if (autoSwitchSurface && (frameCount_ % dFrameRate == 0)) {
            switchSurfaceFlag = (switchSurfaceFlag == 1) ? 0 : 1;
...
void AVDecodeTest::CheckOutputDescription()
{
...
void AVDecodeTest::AutoSwitchSurface()
{
    if (autoSwitchSurface) {
        switchSurfaceFlag = (switchSurfaceFlag == 1) ? 0 : 1;
...
int32_t AVDecodeTest::CheckAttrFlag(OH_AVCodecBufferAttr attr)
{
...
void AVDecodeTest::OutputFuncTest()
{
...
void AVDecodeTest::SyncOutputFunc()
{
    FILE *outFile = nullptr;
    if (outputYuvFlag) {
        outFile = fopen(outputDir, "wb");
    ...
    while (flag) {
        ...
        if (ret == AV_ERR_OK) {
            cout << "OH_VideoDecoder_QueryOutputBuffer success" << endl;
        } else {
            errCount = errCount + 1;
            continue;
        }
...
int32_t AVDecodeTest::SyncOutputFuncEos(OH_AVCodecBufferAttr attr, uint32_t index)
{
    if (CheckAttrFlag(attr) == -1) {
...
void AVDecodeTest::ProcessOutputData(OH_AVBuffer *buffer, uint32_t index, int32_t size)
{
    if (!sfOutput) {
        int32_t nTmpSize = (defaultWidth * defaultHeight * THREE) >> 1;
        if (size >= nTmpSize) {
            int32_t ret = OH_VideoDecoder_FreeOutputData(vdec_, index);
            if (ret == AV_ERR_OK) {
                cout << "OH_VideoDecoder_FreeOutputData success" << endl;
            } else {
                errCount = errCount + 1;
            }
        }
        ...
        if (rsAtTime) {
            ...
            int32_t ret = OH_VideoDecoder_RenderOutputBufferAtTime(vdec_, index, renderTimestampNs);
            if (ret == AV_ERR_OK) {
                cout << "Fatal: RenderOutputBufferAtTime success" << endl;
            } else {
                errCount = errCount + 1;
            }
        } else {
...
void AVDecodeTest::SetEOS(uint32_t index, OH_AVBuffer *buffer)
{
    OH_AVCodecBufferAttr attr;
    attr.pts = 0;
    attr.size = 0;
...
int32_t AVDecodeTest::Start()
{
    isRunning_.store(true);
    return OH_VideoDecoder_Start(vdec_);
}

void AVDecodeTest::StopOutloop()
{
...
int32_t AVDecodeTest::SetParameter(OH_AVFormat *format)
{
    return OH_VideoDecoder_SetParameter(vdec_, format);
}

int32_t AVDecodeTest::SwitchSurface()
{
...
int32_t AVDecodeTest::RepeatCallSetSurface()
{
...
int32_t AVDecodeTest::DecodeSetSurface()
{
    CreateSurface();
