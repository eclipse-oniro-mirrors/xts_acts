...
OH_PhotoNative* NDKCamera::g_photoNative = nullptr;
...
const Camera_Profile YUV_PHOTO_PROFILE = { CAMERA_FORMAT_YUV_420_SP, { 640, 480 } };
...
NDKCamera::NDKCamera(char* str)
    : cameras_(nullptr), cameraOutputCapability_(nullptr),
      ...
      isPhotoQualityPrioritizationSupported_(false), photoNative_(nullptr),
...
Camera_ErrorCode NDKCamera::GetCameraHostNameErrorParameter(void)
{
...
Camera_ErrorCode NDKCamera::GetCameraHostTypeErrorParameter(void)
{
...
Camera_ErrorCode NDKCamera::GetCameraHostNameInspection(void)
{
    for (int32_t deviceIndex = 0; deviceIndex < size_; deviceIndex++) {
...
Camera_ErrorCode NDKCamera::GetCameraHostTypeInspection(void)
{
    for (int32_t deviceIndex = 0; deviceIndex < size_; deviceIndex++) {
...
Camera_ErrorCode NDKCamera::GetSupportedFullOutputCapability(void)
{
    if (cameraManager_ == nullptr) {
        LOG("ndkXTS cameraManager_ is null.");
    } else if (cameras_ == nullptr) {
    ...
    ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(
        cameraManager_, cameras_, NORMAL_PHOTO, &cameraOutputCapability_);
    if (cameraOutputCapability_ == nullptr || ret_ != CAMERA_OK) {
        LOG("ndkXTS GetSupportedFullCameraOutputCapabilityWithSceneMode failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
    return ret_;
}

int NDKCamera::AdjustHeifIdx(int idx)
{
    if (idx == -1) {
        return idx;
    }
    if (cameraOutputCapability_ == nullptr) {
        return CAMERA_INVALID_ARGUMENT;
    }
    for (; idx < cameraOutputCapability_->photoProfilesSize; idx++) {
        if (cameraOutputCapability_->photoProfiles[idx]->format == CAMERA_FORMAT_HEIC) {
            break;
        }
    }
    if (idx >= cameraOutputCapability_->photoProfilesSize) {
        idx = -1;
    }
    return idx;
}
...
{
    LOG("ndkXTS CreatePreviewOutput begin.");
...
Camera_ErrorCode NDKCamera::CreateDeferredPreviewOutput(void)
{
    LOG("ndkXTS CreateDeferredPreviewOutput begin.");
    profile_ = cameraOutputCapability_->previewProfiles[0];
    for (decltype(cameraOutputCapability_->previewProfilesSize) i = 0; i < cameraOutputCapability_->previewProfilesSize;
         i++) {
        if (cameraOutputCapability_->previewProfiles[i]->size.width == DEFAULT_PREVIEW_PROFILE.size.width &&
    ...
    ret_ = OH_CameraManager_CreateDeferredPreviewOutput(cameraManager_, profile_, &previewOutput_);
...
Camera_ErrorCode NDKCamera::CreateHeifPhotoOutput(char* photoSurfaceId, int idx)
{
    LOG("ndkXTS CreateHeifPhotoOutput start.");
    profile_ = cameraOutputCapability_->photoProfiles[idx];
    LOG("ndkXTS CreateHeifPhotoOutput photoProfiles start.");
    if (profile_ == nullptr) {
        LOG("ndkXTS Get photoProfiles failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    if (photoSurfaceId == nullptr) {
        LOG("ndkXTS CreateHeifPhotoOutput failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
    LOG("ndkXTS CreateHeifPhotoOutput start.");
    ret_ = OH_CameraManager_CreatePhotoOutput(cameraManager_, profile_, photoSurfaceId, &photoOutput_);
...
Camera_ErrorCode NDKCamera::CreateVideoOutput(char* videoId)
{
    ...
    for (decltype(cameraOutputCapability_->videoProfilesSize) i = 0; i < cameraOutputCapability_->videoProfilesSize;
        ...
        if (cameraOutputCapability_->videoProfiles[i]->size.width == DEFAULT_VIDEO_PROFILE.size.width &&
            ...
            if ((float)(cameraOutputCapability_->videoProfiles[i]->size.width /
                cameraOutputCapability_->videoProfiles[i]->size.height) -
                (float)(cameraOutputCapability_->previewProfiles[0]->size.width /
                cameraOutputCapability_->previewProfiles[0]->size.height) < errorRange) {
...
{
    if (cameraOutputCapability_->supportedMetadataObjectTypes == nullptr) {
        return CAMERA_SERVICE_FATAL_ERROR;
    }
...
bool NDKCamera::IsMetadataObjectTypeValid()
{
    if (cameraOutputCapability_->supportedMetadataObjectTypes == nullptr) {
        return false;
    }
...
Camera_ErrorCode NDKCamera::MetadataOutputRemoveMetadataObjectTypes()
{
    LOG("ndkXTS MetadataOutputRemoveMetadataObjectTypes begin.");
    uint32_t size = 1;
    Camera_MetadataObjectType* types = new Camera_MetadataObjectType[size];
    types[0] = Camera_MetadataObjectType::CAMERA_METADATA_OBJECT_TYPE_HUMAN_BODY;
    Camera_ErrorCode ret = OH_MetadataOutput_RemoveMetadataObjectTypes(metadataOutput_, types, size);
    if (ret == CAMERA_OK) {
        LOG("ndkXTS OH_MetadataOutput_RemoveMetadataObjectTypes success.");
    } else {
        LOG("ndkXTS OH_MetadataOutput_RemoveMetadataObjectTypes failed. %d ", ret);
    }
    return ret;
}

Camera_ErrorCode NDKCamera::MetadataOutputAddMetadataObjectTypes()
{
    LOG("ndkXTS MetadataOutputAddMetadataObjectTypes begin.");
    uint32_t size = 1;
    Camera_MetadataObjectType* types = new Camera_MetadataObjectType[size];
    types[0] = Camera_MetadataObjectType::CAMERA_METADATA_OBJECT_TYPE_HUMAN_BODY;
    Camera_ErrorCode ret = OH_MetadataOutput_AddMetadataObjectTypes(metadataOutput_, types, size);
    if (ret == CAMERA_OK) {
        LOG("ndkXTS OH_MetadataOutput_AddMetadataObjectTypes success.");
    } else {
        LOG("ndkXTS OH_MetadataOutput_AddMetadataObjectTypes failed. %d ", ret);
    }
    return ret;
}

Camera_ErrorCode NDKCamera::CreateMetadataOutputWithObjectTypes()
{
    uint32_t size = 2;
    Camera_MetadataObjectType* types = new Camera_MetadataObjectType[size];
    types[0] = Camera_MetadataObjectType::CAMERA_METADATA_OBJECT_TYPE_HUMAN_BODY;
    types[1] = Camera_MetadataObjectType::CAMERA_METADATA_OBJECT_TYPE_FACE_DETECTION;
    ret_ = OH_CameraManager_CreateMetadataOutputWithObjectTypes(cameraManager_, types, size, &metadataOutput_);
    if (metadataOutput_ == nullptr || ret_ != CAMERA_OK) {
        LOG("ndkXTS CreateMetadataOutputWithMetadataObjectTypes failed.");
        return CAMERA_INVALID_ARGUMENT;
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::SessionGetPhotoRotationWithoutDeviceDegree()
{
    LOG("GetPhotoRotationWithoutDeviceDegree begin.");
    Camera_ErrorCode ret = OH_PhotoOutput_GetPhotoRotationWithoutDeviceDegree(photoOutput_, &imageRotation_);
    if (ret == CAMERA_OK) {
        LOG("OH_PhotoOutput_GetPhotoRotationWithoutDeviceDegree success.");
    } else {
        LOG("OH_PhotoOutput_GetPhotoRotationWithoutDeviceDegree failed. %d ", ret);
    }
    return ret;
}
...
Camera_ErrorCode NDKCamera::SessionGetVideoRotationWithoutDeviceDegree()
{
    LOG("GetVideoRotationWithoutDeviceDegree begin.");
    Camera_ErrorCode ret = OH_VideoOutput_GetVideoRotationWithoutDeviceDegree(videoOutput_, &imageRotation_);
    if (ret == CAMERA_OK) {
        LOG("OH_VideoOutput_GetVideoRotationWithoutDeviceDegree success.");
    } else {
        LOG("OH_VideoOutput_GetVideoRotationWithoutDeviceDegree failed. %d ", ret);
    }
    return ret;
}
...
Camera_ErrorCode NDKCamera::SessionGetPreviewRotationWithoutDisplayRotation()
{
    LOG("GetPreviewRotationWithoutDisplayRotation begin.");
    Camera_ErrorCode ret = OH_PreviewOutput_GetPreviewRotationWithoutDisplayRotation(previewOutput_, &imageRotation_);
    if (ret == CAMERA_OK) {
        LOG("OH_PreviewOutput_GetPreviewRotationWithoutDisplayRotation success.");
    } else {
        LOG("OH_PreviewOutput_GetPreviewRotationWithoutDisplayRotation failed. %d ", ret);
    }
    return ret;
}

Camera_ImageRotation GetCameraImageRotation(int rotation)
{
    Camera_ImageRotation testRotation = static_cast<Camera_ImageRotation>(-1);
    switch (rotation) {
        case IMAGE_0:
            testRotation = IAMGE_ROTATION_0;
            break;
        case IMAGE_90:
            testRotation = IAMGE_ROTATION_90;
            break;
        case IMAGE_180:
            testRotation = IAMGE_ROTATION_180;
            break;
        case IMAGE_270:
            testRotation = IAMGE_ROTATION_270;
            break;
        case ROTATION_0:
            testRotation = CAMERA_IMAGE_ROTATION_0;
            break;
        case ROTATION_90:
            testRotation = CAMERA_IMAGE_ROTATION_90;
            break;
        case ROTATION_180:
            testRotation = CAMERA_IMAGE_ROTATION_180;
            break;
        case ROTATION_270:
            testRotation = CAMERA_IMAGE_ROTATION_270;
            break;
        default: break;
    }
    return testRotation;
...
////问题14
    Camera_ImageRotation testRotation = GetCameraImageRotation(rotation);
...
Camera_ErrorCode NDKCamera::SessionIsControlCenterSupported(void)
{
    LOG("IsControlCenterSupported begin.");
    Camera_ErrorCode ret = OH_CaptureSession_IsControlCenterSupported(captureSession_, &isContolCenterSupported_);
    if (ret == CAMERA_OK) {
        LOG("OH_CaptureSession_IsControlCenterSupported success.");
    } else {
        LOG("OH_CaptureSession_IsControlCenterSupported failed. %d ", ret);
    }
    return ret;
}

Camera_ErrorCode NDKCamera::SessionGetSupportedEffectTypes(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        controlCenterEffectType_ = new Camera_ControlCenterEffectType[1];
        ret_ = OH_CaptureSession_GetSupportedEffectTypes(captureSession_, &controlCenterEffectType_, &effectTypeSize_);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedEffectTypes(nullptr, &controlCenterEffectType_, &effectTypeSize_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedEffectTypes(captureSession_, nullptr, &effectTypeSize_);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedEffectTypes(captureSession_, &controlCenterEffectType_, nullptr);
    }
    if (controlCenterEffectType_ == nullptr || effectTypeSize_ == 0 || ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_GetSupportedEffectTypes failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SessionDeleteSupportedEffectTypes(int useCaseCode)
{
    LOG("NDKCamera::SessionDeleteSupportedEffectTypes");
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_DeleteSupportedEffectTypes(captureSession_, controlCenterEffectType_, effectTypeSize_);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_DeleteSupportedEffectTypes(nullptr, controlCenterEffectType_, effectTypeSize_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_DeleteSupportedEffectTypes(captureSession_, nullptr, effectTypeSize_);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CaptureSession_DeleteSupportedEffectTypes(captureSession_, controlCenterEffectType_, 0);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_DeleteSupportedEffectTypes failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SessionEnableControlCenter(bool enabled)
{
    ret_ = OH_CaptureSession_EnableControlCenter(captureSession_, enabled);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_EnableControlCenter failed.");
    }
    return ret_;
}
...
{
    LOG("ControlCenterEffectStatusChange is called.");
}

Camera_ErrorCode NDKCamera::RegisterControlCenterEffectStatusChange(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_RegisterControlCenterEffectStatusChangeCallback(captureSession_,
            ControlCenterEffectStatusChange);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_RegisterControlCenterEffectStatusChangeCallback(nullptr,
            ControlCenterEffectStatusChange);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_RegisterControlCenterEffectStatusChangeCallback(captureSession_,
            nullptr);
    ...
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_RegisterControlCenterEffectStatusChangeCallback failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::UnregisterControlCenterEffectStatusChange(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_UnregisterControlCenterEffectStatusChangeCallback(captureSession_,
            ControlCenterEffectStatusChange);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_UnregisterControlCenterEffectStatusChangeCallback(nullptr,
            ControlCenterEffectStatusChange);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_UnregisterControlCenterEffectStatusChangeCallback(captureSession_,
            nullptr);
    ...
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_UnregisterControlCenterEffectStatusChangeCallback failed.");
    }
    return ret_;
}
...
void OcclusionDetectionCallback(const Camera_Input* cameraInput,
    Camera_OcclusionDetectionResult occlusionDetectionResult)
{
    if (occlusionDetectionResult.isCameraOccluded) {
        LOG("isCameraOccluded true");
    }
    if (occlusionDetectionResult.isCameraLensDirty) {
        LOG("isCameraLensDirty true");
    }
}

Camera_ErrorCode NDKCamera::CameraInputRegisterOcclusionDetectionCallback(void)
{
    LOG("CameraOcclusionDetectChangeTest begin.");
    if (cameraInput_ == nullptr) {
        LOG("ndkXTS CameraInput is nullptr");
        return CAMERA_INVALID_ARGUMENT;
    }
    ret_ = OH_CameraInput_RegisterOcclusionDetectionCallback(
        cameraInput_, OcclusionDetectionCallback);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraInput_RegisterOcclusionDetectionCallback failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::CameraInputUnRegisterOcclusionDetectionCallback(void)
{
    LOG("CameraOcclusionDetectChangeTest begin.");
    if (cameraInput_ == nullptr) {
        LOG("ndkXTS CameraInput is nullptr");
    ...
    ret_ = OH_CameraInput_UnregisterOcclusionDetectionCallback(
        cameraInput_, OcclusionDetectionCallback);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraInput_UnregisterOcclusionDetectionCallback failed.");
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::GetSupportedFullCameraOutputCapabilityWithSceneMode(int useCaseCode)
{
    if (camera_ == nullptr) {
        ret_ = GetCameraFromCameras(cameras_, &camera_);
    }
    if (useCaseCode == PARAMETER_OK) {
        ...
        ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(cameraManager_,
            camera_, sceneMode_, &cameraOutputCapability_);
    } else if (useCaseCode == PARAMETER4_ERROR) {
        ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(cameraManager_,
            camera_, sceneMode_, nullptr);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(cameraManager_,
            camera_, static_cast<Camera_SceneMode>(0), &cameraOutputCapability_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(cameraManager_,
            nullptr, sceneMode_, &cameraOutputCapability_);
    } else {
        ret_ = OH_CameraManager_GetSupportedFullCameraOutputCapabilityWithSceneMode(nullptr,
            camera_, sceneMode_, &cameraOutputCapability_);
    }
    if (cameraOutputCapability_ == nullptr || ret_ != CAMERA_OK) {
        LOG("ndkXTS OH_CaptureSession_UnregisterCallback failed.%d", ret_);
...
Camera_ErrorCode NDKCamera::IsPhysicalCameraOrientationVariable(bool* isVariable)
{
    ret_ = OH_CameraInput_IsPhysicalCameraOrientationVariable(cameraInput_, isVariable);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraInput_IsPhysicalCameraOrientationVariable failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetPhysicalCameraOrientation(uint32_t* orientation)
{
    ret_ = OH_CameraInput_GetPhysicalCameraOrientation(cameraInput_, orientation);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraInput_GetPhysicalCameraOrientation failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::UsePhysicalCameraOrientation(bool isUsed)
{
    ret_ = OH_CameraInput_UsePhysicalCameraOrientation(cameraInput_, isUsed);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraInput_UsePhysicalCameraOrientation failed.");
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::IsPhotoQualityPrioritizationSupported(int useCaseCode, int quality)
{
    Camera_PhotoQualityPrioritization qualityPrioritization = static_cast<Camera_PhotoQualityPrioritization>(quality);
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_PhotoOutput_IsPhotoQualityPrioritizationSupported(photoOutput_, qualityPrioritization,
            &isPhotoQualityPrioritizationSupported_);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_PhotoOutput_IsPhotoQualityPrioritizationSupported(nullptr, qualityPrioritization,
            &isPhotoQualityPrioritizationSupported_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_PhotoOutput_IsPhotoQualityPrioritizationSupported(photoOutput_, qualityPrioritization,
            &isPhotoQualityPrioritizationSupported_);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_PhotoOutput_IsPhotoQualityPrioritizationSupported(photoOutput_, qualityPrioritization, nullptr);
    }

    if (ret_ != CAMERA_OK) {
        LOG("OH_PhotoOutput_IsPhotoQualityPrioritizationSupported failed. %d", ret_);
    } else {
        if (isMovingPhotoSupported_ != true) {
            LOG("PhotoQualityPrioritization is not supported.");
        } else {
            LOG("PhotoQualityPrioritization is supported.");
        }
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SetPhotoQualityPrioritization(int useCaseCode, int quality)
{
    Camera_PhotoQualityPrioritization qualityPrioritization = static_cast<Camera_PhotoQualityPrioritization>(quality);
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_PhotoOutput_SetPhotoQualityPrioritization(photoOutput_, qualityPrioritization);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_PhotoOutput_SetPhotoQualityPrioritization(nullptr, qualityPrioritization);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_PhotoOutput_SetPhotoQualityPrioritization(photoOutput_, qualityPrioritization);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_PhotoOutput_SetPhotoQualityPrioritization failed. %d", ret_);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::AddDeferredSurface(int useCaseCode, char* surfaceId)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_PreviewOutput_AddDeferredSurface(previewOutput_, surfaceId);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_PreviewOutput_AddDeferredSurface(nullptr, surfaceId);
    } else {
        ret_ = OH_PreviewOutput_AddDeferredSurface(previewOutput_, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_PreviewOutput_EnableBandwidthCompression failed. %d", ret_);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::GetUncompressedImage(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_PhotoNative_GetUncompressedImage(g_photoNative, &pictureNative_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_PhotoNative_GetUncompressedImage(g_photoNative, nullptr);
    } else {
        ret_ = OH_PhotoNative_GetUncompressedImage(nullptr, &pictureNative_);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_PhotoNative_GetUncompressedImage failed. %d", ret_);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::PhotoNativeRelease(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ...
        ret_ = OH_PhotoNative_Release(g_photoNative);
    } else {
...
Camera_ErrorCode NDKCamera::CreateYuvPhotoOutputWithoutSurface(int useCaseCode)
{
    for (decltype(cameraOutputCapability_->photoProfilesSize) i = 0;
            i < cameraOutputCapability_->photoProfilesSize; i++) {
        if (cameraOutputCapability_->photoProfiles[i]->format == YUV_PHOTO_PROFILE.format) {
            photoProfile_ = cameraOutputCapability_->photoProfiles[i];
            break;
        }
    }
    if (!photoProfile_) {
        LOG("photoProfile_ is not yuv");
        photoProfile_ = cameraOutputCapability_->photoProfiles[0];
    }
    LOG("createYuvPhotoOutputWithoutSurface photoProfile width=%u, height=%u, format=%d",
        photoProfile_->size.width, photoProfile_->size.height, photoProfile_->format);
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CameraManager_CreatePhotoOutputWithoutSurface(cameraManager_, photoProfile_, &photoOutput_);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CameraManager_CreatePhotoOutputWithoutSurface(cameraManager_, photoProfile_, nullptr);
    ...
    if (ret_ != CAMERA_OK || !photoOutput_) {
        ret_ = CAMERA_INVALID_ARGUMENT;
        LOG("createYuvPhotoOutputWithoutSurface failed. %d", ret_);
    }
    ret_ = OH_PhotoOutput_RegisterPhotoAvailableCallback(photoOutput_, PhotoOutputOnPhotoAvailable);
    return ret_;
}
...
Camera_ErrorCode NDKCamera::TorchMode(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ...
        torchMode_ = AUTO;
    } else if (useCaseCode == TORCH_MODE_OFF) {
        torchMode_ = CAMERA_TORCH_MODE_OFF;
    } else if (useCaseCode == TORCH_MODE_ON) {
        torchMode_ = CAMERA_TORCH_MODE_ON;
    } else if (useCaseCode == TORCH_MODE_AUTO) {
        torchMode_ = CAMERA_TORCH_MODE_AUTO;
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CameraManager_IsTorchSupported failed.");
    }
...
bool NDKCamera::IsTorchLevelControlSupported()
{
    bool ret = false;
    OH_CameraManager_IsTorchLevelControlSupported(cameraManager_, &ret);
    return ret;
}
 
Camera_ErrorCode NDKCamera::SetTorchModeOnWithLevel(double level)
{
    return OH_CameraManager_SetTorchModeOnWithLevel(cameraManager_, level);
}
...
Camera_ErrorCode NDKCamera::SetSmoothZoom(int useCaseCode)
{
    ...
    Camera_SmoothZoomMode cameraSmoothZoomMode = CAMERA_SMOOTH_ZOOM_MODE_NORMAL;
    if (useCaseCode == PARAMETER_OK) {
    ...
    } else if (useCaseCode == SMOOTH_ZOOM_MODE_NORMAL) {
        ret_ = OH_CaptureSession_SetSmoothZoom(captureSession_, targetZoom, cameraSmoothZoomMode);
    }
    if (ret_ != CAMERA_OK) {
...
void CameraSystemPressureLevelCallback(Camera_CaptureSession* session,
    Camera_SystemPressureLevel systemPressureLevel)
{
    LOG("CameraSystemPressureLevelCallback is called.");
}
 
Camera_ErrorCode NDKCamera::RegisterSystemPressureLevel(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_RegisterSystemPressureLevelChangeCallback(captureSession_,
            CameraSystemPressureLevelCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_RegisterSystemPressureLevelChangeCallback(nullptr,
            CameraSystemPressureLevelCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_RegisterSystemPressureLevelChangeCallback(captureSession_, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_RegisterSystemPressureLevelChangeCallback failed.");
    }
    return ret_;
}
 
Camera_ErrorCode NDKCamera::UnregisterSystemPressureLevel(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_UnregisterSystemPressureLevelChangeCallback(captureSession_,
            CameraSystemPressureLevelCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_UnregisterSystemPressureLevelChangeCallback(nullptr,
            CameraSystemPressureLevelCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_UnregisterSystemPressureLevelChangeCallback(captureSession_, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_UnregisterSystemPressureLevelChangeCallback failed.");
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::GetCameraDevice(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ...
        LOG("camera getSupportedCameras success : %d", size_);
        if (size_ == 0) {
            return CAMERA_OK;
        }
        ...
        ret_ = OH_CameraManager_GetCameraDevice(cameraManager_,
            cameraArray[0].cameraPosition, cameraArray[0].cameraType, cameranow);
    }
    return ret_;
}
...
Camera_ErrorCode NDKCamera::GetCameraDevices(int useCaseCode)
{
    cameraDevices_ = nullptr;
    cameraSize_ = 0;
    OH_CameraManager_GetSupportedCameras(cameraManager_, &cameraDevices_, &cameraSize_);
    if (isDeviceQueryInfoInvalid_) {
        deviceQueryInfo_ = new Camera_DeviceQueryInfo;
        deviceQueryInfo_->cameraPosition = CAMERA_POSITION_UNSPECIFIED;
        deviceQueryInfo_->connectionType = CAMERA_CONNECTION_BUILT_IN;
        deviceQueryInfo_->cameraTypeSize = 0;
        Camera_Type* types = new Camera_Type[1];
        types[0] = CAMERA_TYPE_DEFAULT;
        deviceQueryInfo_->cameraType = types;
    } else {
        deviceQueryInfo_ = new Camera_DeviceQueryInfo;
        deviceQueryInfo_->cameraPosition = cameraDevices_[0].cameraPosition;
        deviceQueryInfo_->connectionType = cameraDevices_[0].connectionType;
        deviceQueryInfo_->cameraTypeSize = 1;
        Camera_Type* types = new Camera_Type[1];
        types[0] = cameraDevices_[0].cameraType;
        deviceQueryInfo_->cameraType = types;
    }
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CameraManager_GetCameraDevices(cameraManager_, deviceQueryInfo_, &cameraSize_, &cameraDevices_);
        if (ret_ == CAMERA_OK) {
            LOG("camera getCameraDevices success");
        }
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CameraManager_GetCameraDevices(nullptr, deviceQueryInfo_, &cameraSize_, &cameraDevices_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CameraManager_GetCameraDevices(cameraManager_, nullptr, &cameraSize_, &cameraDevices_);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CameraManager_GetCameraDevices(cameraManager_, deviceQueryInfo_, nullptr, &cameraDevices_);
    } else if (useCaseCode == PARAMETER4_ERROR) {
        ret_ = OH_CameraManager_GetCameraDevices(cameraManager_, deviceQueryInfo_, &cameraSize_, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("ndkXTS OH_CameraManager_GetCameraDevices failed : %{public}d", ret_);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::DeleteCameraDevices(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CameraManager_DeleteCameraDevices(cameraManager_, cameraDevices_);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CameraManager_DeleteCameraDevices(nullptr, cameraDevices_);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CameraManager_DeleteCameraDevices(cameraManager_, nullptr);
    }
    if (cameras_ != nullptr || ret_ != CAMERA_OK) {
        LOG("ndkXTS OH_CameraManager_DeleteCameraDevices failed : %{public}d", ret_);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SetInvalidDeviceQueryInfo(bool isDeviceQueryInfoInvalid)
{
    isDeviceQueryInfoInvalid_ = isDeviceQueryInfoInvalid;
    return CAMERA_OK;
}
...
Camera_ErrorCode NDKCamera::WhiteBalanceTest(void)
{
    ...
    bool flag;
    ret_ = OH_CaptureSession_IsWhiteBalanceModeSupported(
        captureSession_, Camera_WhiteBalanceMode::CAMERA_WHITE_BALANCE_MODE_CLOUDY, &flag);
    int32_t max = 0;
    int32_t min = 0;
    ret_ = OH_CaptureSession_GetWhiteBalanceRange(captureSession_, &min, &max);
    int32_t temp = 3000;
    ret_ = OH_CaptureSession_SetWhiteBalance(captureSession_, temp);
    ret_ = OH_CaptureSession_GetWhiteBalance(captureSession_, &temp);
    ret_ = OH_CaptureSession_SetWhiteBalanceMode(
    ...
    ret_ = OH_CaptureSession_GetWhiteBalanceMode(captureSession_, &mode);
    return ret_;
}

Camera_ErrorCode NDKCamera::CreatePreviewOutputByIndex(int index)
{
...
Camera_ErrorCode NDKCamera::CreateVideoOutputByIndex(char* videoId, int index)
{
    LOG("ndkXTS CreateVideoOutput start.");
...
void OnMacroStatusChange(Camera_CaptureSession* session, bool isMacroActive)
{
    LOG("OnMacroStatusChange MacroStatus ENTER");
}

Camera_ErrorCode NDKCamera::MacroStatusChangeTest(void)
{
    LOG("MacroStatusChangeTest begin.");
    bool flag;
    ret_ = OH_CaptureSession_RegisterMacroStatusChangeCallback(
        captureSession_, OnMacroStatusChange);
    ret_ = OH_CaptureSession_UnregisterMacroStatusChangeCallback(
        captureSession_, OnMacroStatusChange);
    return ret_;
}

void OnIsoInfoChange(Camera_CaptureSession* session, int32_t isoValue)
{
    LOG("OnIsoInfoChange IsoInfo ENTER");
}

Camera_ErrorCode NDKCamera::IsoInfoChangeTest(void)
{
    LOG("IsoInfoChangeTest begin.");
    ret_ = OH_CaptureSession_RegisterIsoChangeCallback(
        captureSession_, OnIsoInfoChange);
    ret_ = OH_CaptureSession_UnregisterIsoChangeCallback(
        captureSession_, OnIsoInfoChange);
    return ret_;
}

void TestFlashStateCallback(const Camera_CaptureSession* session, OH_Camera_FlashState  flashState)
{
    LOG("FlashStateCallback is called.");
}

Camera_ErrorCode NDKCamera::TestRegisterFlashStateChangeCallback(int useCaseCode)
{
    LOG("TestRegisterFlashStateChangeCallback begin.");
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_RegisterFlashStateChangeCallback(captureSession_, TestFlashStateCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_RegisterFlashStateChangeCallback(nullptr, TestFlashStateCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_RegisterFlashStateChangeCallback(captureSession_, nullptr);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::TestUnregisterFlashStateChangeCallback(int useCaseCode)
{
    LOG("TestRegisterFlashStateChangeCallback begin.");
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_UnregisterFlashStateChangeCallback(captureSession_, TestFlashStateCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_UnregisterFlashStateChangeCallback(nullptr, TestFlashStateCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_UnregisterFlashStateChangeCallback(captureSession_, nullptr);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::IsExposureMeteringModeSupported(int useCaseCode)
{
    LOG("IsExposureMeteringModeSupported begin.");
    bool isSupported;
    ...
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_IsExposureMeteringModeSupported(captureSession_, exposureMeteringMode, &isSupported);
        LOG("IsExposureMeteringModeSupported: %d", isSupported);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_IsExposureMeteringModeSupported(nullptr, exposureMeteringMode, &isSupported);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_IsExposureMeteringModeSupported(captureSession_, exposureMeteringMode, nullptr);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetExposureMeteringMode(int useCaseCode)
{
    LOG("GetExposureMeteringMode begin.");
    OH_Camera_ExposureMeteringMode getExposureMeteringMode;
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_GetExposureMeteringMode(captureSession_, &getExposureMeteringMode);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetExposureMeteringMode(nullptr, &getExposureMeteringMode);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_GetExposureMeteringMode(captureSession_, nullptr);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SetExposureMeteringMode(int useCaseCode)
{
    LOG("SetExposureMeteringMode begin.");
    OH_Camera_ExposureMeteringMode exposureMeteringMode =
        OH_Camera_ExposureMeteringMode::OH_CAMERA_EXPOSURE_METERING_MODE_CENTER ;
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_SetExposureMeteringMode(captureSession_, exposureMeteringMode);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_SetExposureMeteringMode(nullptr, exposureMeteringMode);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetSupportedISORange(int useCaseCode)
{
    LOG("GetSupportedISORange begin.");
    int32_t minIsoValue = 0;
    int32_t maxIsoValue = 0;
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_GetSupportedISORange(captureSession_, &minIsoValue, &maxIsoValue);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedISORange(nullptr, &minIsoValue, &maxIsoValue);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedISORange(captureSession_, nullptr, &maxIsoValue);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CaptureSession_GetSupportedISORange(captureSession_, &minIsoValue, nullptr);
    }
    return ret_;
}


Camera_ErrorCode NDKCamera::getIso(int useCaseCode)
{
    LOG("getIso begin.");
    int32_t getIso;
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_GetIso(captureSession_, &getIso);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetIso(nullptr, &getIso);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::setIso(int useCaseCode)
{
    LOG("setIso begin.");
    int32_t minIsoValue = 0;
    if (useCaseCode == CAMERA_OK) {
        ret_ = OH_CaptureSession_SetIso(captureSession_, minIsoValue);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_SetIso(nullptr, minIsoValue);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetCameraDevices(void)
{
    Camera_Type type = CAMERA_TYPE_ULTRA_WIDE;
    Camera_DeviceQueryInfo info ;
    info.cameraPosition = CAMERA_POSITION_BACK;
    info.cameraType = &type;
    info.cameraTypeSize = 1;
    info.connectionType = CAMERA_CONNECTION_BUILT_IN;
    ret_ = OH_CameraManager_GetCameraDevices(cameraManager_, &info, &size_, &cameras_);
    if (cameras_ == nullptr || ret_ != CAMERA_OK) {
        return CAMERA_INVALID_ARGUMENT;
    }
    return ret_;
...
Camera_ErrorCode NDKCamera::TestPhyAptNullSession()
{
    uint32_t size = 0;
    OH_Camera_PhysicalAperture* apertures = nullptr;
    return OH_CaptureSession_GetSupportedPhysicalApertures(
        nullptr, &apertures, &size);
}

Camera_ErrorCode NDKCamera::TestPhyAptNullApts()
{
    ...
    uint32_t size = 0;
    return OH_CaptureSession_GetSupportedPhysicalApertures(
        captureSession_, nullptr, &size);
}

Camera_ErrorCode NDKCamera::TestPhyAptNoInput()
{
    if (cameraManager_ == nullptr) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    Camera_ErrorCode ret = OH_CameraManager_CreateCaptureSession(
        cameraManager_, &session);
    if (ret != CAMERA_OK || session == nullptr) {
        return (ret != CAMERA_OK) ? ret : CAMERA_INVALID_ARGUMENT;
    }

    ret = OH_CaptureSession_BeginConfig(session);
    if (ret != CAMERA_OK) {
    ...
    uint32_t size = 0;
    OH_Camera_PhysicalAperture* apertures = nullptr;
    ret = OH_CaptureSession_GetSupportedPhysicalApertures(
        session, &apertures, &size);

    if (apertures != nullptr) {
        OH_CaptureSession_DeletePhysicalApertures(session, apertures, size);
    }
    OH_CaptureSession_Release(session);
    return ret;
}

Camera_ErrorCode NDKCamera::TestPhyAptNoOutput()
{
    LOG("TestPhyAptNoOutput begin.");

    Camera_CaptureSession* session = nullptr;
    Camera_ErrorCode ret = OH_CameraManager_CreateCaptureSession(cameraManager_, &session);
    if (ret != CAMERA_OK || session == nullptr) {
        return (ret != CAMERA_OK) ? ret : CAMERA_INVALID_ARGUMENT;
    }

    LOG("TestPhyAptNoOutput no output configured, only input added.");

    uint32_t size = 0;
    OH_Camera_PhysicalAperture* apertures = nullptr;

    ret = OH_CaptureSession_GetSupportedPhysicalApertures(session, &apertures, &size);
    LOG("OH_CaptureSession_GetSupportedPhysicalApertures ret = %d, apertures = %p, size = %u.",
        ret, apertures, size);

    if (ret != CAMERA_OK) {
        LOG("OH_CaptureSession_GetSupportedPhysicalApertures failed. %d ", ret);
    }

    if (apertures != nullptr) {
        LOG("OH_CaptureSession_DeletePhysicalApertures begin, apertures = %p, size = %u.", apertures, size);
        Camera_ErrorCode delRet = OH_CaptureSession_DeletePhysicalApertures(session, apertures, size);
        LOG("OH_CaptureSession_DeletePhysicalApertures ret = %d.", delRet);
        if (delRet != CAMERA_OK) {
            LOG("OH_CaptureSession_DeletePhysicalApertures failed. %d ", delRet);
        }
    } else {
        LOG("OH_CaptureSession_DeletePhysicalApertures not needed, apertures is nullptr.");
    }

    Camera_ErrorCode releaseRet = OH_CaptureSession_Release(session);
    LOG("OH_CaptureSession_Release ret = %d.", releaseRet);
    if (releaseRet != CAMERA_OK) {
        LOG("OH_CaptureSession_Release failed. %d ", releaseRet);
    }

    LOG("TestPhyAptNoOutput end. final ret = %d.", ret);
    return ret;
}

Camera_ErrorCode NDKCamera::TestDelPhyApts()
{
    if (captureSession_ == nullptr) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    uint32_t size = 0;
    OH_Camera_PhysicalAperture* apertures = nullptr;
    Camera_ErrorCode ret = OH_CaptureSession_GetSupportedPhysicalApertures(
        captureSession_, &apertures, &size);
    if (ret != CAMERA_OK) {
        return ret;
    }

    return OH_CaptureSession_DeletePhysicalApertures(
        captureSession_, apertures, size);
}

Camera_ErrorCode NDKCamera::TestDelPhyAptsNullSession()
{
    if (captureSession_ == nullptr) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    ret = OH_CaptureSession_DeletePhysicalApertures(nullptr, apertures, size);
    if (apertures != nullptr) {
...
Camera_ErrorCode NDKCamera::TestDelPhyAptsNullApts()
{
    if (captureSession_ == nullptr) {
        return CAMERA_INVALID_ARGUMENT;
    }
...
Camera_ErrorCode NDKCamera::TestLensDistNullCam()
{
    float* lens = nullptr;
    uint32_t size = 0;
    return OH_CameraDevice_GetLensDistortion(nullptr, &lens, &size);
}

Camera_ErrorCode NDKCamera::TestLensDistNullLens()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    uint32_t size = 0;
    return OH_CameraDevice_GetLensDistortion(&cameras_[cameraDeviceIndex_], nullptr, &size);
}

Camera_ErrorCode NDKCamera::TestLensDistNullSize()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    float* lens = nullptr;
    return OH_CameraDevice_GetLensDistortion(&cameras_[cameraDeviceIndex_], &lens, nullptr);
}

Camera_ErrorCode NDKCamera::TestIntrinsicNullCam()
{
    float* data = nullptr;
    uint32_t size = 0;
    return OH_CameraDevice_GetIntrinsicCalibration(nullptr, &data, &size);
}

Camera_ErrorCode NDKCamera::TestIntrinsicNullData()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    uint32_t size = 0;
    return OH_CameraDevice_GetIntrinsicCalibration(&cameras_[cameraDeviceIndex_], nullptr, &size);
}

Camera_ErrorCode NDKCamera::TestIntrinsicNullSize()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    float* data = nullptr;
    return OH_CameraDevice_GetIntrinsicCalibration(&cameras_[cameraDeviceIndex_], &data, nullptr);
}

Camera_ErrorCode NDKCamera::TestSensorSizeNullCam()
{
    float width = 0.0f;
    float height = 0.0f;
    return OH_CameraDevice_GetSensorPhysicalSize(nullptr, &width, &height);
}

Camera_ErrorCode NDKCamera::TestSensorSizeNullW()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    float height = 0.0f;
    return OH_CameraDevice_GetSensorPhysicalSize(&cameras_[cameraDeviceIndex_], nullptr, &height);
}

Camera_ErrorCode NDKCamera::TestSensorSizeNullH()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    float width = 0.0f;
    return OH_CameraDevice_GetSensorPhysicalSize(&cameras_[cameraDeviceIndex_], &width, nullptr);
}

Camera_ErrorCode NDKCamera::TestEqFocalNullCam()
{
    uint32_t* data = nullptr;
    uint32_t size = 0;
    return OH_CameraDevice_GetLensEquivalentFocalLengths(nullptr, &data, &size);
}

Camera_ErrorCode NDKCamera::TestEqFocalNullData()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    uint32_t size = 0;
    return OH_CameraDevice_GetLensEquivalentFocalLengths(
        &cameras_[cameraDeviceIndex_], nullptr, &size);
}

Camera_ErrorCode NDKCamera::TestEqFocalNullSize()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    uint32_t* data = nullptr;
    return OH_CameraDevice_GetLensEquivalentFocalLengths(
        &cameras_[cameraDeviceIndex_], nullptr, nullptr);
}

Camera_ErrorCode NDKCamera::TestEqFocal()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    uint32_t size = 0;
    return OH_CameraDevice_GetLensEquivalentFocalLengths(
        &cameras_[cameraDeviceIndex_], &data, &size);
}

Camera_ErrorCode NDKCamera::TestIsLogicNullCam()
{
    bool isLogical = false;
    return OH_CameraDevice_IsLogicalCamera(nullptr, &isLogical);
}

Camera_ErrorCode NDKCamera::TestIsLogicNullFlag()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }

    return OH_CameraDevice_IsLogicalCamera(
        &cameras_[cameraDeviceIndex_], nullptr);
}

Camera_ErrorCode NDKCamera::TestIsLogic()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    bool isLogical = false;
    return OH_CameraDevice_IsLogicalCamera(
        &cameras_[cameraDeviceIndex_], &isLogical);
}

Camera_ErrorCode NDKCamera::TestLogicCamDevsNullCam()
{
    Camera_Device* data = nullptr;
    uint32_t size = 0;
    return OH_CameraDevice_GetLogicalCameraConstituentCameraDevices(
        nullptr, &data, &size);
}

Camera_ErrorCode NDKCamera::TestLogicCamDevsNullData()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
...
Camera_ErrorCode NDKCamera::TestLogicCamDevsNullSize()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
    ...
    Camera_Device* data = nullptr;
    return OH_CameraDevice_GetLogicalCameraConstituentCameraDevices(
        &cameras_[cameraDeviceIndex_], &data, nullptr);
}

Camera_ErrorCode NDKCamera::TestLogicCamDevs()
{
    if (cameras_ == nullptr || size_ <= 0 || cameraDeviceIndex_ >= size_) {
        return CAMERA_INVALID_ARGUMENT;
    }
...
Camera_ErrorCode NDKCamera::DeleteConstituentCameraDevices(int useCaseCode)
{
    LOG("DeleteConstituentCameraDevices begin.");
    ret_ = CAMERA_OK;
    Camera_Device* constituentCameras;
    uint32_t size;
    for (int32_t i = 0; i < size_; i++) {
        OH_CameraDevice_GetLogicalCameraConstituentCameraDevices(&cameras_[i], &constituentCameras, &size);
        LOG("ConstituentCamera ret:%{public}d.", ret_);
        LOG("size size:%{public}d.", size);
        if (useCaseCode == CAMERA_OK) {
            if (size > 0) {
                ret_ = OH_CameraDevice_DeleteConstituentCameraDevices(&cameras_[i], constituentCameras, size);
                LOG("ConstituentCamera ret:%{public}d.", ret_);
            } else {
                ret_ = CAMERA_OK;
            }
        } else if (useCaseCode == PARAMETER1_ERROR) {
            ret_ = OH_CameraDevice_DeleteConstituentCameraDevices(nullptr, constituentCameras, size);
        } else if (useCaseCode == PARAMETER2_ERROR) {
            ret_ = OH_CameraDevice_DeleteConstituentCameraDevices(&cameras_[i], nullptr, size);
        }
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetLensFocalLength(int useCaseCode)
{
    LOG("GetLensFocalLength begin.");
    ret_ = CAMERA_OK;
    for (int32_t i = 0; i < size_; i++) {
        float lensFocalLength;
        if (useCaseCode == CAMERA_OK) {
            ret_ = OH_CameraDevice_GetLensFocalLength(&cameras_[i], &lensFocalLength);
            LOG("GetLensFocalLength ret:%{public}d.", lensFocalLength);
        } else if (useCaseCode == PARAMETER1_ERROR) {
            ret_ = OH_CameraDevice_GetLensFocalLength(nullptr, &lensFocalLength);
        } else if (useCaseCode == PARAMETER2_ERROR) {
            ret_ = OH_CameraDevice_GetLensFocalLength(&cameras_[i], nullptr);
        }
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetMinimumFocusDistance(int useCaseCode)
{
    LOG("GetMinimumFocusDistance begin.");
    ret_ = CAMERA_OK;
    for (int32_t i = 0; i < size_; i++) {
        float minimumFocusDistance;
        if (useCaseCode == CAMERA_OK) {
            ret_ = OH_CameraDevice_GetMinimumFocusDistance(&cameras_[i], &minimumFocusDistance);
            LOG("minimumFocusDistance ret:%{public}d.", minimumFocusDistance);
        } else if (useCaseCode == PARAMETER1_ERROR) {
            ret_ = OH_CameraDevice_GetMinimumFocusDistance(nullptr, &minimumFocusDistance);
        } else if (useCaseCode == PARAMETER2_ERROR) {
            ret_ = OH_CameraDevice_GetMinimumFocusDistance(&cameras_[i], nullptr);
        }
    }
    return ret_;
}

void ExposureStateCallback(void* context, OH_Camera_ExposureState exposureState)
{
    LOG("ExposureStateCallback is called.");
}
 
Camera_ErrorCode NDKCamera::RegisterExposureStateChangeCallback()
{
    LOG("RegisterExposureStateChangeCallback begin.");
    Camera_ErrorCode ret = OH_CaptureSession_RegisterExposureStateChangeCallback(captureSession_,
        nullptr, ExposureStateCallback);
    if (ret == CAMERA_OK) {
        LOG("RegisterExposureStateChangeCallback success.");
    } else {
        LOG("RegisterExposureStateChangeCallback failed, %d.", ret);
    }
    return ret;
}
 
Camera_ErrorCode NDKCamera::UnregisterExposureStateChangeCallback()
{
    LOG("RegisterExposureStateChangeCallback begin.");
    Camera_ErrorCode ret = OH_CaptureSession_UnregisterExposureStateChangeCallback(captureSession_,
        nullptr, ExposureStateCallback);
    if (ret == CAMERA_OK) {
        LOG("UnregisterExposureStateChangeCallback success.");
    } else {
        LOG("UnregisterExposureStateChangeCallback failed, %d.", ret);
    }
    return ret;
}
 
Camera_ErrorCode NDKCamera::RegisterExposureStateChangeCallbackWithoutSession()
{
    LOG("RegisterExposureStateChangeCallback begin.");
    Camera_ErrorCode ret = OH_CaptureSession_RegisterExposureStateChangeCallback(nullptr,
        nullptr, ExposureStateCallback);
...
Camera_ErrorCode NDKCamera::UnregisterExposureStateChangeCallbackWithoutSession()
{
    LOG("RegisterExposureStateChangeCallback begin.");
    Camera_ErrorCode ret = OH_CaptureSession_UnregisterExposureStateChangeCallback(nullptr,
        nullptr, ExposureStateCallback);
    if (ret == CAMERA_INVALID_ARGUMENT) {
        LOG("UnregisterExposureStateChangeCallback success.");
    } else {
...
bool g_isExposureStateCallback;
bool g_isExposureStart;
bool g_isExposureFinish;
void OnExposureStateCallback(void* context, OH_Camera_ExposureState state)
{
    LOG("Camera_ExposureState is called, state: %d", state);
    g_isExposureStateCallback = true;
    if (state == OH_CAMERA_EXPOSURE_STATE_SCAN) {
        LOG("Camera_ExposureState is called, state: OH_CAMERA_EXPOSURE_STATE_SCAN");
        g_isExposureStart = true;
    }
    if (state == OH_CAMERA_EXPOSURE_STATE_CONVERGED) {
        LOG("Camera_ExposureState is called, state: OH_CAMERA_EXPOSURE_STATE_CONVERGED");
        g_isExposureFinish = true;
    }
}

Camera_ErrorCode NDKCamera::RegisterExposureStateChangeCallback(int useCaseCode)
{
    g_isExposureStart = false;
    g_isExposureFinish = false;
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_RegisterExposureStateChangeCallback(captureSession_, nullptr, OnExposureStateCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_RegisterExposureStateChangeCallback(nullptr, nullptr, OnExposureStateCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_RegisterExposureStateChangeCallback(captureSession_, nullptr, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_RegisterExposureStateChangeCallback failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::UnregisterExposureStateChangeCallback(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_UnregisterExposureStateChangeCallback(captureSession_, nullptr,
            OnExposureStateCallback);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_UnregisterExposureStateChangeCallback(nullptr, nullptr, OnExposureStateCallback);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_UnregisterExposureStateChangeCallback(captureSession_, nullptr, nullptr);
    }
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_UnregisterExposureStateChangeCallback failed.");
    }
    return ret_;
}

bool NDKCamera::WaitForExposureFinish()
{
    const int waitExposureTimeout = 5;
    const int sleepInterval = 20;
    auto startTime = std::chrono::system_clock::now();
    while (!g_isExposureStateCallback) {
        std::this_thread::sleep_for(std::chrono::milliseconds(sleepInterval));
        auto endTime = std::chrono::system_clock::now();
        auto duration_seconds = std::chrono::duration_cast<std::chrono::seconds>(endTime - startTime).count();
        if (duration_seconds > waitExposureTimeout) {
            LOG("WaitForExposureFinish timeout.");
            break;
        }
    }
    return g_isExposureStateCallback;
}

int32_t g_minTint;
int32_t g_maxTint;
int32_t g_currentTint;
Camera_ErrorCode NDKCamera::GetColorTintRange()
{
    ret_ = OH_CaptureSession_GetColorTintRange(captureSession_, &g_minTint, &g_maxTint);
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_GetColorTintRange failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::SetColorTint(int useCaseCode)
{
    uint32_t colorTint;
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, g_minTint);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, g_maxTint);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        const int average = 2;
        colorTint = (g_minTint + g_maxTint) / average;
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, colorTint);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        colorTint = g_maxTint + 1;
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, colorTint);
    } else if (useCaseCode == PARAMETER4_ERROR) {
        ret_ = OH_CaptureSession_SetColorTint(nullptr, g_minTint);
    } else if (useCaseCode == PARAMETER5_ERROR) {
        uint32_t colorTintNull;
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, colorTintNull);
    }

    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_SetColorTint failed.");
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::GetColorTint(int useCaseCode)
{
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_GetColorTint(captureSession_, &g_currentTint);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetColorTintRange(captureSession_, &g_minTint, &g_maxTint);
        uint32_t colorTint = (g_minTint + g_maxTint) / 2;
        ret_ = OH_CaptureSession_SetColorTint(captureSession_, colorTint);
        ret_ = OH_CaptureSession_GetColorTint(captureSession_, &g_currentTint);
        if (colorTint != g_currentTint) {
            LOG("set: %d, get: %d", colorTint, g_currentTint);
            ret_ = static_cast<Camera_ErrorCode>(-1);
        };
    }

    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_GetColorTint failed.");
    }
    return ret_;
}

OH_Camera_ZoomPointInfo* cameraZoomPointInfo;
uint32_t g_zoomSize;
Camera_ErrorCode NDKCamera::GetZoomPointInfos(int useCaseCode)
{
    LOG("OH_CaptureSession_GetZoomPointInfos useCaseCode: %d", useCaseCode);
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_GetZoomPointInfos(captureSession_, &g_zoomSize, &cameraZoomPointInfo);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_GetZoomPointInfos(nullptr, &g_zoomSize, &cameraZoomPointInfo);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_GetZoomPointInfos(captureSession_, nullptr, &cameraZoomPointInfo);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        ret_ = OH_CaptureSession_GetZoomPointInfos(captureSession_, &g_zoomSize, nullptr);
    }

    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_GetZoomPointInfos failed.ret: %d", ret_);
    } else {
        LOG("OH_CaptureSession_GetZoomPointInfos success. ret: %d", ret_);
    }
    return ret_;
}

Camera_ErrorCode NDKCamera::DeleteZoomPointInfo(int useCaseCode)
{
    LOG("OH_CaptureSession_DeleteZoomPointInfos useCaseCode: %d", useCaseCode);
    if (useCaseCode == PARAMETER_OK) {
        ret_ = OH_CaptureSession_DeleteZoomPointInfos(captureSession_, cameraZoomPointInfo);
    } else if (useCaseCode == PARAMETER1_ERROR) {
        ret_ = OH_CaptureSession_DeleteZoomPointInfos(nullptr, cameraZoomPointInfo);
    } else if (useCaseCode == PARAMETER2_ERROR) {
        ret_ = OH_CaptureSession_DeleteZoomPointInfos(captureSession_, nullptr);
    } else if (useCaseCode == PARAMETER3_ERROR) {
        OH_Camera_ZoomPointInfo* testZoomPointInfo = new OH_Camera_ZoomPointInfo[1];
        const int EQUIVALENT_FOCAL_LENGTH_24MM = 24;
        testZoomPointInfo[0].equivalentFocalLength = EQUIVALENT_FOCAL_LENGTH_24MM;
        testZoomPointInfo[0].zoomRatio = 1;
        ret_ = OH_CaptureSession_DeleteZoomPointInfos(captureSession_, testZoomPointInfo);
    }
    
    if (ret_ != CAMERA_OK) {
        LOG("OH_CaptureSession_DeleteZoomPointInfos failed.ret: %d", ret_);
    } else {
        LOG("OH_CaptureSession_DeleteZoomPointInfos success. ret: %d", ret_);
    }
    return ret_;
}
