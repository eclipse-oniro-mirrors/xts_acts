...
#include "ohaudio/native_audio_session_base.h"
#include "hilog/log.h"
#include <ctime>
#include <set>
#include <algorithm>
...
#include "AudioResourceManagerTest.h"
...
const int32_t CHANNEL_LAYOUT = 3;
const int32_t ENCODING_TYPE = 0;
const int32_t STATE_TIME = 0;
const int32_t END_TIME = 20;
const int32_t NORMAL_RATE = 32000;
bool g_flag = false;
bool g_mark = false;
...
size_t g_fileOffset = 0;
bool randomTestEnable = true;
size_t latencySize = 0;
constexpr size_t g_smallUnit = 4;
constexpr size_t g_randomLowerBound = 1;

std::random_device rd;  // 真随机数源（硬件依赖）
std::mt19937 gen(rd()); // 使用默认种子
// 2. 分布器：指定分布区间
...
void MyAudioCapturerOnFastStatusChangeCallback(OH_AudioCapturer* capturer, void* userData,
    OH_AudioStream_FastStatus status)
{
    (void)userData;
    LOG("MyAudioCapturerOnFastStatusChangeCallback, status is %{public}d", status);
}

void MyAudioRendererOnFastStatusChangeCallback(OH_AudioRenderer* renderer, void* userData,
    OH_AudioStream_FastStatus status)
{
    (void)userData;
    LOG("MyAudioRendererOnFastStatusChangeCallback, status is %{public}d", status);
}

void MyPlaybackCapturerStartCallback(OH_AudioCapturer* capturer, void* userData,
    OH_AudioStream_PlaybackCaptureStartState state)
{
    (void)userData;
    LOG("MyPlaybackCapturerStartCallback, state is %{public}d", state);
}
...
static napi_value AudioWorkGroup(napi_env env, napi_callback_info info)
{
    int32_t tid;
    OH_AudioResourceManager *mgr;
    OH_AudioWorkgroup *grp;
    OH_AudioManager_GetAudioResourceManager(&mgr);
    OH_AudioResourceManager_CreateWorkgroup(mgr, "new", &grp);
    OH_AudioWorkgroup_AddCurrentThread(grp, &tid);
    OH_AudioWorkgroup_Start(grp, STATE_TIME, END_TIME);
    OH_AudioWorkgroup_Stop(grp);
    OH_AudioWorkgroup_RemoveThread(grp, tid);
    OH_AudioCommon_Result result=OH_AudioResourceManager_ReleaseWorkgroup(mgr, grp);
    napi_value res;
    napi_create_int32(env, result, &res);
    return res;
}

static napi_value AudioWorkGroup_001(napi_env env, napi_callback_info info)
{
    OH_AudioResourceManager *mgr;
    OH_AudioWorkgroup *grp;
    OH_AudioManager_GetAudioResourceManager(&mgr);
    OH_AudioCommon_Result result = OH_AudioResourceManager_CreateWorkgroup(mgr, nullptr, &grp);
    napi_value res;
...
static napi_value AudioStreamBuilderSetLatencyMode_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder* builder;
    OH_AudioStream_Type type = AUDIOSTREAM_TYPE_CAPTURER;
    OH_AudioStreamBuilder_Create(&builder, type);
...
static napi_value AudioStreamBuilderSetCapturerInfo_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder* builder;
    OH_AudioStream_Type type = AUDIOSTREAM_TYPE_CAPTURER;
    OH_AudioStreamBuilder_Create(&builder, type);
    OH_AudioStream_SourceType sourceType = AUDIOSTREAM_SOURCE_TYPE_MIC;
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerInfo(nullptr, sourceType);
    OH_AudioStreamBuilder_Destroy(builder);
    napi_value res;
...
static napi_value AudioCreateEngine(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSuiteEngine *audioSuiteEngine = nullptr;
    OH_AudioSuite_Result result1 = OH_AudioSuiteEngine_Create(&audioSuiteEngine);
    LOG("AudioCreateEngine, result %{public}d", result1);
    if (result1 != AUDIOSUITE_SUCCESS) {
...
void WaitForFlagWithTimeout(bool* flag, int32_t maxSeconds) {
    int32_t elapsed = 0;
    while (elapsed < maxSeconds * 1000) {
        if (*flag) {
            LOG("WaitForFlagWithTimeout return after: %{public}d", elapsed);
            return;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        elapsed += 1;
    }
    LOG("WaitForFlagWithTimeout timeout after: %{public}d", maxSeconds);
}
...
static napi_value AudioRendererSetOnMarkReached_08(napi_env env, napi_callback_info info)
{
    ...
    WaitForFlagWithTimeout(&g_flag, waitTime);
    OH_AudioRenderer_Stop(audioRenderer);
...
static napi_value AudioAudioInternalRecordingSuccess03(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioStream_PrivacyType type = AUDIO_STREAM_PRIVACY_TYPE_SHARED;
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetRendererPrivacy(builder, type);
    OH_AudioRenderer *audioRenderer = nullptr;
    OH_AudioStreamBuilder_GenerateRenderer(builder, &audioRenderer);
    OH_AudioStream_PrivacyType type_get;
    OH_AudioStream_Result result1 = OH_AudioRenderer_GetRendererPrivacy(audioRenderer, &type_get);
    if (result == AUDIOSTREAM_SUCCESS && result1 == AUDIOSTREAM_SUCCESS
        && type_get == AUDIO_STREAM_PRIVACY_TYPE_SHARED) {
        result = AUDIOSTREAM_SUCCESS;
    } else {
        result = AUDIOSTREAM_ERROR_INVALID_PARAM;
    };
...
static napi_value AudioAudioInternalRecordingFalse(napi_env env, napi_callback_info info)
{
    ...
    OH_AudioRenderer *audioRenderer;
    OH_AudioStreamBuilder_GenerateRenderer(builder, &audioRenderer);
    OH_AudioStream_PrivacyType type = static_cast<OH_AudioStream_PrivacyType>(3);
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetRendererPrivacy(builder, type);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    napi_value res;
...
static void MyStateCallback(OH_AudioSession_StateChangedEvent event)
{
    return;
}

static void MyDeviceCallback(OH_AudioDeviceDescriptorArray *devices,
...
static napi_value AudioSessionManagerSetScene_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager;
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("get audio session manager fail, result is: %{public}d", TEST_FAIL);
        return res;
    }

    OH_AudioSession_Scene scene = AUDIO_SESSION_SCENE_MEDIA;
    result = OH_AudioSessionManager_SetScene(audioSessionManager, scene);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetScene fail AUDIO_SESSION_SCENE_MEDIA, result is: %{public}d", TEST_FAIL);
        return res;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetScene fail AUDIO_SESSION_SCENE_GAME, result is: %{public}d", TEST_FAIL);
        return res;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetScene fail AUDIO_SESSION_SCENE_VOICE_COMMUNICATION, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerRegisterStateChangeCallback_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_RegisterStateChangeCallback(audioSessionManager, MyStateCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("RegisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    result = OH_AudioSessionManager_UnregisterStateChangeCallback(audioSessionManager, MyStateCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("UnregisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerRegisterStateChangeCallback_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    ...
    result = OH_AudioSessionManager_RegisterStateChangeCallback(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("RegisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
    ...
    result = OH_AudioSessionManager_UnregisterStateChangeCallback(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("UnregisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
...
static napi_value AudioSessionManagerRegisterStateChangeCallback_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("RegisterStateChangeCallback double fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("UnregisterStateChangeCallback double fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerSetDefaultOutputDevice_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_GetDefaultOutputDevice(audioSessionManager, &deviceTypeGet);
    if (result != AUDIOCOMMON_RESULT_SUCCESS || deviceTypeGet != AUDIO_DEVICE_TYPE_INVALID) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("GetDefaultOutputDevice fail, result is: %{public}d", TEST_FAIL);
        return res;
    }

    OH_AudioDevice_Type deviceType = AUDIO_DEVICE_TYPE_DEFAULT;
    result = OH_AudioSessionManager_SetDefaultOutputDevice(audioSessionManager, deviceType);
    if ((result != AUDIOCOMMON_RESULT_SUCCESS) && (result != AUDIOCOMMON_RESULT_ERROR_SYSTEM)) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetDefaultOutputDevice AUDIO_DEVICE_TYPE_DEFAULT fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
...
static napi_value AudioSessionManagerSetDefaultOutputDevice_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetDefaultOutputDevice fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerGetDefaultOutputDevice_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_GetDefaultOutputDevice(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("GetDefaultOutputDevice fail, result is: %{public}d", TEST_FAIL);
        return res;
...
static napi_value AudioSessionManagerRegisterDeviceCallback_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_RegisterCurrentOutputDeviceChangeCallback(audioSessionManager, MyDeviceCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("RegisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    result = OH_AudioSessionManager_UnregisterCurrentOutputDeviceChangeCallback(audioSessionManager, MyDeviceCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("UnregisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
...
static napi_value AudioSessionManagerRegisterDeviceCallback_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_RegisterCurrentOutputDeviceChangeCallback(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("RegisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
    ...
    result = OH_AudioSessionManager_UnregisterCurrentOutputDeviceChangeCallback(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("UnregisterStateChangeCallback fail, result is: %{public}d", TEST_FAIL);
        return res;
...
static napi_value AudioSessionManagerRegisterDeviceCallback_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
...
static napi_value AudioSessionManagerReleaseDevices_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("get audio route manager fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS || array == nullptr) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioRoutingManager_GetDevices fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSessionManager_ReleaseDevices fail, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerReleaseDevices_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSessionManager_ReleaseDevices fail, manager nullptr, result is: %{public}d", TEST_FAIL);
        return res;
    }

    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSessionManager_ReleaseDevices fail, devices nullptr, result is: %{public}d", TEST_FAIL);
        return res;
    }
    OH_AudioDeviceDescriptorArray audioDeviceDescriptorArray;
    audioDeviceDescriptorArray.descriptors = nullptr;

    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, &audioDeviceDescriptorArray);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSessionManager_ReleaseDevices fail, descriptors nullptr, result is: %{public}d", TEST_FAIL);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioCaptureGetFastStatus_001(napi_env env, napi_callback_info info)
{
    OH_AudioCapturer *audioCapturer = nullptr;
    OH_AudioStream_FastStatus fastStatus;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioCapturer_GetFastStatus(audioCapturer, &fastStatus);
    if (res != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("OH_AudioCapturer_GetFastStatus FAILED, res is %{public}d", res);
        OH_AudioCapturer_Release(audioCapturer);
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
...
static napi_value AudioCaptureGetFastStatus_002(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    napi_value result;
    OH_AudioCapturer *audioCapturer;
    OH_AudioStream_FastStatus fastStatus;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);

    res = OH_AudioCapturer_GetFastStatus(audioCapturer, &fastStatus);
    if (res != AUDIOSTREAM_SUCCESS) {
        LOG("OH_AudioCapturer_GetFastStatus FAILED, res is %{public}d", res);
        OH_AudioCapturer_Release(audioCapturer);
        OH_AudioStreamBuilder_Destroy(builder);
...
static napi_value AudioRendererGetFastStatus_001(napi_env env, napi_callback_info info)
{
    OH_AudioRenderer *audioRenderer = nullptr;
    OH_AudioStream_FastStatus fastStatus;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioRenderer_GetFastStatus(audioRenderer, &fastStatus);
    if (res != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("OH_AudioRenderer_GetFastStatus FAILED, res is %{public}d", res);
        OH_AudioRenderer_Release(audioRenderer);
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
...
static napi_value AudioRendererGetFastStatus_002(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    napi_value result;
    OH_AudioRenderer *audioRenderer;
    OH_AudioStream_FastStatus fastStatus;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_GenerateRenderer(builder, &audioRenderer);

    res = OH_AudioRenderer_GetFastStatus(audioRenderer, &fastStatus);
    if (res != AUDIOSTREAM_SUCCESS) {
        LOG("OH_AudioRenderer_GetFastStatus FAILED, res is %{public}d", res);
        OH_AudioRenderer_Release(audioRenderer);
        OH_AudioStreamBuilder_Destroy(builder);
...
static napi_value AudioStreamManagerIsFastPlaybackSupported_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamManager *streamManager;
    OH_AudioStreamInfo streamInfo;
    napi_value result;
    OH_AudioCommon_Result res = OH_AudioManager_GetAudioStreamManager(&streamManager);
    if (res != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioManager_GetAudioStreamManager FAILED, res is %{public}d", res);
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }

    streamInfo.samplingRate = NORMAL_RATE;
    streamInfo.channelLayout = (OH_AudioChannelLayout)CHANNEL_LAYOUT;
    streamInfo.encodingType = (OH_AudioStream_EncodingType)ENCODING_TYPE;
    streamInfo.sampleFormat = (OH_AudioStream_SampleFormat)g_sampleFormat;

    bool isFastStatus = false;
    for (auto usage : VALID_OH_STREAM_USAGES) {
        isFastStatus = OH_AudioStreamManager_IsFastPlaybackSupported(streamManager, &streamInfo, usage);
        if (isFastStatus != false && isFastStatus != true) {
            LOG("AudioStreamManagerIsFastPlaybackSupported_001:FAIL, isFastStatus is: %{public}d", isFastStatus);
            napi_create_int32(env, TEST_FAIL, &result);
            return result;
        }
    }
...
static napi_value AudioStreamManagerIsFastRecordingSupported_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamManager *streamManager;
    OH_AudioStreamInfo streamInfo;
    napi_value result;
    ...
    bool isFastStatus = false;
    for (auto source : VALID_OH_SOURCE_TYPES) {
        isFastStatus = OH_AudioStreamManager_IsFastRecordingSupported(streamManager, &streamInfo, source);
        if (isFastStatus != false && isFastStatus != true) {
            LOG("AudioStreamManagerIsFastRecordingSupported_001:FAIL, isFastStatus is: %{public}d", isFastStatus);
            napi_create_int32(env, TEST_FAIL, &result);
            return result;
        }
    }
...
static napi_value AudioStreamManagerIsMultichannelPlaybackSupported_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamManager *streamManager;
    OH_AudioStreamInfo streamInfo;
    napi_value result;
    ...
    streamInfo.samplingRate = g_samplingRate;
    streamInfo.channelLayout = OH_AudioChannelLayout::CH_LAYOUT_2POINT1;
    streamInfo.encodingType = OH_AudioStream_EncodingType::AUDIOSTREAM_ENCODING_TYPE_RAW;
    streamInfo.sampleFormat = OH_AudioStream_SampleFormat::AUDIOSTREAM_SAMPLE_S32LE;

    bool isMultichannel = false;
    isMultichannel = OH_AudioStreamManager_IsMultichannelPlaybackSupported(streamManager, &streamInfo,
                                                                           AUDIOSTREAM_USAGE_MUSIC);
    if (isMultichannel) {
        napi_create_int32(env, TEST_PASS, &result);
        return result;
    ...
    LOG("IsMultichannelPlaybackSupported_001: FAIL, isMultichannel is: %{public}d", isMultichannel);
    napi_create_int32(env, TEST_FAIL, &result);
    return result;
}

static napi_value AudioStreamManagerIsDirectPlaybackSupported_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamManager *streamManager;
    OH_AudioStreamInfo streamInfo;
    napi_value result;
    ...
    isDirectPlay = OH_AudioStreamManager_IsDirectPlaybackSupported(streamManager, &streamInfo, AUDIOSTREAM_USAGE_MUSIC);
    if (isDirectPlay) {
        napi_create_int32(env, TEST_PASS, &result);
        return result;
    ...
    LOG("IsDirectPlaybackSupported: FAIL, isDirectPlay is: %{public}d", isDirectPlay);
    napi_create_int32(env, TEST_FAIL, &result);
    return result;
}

static napi_value AudioStreamManagerIsOffloadPlaybackSupported_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamManager *streamManager;
    OH_AudioStreamInfo streamInfo;
    napi_value result;
    ...
    isOffload = OH_AudioStreamManager_IsOffloadPlaybackSupported(streamManager, &streamInfo, AUDIOSTREAM_USAGE_MUSIC);
    if (isOffload) {
        napi_create_int32(env, TEST_PASS, &result);
        return result;
    } else if (!isOffload) {
    ...
    LOG("IsOffloadPlaybackSupported: FAIL, isOffload is: %{public}d", isOffload);
    napi_create_int32(env, TEST_FAIL, &result);
    return result;
}

static napi_value AudioStreamBuilderSetCapturerFastStatusChangeCallback_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = nullptr;
    void *userData = nullptr;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_SetCapturerFastStatusChangeCallback(builder, nullptr, userData);
    if (res != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("OH_AudioStreamBuilder_SetCapturerFastStatusChangeCallback FAILED, res is %{public}d", res);
        OH_AudioStreamBuilder_Destroy(builder);
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
...
static napi_value AudioStreamBuilderSetCapturerFastStatusChangeCallback_002(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    void *userData = nullptr;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_SetCapturerFastStatusChangeCallback(builder,
        MyAudioCapturerOnFastStatusChangeCallback, userData);
    if (res != AUDIOSTREAM_SUCCESS) {
        LOG("OH_AudioStreamBuilder_SetCapturerFastStatusChangeCallback FAILED, res is %{public}d", res);
        OH_AudioStreamBuilder_Destroy(builder);
        napi_create_int32(env, TEST_FAIL, &result);
...
static napi_value AudioStreamBuilderSetRendererFastStatusChangeCallback_001(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = nullptr;
    void *userData = nullptr;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_SetRendererFastStatusChangeCallback(builder, nullptr, userData);
    if (res != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("OH_AudioStreamBuilder_SetRendererFastStatusChangeCallback FAILED, res is %{public}d", res);
        OH_AudioStreamBuilder_Destroy(builder);
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
...
static napi_value AudioStreamBuilderSetRendererFastStatusChangeCallback_002(napi_env env, napi_callback_info info)
{
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    void *userData = nullptr;
    napi_value result;
    OH_AudioStream_Result res = OH_AudioStreamBuilder_SetRendererFastStatusChangeCallback(builder,
        MyAudioRendererOnFastStatusChangeCallback, userData);
    if (res != AUDIOSTREAM_SUCCESS) {
        LOG("OH_AudioStreamBuilder_SetRendererFastStatusChangeCallback FAILED, res is %{public}d", res);
        OH_AudioStreamBuilder_Destroy(builder);
        napi_create_int32(env, TEST_FAIL, &result);
...
static int32_t AudioRendererOnWriteDataEnhance(
...
{
    if (latencySize == 0) {
        latencySize = audioDataSize;
        dist = std::uniform_int_distribution<>(g_randomLowerBound, latencySize / g_smallUnit);
    }
    if (randomTestEnable && g_fileOffset != FileRecourceManager::len) {
        int random_int = dist(gen);
        LOG("random_int SUCCESS, result is: %{public}d", random_int);
        audioDataSize = static_cast<int32_t>(random_int * g_smallUnit);
    }
    // 将待播放的数据，按audioDataSize长度写入audioData。
    // 如果开发者不希望播放某段audioData，返回AUDIO_DATA_CALLBACK_RESULT_INVALID即可。
    if (g_fileOffset + audioDataSize < FileRecourceManager::len) {
        std::copy(FileRecourceManager::dataCopy.get() + g_fileOffset,
                  FileRecourceManager::dataCopy.get() + g_fileOffset + audioDataSize,
                  static_cast<uint8_t*>(audioData));
        g_fileOffset += audioDataSize;
        LOG("AudioRendererOnWriteData SUCCESS, result is: %{public}d, res is %{public}d",
             audioDataSize, *(FileRecourceManager::dataCopy.get() + g_fileOffset));
        return audioDataSize;
    }
    else {
        if (g_fileOffset == FileRecourceManager::len) { return 0; }
        std::copy(FileRecourceManager::dataCopy.get() + g_fileOffset,
                  FileRecourceManager::dataCopy.get() + g_fileOffset + (FileRecourceManager::len - g_fileOffset),
                  static_cast<uint8_t*>(audioData));
        g_fileOffset = FileRecourceManager::len;
        LOG("AudioRendererOnWriteData SUCCESS, result is: %{public}d, res is %{public}d",
             FileRecourceManager::len - g_fileOffset, *(FileRecourceManager::dataCopy.get() + g_fileOffset));
        return FileRecourceManager::len - g_fileOffset;
    }
}
    ...
    OH_AudioStream_Result error)
{
    ...
    LOG("OnError FAILED, result is: %{public}d", static_cast<int32_t>(error));
    return 0;
}

static napi_value StopAndReleaseRenderer(napi_env env, napi_callback_info info)
{
    OH_AudioRenderer_Pause(audioRenderer);
    OH_AudioRenderer_Stop(audioRenderer);
    LOG("OH_AudioRenderer_Pause and OH_AudioRenderer_Stop SUCCESS");
    if (audioRenderer) {
        OH_AudioRenderer_Release(audioRenderer);
        OH_AudioStreamBuilder_Destroy(builder);
        audioRenderer = nullptr;
        builder = nullptr;
        LOG("OH_AudioRenderer_Release and OH_AudioStreamBuilder_Destroy SUCCESS");
    }
    napi_value result;
    napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_FAIL), &result);
    return result;
}

static napi_value AudioLowLatencyOut(napi_env env, napi_callback_info info)
{
    napi_value result;

    constexpr float volume = 0.5f;
    constexpr int32_t streamRate = 48000;
    OH_AudioStream_Type streamType = OH_AudioStream_Type::AUDIOSTREAM_TYPE_RENDERER;

    OH_AudioStream_Result BuilderCreateResult = OH_AudioStreamBuilder_Create(&builder, streamType);
    if (BuilderCreateResult != OH_AudioStream_Result::AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_FAIL), &result);
        LOG("OH_AudioStreamBuilder_Create FAILED, result is: %{public}d", result);
        return result;
    }
    LOG("OH_AudioStreamBuilder_Create SUCCESS");

    // 设置音频采样率。
    OH_AudioStreamBuilder_SetSamplingRate(builder, streamRate);
    // 设置音频声道。
    OH_AudioStreamBuilder_SetChannelLayout(builder, CH_LAYOUT_STEREO);
    // 设置音频采样格式。
    OH_AudioStreamBuilder_SetSampleFormat(builder, OH_AudioStream_SampleFormat::AUDIOSTREAM_SAMPLE_S16LE);
    // 设置音频流的编码类型。
    ...
    rendererCallBacks.OH_AudioRenderer_OnStreamEvent = OnStreamEvent;
    rendererCallBacks.OH_AudioRenderer_OnInterruptEvent = OnInterruptEvent;
    rendererCallBacks.OH_AudioRenderer_OnError = OnError;
    rendererCallBacks.OH_AudioRenderer_OnWriteData = nullptr;

    OH_AudioStreamBuilder_SetRendererCallback(builder, rendererCallBacks, nullptr);

    OH_AudioRenderer_OnWriteDataCallbackAdvanced writeDataCb = AudioRendererOnWriteDataEnhance;
    OH_AudioStreamBuilder_SetRendererWriteDataCallbackAdvanced(builder, writeDataCb, nullptr);

    OH_AudioStream_Result GenerateRendererResult = OH_AudioStreamBuilder_GenerateRenderer(builder, &audioRenderer);
    if (GenerateRendererResult != OH_AudioStream_Result::AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_FAIL), &result);
        LOG("OH_AudioStreamBuilder_GenerateRenderer FAILED, result is: %{public}d", result);
        return result;
    }
    LOG("OH_AudioStreamBuilder_GenerateRenderer SUCCESS");

    // 设置音频流音量
    OH_AudioStream_Result SetVolumeResult = OH_AudioRenderer_SetVolume(audioRenderer, volume);
    if (SetVolumeResult != OH_AudioStream_Result::AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_FAIL), &result);
        LOG("OH_AudioRenderer_SetVolume FAILED, result is: %{public}d", result);
        return result;
    }
    LOG("OH_AudioRenderer_SetVolume SUCCESS");

    g_fileOffset = 0;

    LOG("FileRecourceManager::len SUCCESS: %{public}d", FileRecourceManager::len);
    LOG("FileRecourceManager::data SUCCESS: %{public}p", FileRecourceManager::dataCopy.get());

    napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_PASS), &result);
    return result;
}

static napi_value AudioWorkGroupSet(napi_env env, napi_callback_info info)
{
    napi_value result;
    ...
    OH_AudioResourceManager *mgr;
    OH_AudioWorkgroup *grp;
    OH_AudioCommon_Result mgrresult = OH_AudioManager_GetAudioResourceManager(&mgr);
    LOG("OH_AudioManager_GetAudioResourceManager result is: %{public}d", mgrresult);

    OH_AudioCommon_Result cwgresult = OH_AudioResourceManager_CreateWorkgroup(mgr, "new", &grp);
    LOG("OH_AudioResourceManager_CreateWorkgroup result is: %{public}d", cwgresult);

    OH_AudioCommon_Result addtrdresult = OH_AudioWorkgroup_AddCurrentThread(grp, &tid);
    LOG("OH_AudioWorkgroup_AddCurrentThread result is: %{public}d", addtrdresult);

    OH_AudioCommon_Result startresult = OH_AudioWorkgroup_Start(grp, STATE_TIME, END_TIME);
    LOG("OH_AudioWorkgroup_Start result is: %{public}d", startresult);

    OH_AudioCommon_Result stopresult = OH_AudioWorkgroup_Stop(grp);
    LOG("OH_AudioWorkgroup_Stop result is: %{public}d", stopresult);

    OH_AudioCommon_Result rmresult = OH_AudioWorkgroup_RemoveThread(grp, tid);
    LOG("OH_AudioWorkgroup_RemoveThread result is: %{public}d", rmresult);

    OH_AudioCommon_Result rsresult=OH_AudioResourceManager_ReleaseWorkgroup(mgr, grp);
    LOG("OH_AudioResourceManager_ReleaseWorkgroup result is: %{public}d", rsresult);
...
{
    OH_AudioRenderer_Start(audioRenderer);
    LOG("OH_AudioRenderer_Start SUCCESS");
    napi_value result;
    napi_create_int32(env, static_cast<int32_t>(TEST_RESULT::TEST_PASS), &result);
    return result;
}

static void AvailableDeviceChangedCallback(OH_AudioDevice_ChangeType type,
    OH_AudioDeviceDescriptorArray *audioDeviceDescriptorArray)
{
    return;
}
static void CurrentInputDeviceChangedCallback(OH_AudioDeviceDescriptorArray *devices,
    OH_AudioStream_DeviceChangeReason changeReason)
{
    return;
}

static napi_value IsIntelligentNoiseReductionEnabledForCurrentDevice_001(napi_env env, napi_callback_info info)
{
    napi_value result;
    OH_AudioStreamManager *streamManager;
    bool isIntelligentNoise = false;
    for (auto source : VALID_OH_SOURCE_TYPES) {
        isIntelligentNoise = OH_AudioStreamManager_IsIntelligentNoiseReductionEnabledForCurrentDevice(
            streamManager, source);
        if (isIntelligentNoise != false && isIntelligentNoise != true) {
            LOG("FAIL, isIntelligentNoise is: %{public}d", isIntelligentNoise);
            napi_create_int32(env, TEST_FAIL, &result);
            return result;
        }
    }
...
static napi_value AudioSessionManagerAvailableDevices_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioManager_GetAudioSessionManager fail, result is: %{public}d", result);
        return res;
    }
    OH_AudioDevice_Usage deviceUsage = AUDIO_DEVICE_USAGE_MEDIA_OUTPUT;
    OH_AudioDeviceDescriptorArray *array;
    result = OH_AudioSessionManager_GetAvailableDevices(audioSessionManager, deviceUsage, &array);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_GetAvailableDevices fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_RegisterAvailableDevicesChangeCallback(
        audioSessionManager, deviceUsage, AvailableDeviceChangedCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_RegisterAvailableDevicesChangeCallback fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_UnregisterAvailableDevicesChangeCallback(
        audioSessionManager, AvailableDeviceChangedCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_UnregisterAvailableDevicesChangeCallback fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, array);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_ReleaseDevices fail, result is: %{public}d", result);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerAvailableDevices_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_SelectMediaInputDevice(audioSessionManager, nullptr);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_SelectMediaInputDevice fail, result is: %{public}d", result);
        return res;
    }
    OH_AudioDeviceDescriptor *audioDeviceDescriptor = nullptr;
    result = OH_AudioSessionManager_GetSelectedMediaInputDevice(audioSessionManager, &audioDeviceDescriptor);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_SelectMediaInputDevice fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_ReleaseDevice(audioSessionManager, audioDeviceDescriptor);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_ReleaseDevice fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, array);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
...
static napi_value AudioSessionManagerAvailableDevices_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory category = PREFERRED_NONE;
    result = OH_AudioSessionManager_SetBluetoothAndNearlinkPreferredRecordCategory(audioSessionManager, category);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("SetBluetoothAndNearlinkPreferredRecordCategory fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_GetBluetoothAndNearlinkPreferredRecordCategory(audioSessionManager, &category);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("GetBluetoothAndNearlinkPreferredRecordCategory fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, array);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
...
static napi_value AudioSessionManagerAvailableDevices_004(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    ...
    result = OH_AudioSessionManager_RegisterCurrentInputDeviceChangeCallback(
        audioSessionManager, CurrentInputDeviceChangedCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_RegisterCurrentInputDeviceChangeCallback fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_UnregisterCurrentInputDeviceChangeCallback(
        audioSessionManager, CurrentInputDeviceChangedCallback);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("OH_AudioSessionManager_UnregisterCurrentInputDeviceChangeCallback fail, result is: %{public}d", result);
        return res;
    }
    result = OH_AudioSessionManager_ReleaseDevices(audioSessionManager, array);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
...
static napi_value AudioSessionManagerAvailableDevices_005(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioDeviceDescriptor *audioDeviceDescriptor = nullptr;
    OH_AudioCommon_Result result = OH_AudioSessionManager_ReleaseDevice(nullptr, audioDeviceDescriptor);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_006(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    ...
    OH_AudioDevice_Usage deviceUsage = AUDIO_DEVICE_USAGE_MEDIA_OUTPUT;
    OH_AudioDeviceDescriptorArray *array;
    OH_AudioCommon_Result result = OH_AudioSessionManager_GetAvailableDevices(nullptr, deviceUsage, &array);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_007(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioDevice_Usage deviceUsage = AUDIO_DEVICE_USAGE_MEDIA_OUTPUT;
    OH_AudioCommon_Result result = OH_AudioSessionManager_RegisterAvailableDevicesChangeCallback(
        nullptr, deviceUsage, AvailableDeviceChangedCallback);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_008(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result = OH_AudioSessionManager_UnregisterAvailableDevicesChangeCallback(
        nullptr, AvailableDeviceChangedCallback);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_009(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result = OH_AudioSessionManager_SelectMediaInputDevice(nullptr, nullptr);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_010(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioDeviceDescriptor *audioDeviceDescriptor = nullptr;
    OH_AudioCommon_Result result = OH_AudioSessionManager_GetSelectedMediaInputDevice(nullptr, &audioDeviceDescriptor);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("OH_AudioSessionManager_GetSelectedMediaInputDevice fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManagerAvailableDevices_011(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory category = PREFERRED_NONE;
    OH_AudioCommon_Result result = OH_AudioSessionManager_SetBluetoothAndNearlinkPreferredRecordCategory(
        nullptr, category);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("OH_AudioSessionManager_SetBluetoothAndNearlinkPreferredRecordCategory fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManagerAvailableDevices_012(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory category = PREFERRED_NONE;
    OH_AudioSessionManager_SetBluetoothAndNearlinkPreferredRecordCategory(audioSessionManager, category);
    OH_AudioCommon_Result result = OH_AudioSessionManager_GetBluetoothAndNearlinkPreferredRecordCategory(
        nullptr, &category);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("OH_AudioSessionManager_GetBluetoothAndNearlinkPreferredRecordCategory fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManagerAvailableDevices_013(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result = OH_AudioSessionManager_RegisterCurrentInputDeviceChangeCallback(
        nullptr, CurrentInputDeviceChangedCallback);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
...
static napi_value AudioSessionManagerAvailableDevices_014(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result = OH_AudioSessionManager_UnregisterCurrentInputDeviceChangeCallback(
        nullptr, CurrentInputDeviceChangedCallback);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
...
const int32_t BLUETOOTH_NONE = 0;
const int32_t BLUETOOTH_DEFAULT = 1;
const int32_t BLUETOOTH_LATENCY = 2;
const int32_t BLUETOOTH_QUALITY = 3;
const int32_t ERROR_UNSUPPORTED_FORMAT = 4;
const int32_t STREAM_STATE_STOPPED = 3;
const int32_t STREAM_PRIVACY_TYPE_SHARED = 2;
static napi_value BluetoothAndNearlinkPreferredRecordCategory_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory preferredNone = PREFERRED_NONE;
    if (preferredNone != BLUETOOTH_NONE) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("PREFERRED_NONE fail, result is: %{public}d", preferredNone);
        return res;
    }
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory preferredDefault = PREFERRED_DEFAULT;
    if (preferredDefault != BLUETOOTH_DEFAULT) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("PREFERRED_DEFAULT fail, result is: %{public}d", preferredDefault);
        return res;
    }
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory preferredLowLatency = PREFERRED_LOW_LATENCY;
    if (preferredLowLatency != BLUETOOTH_LATENCY) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("PREFERRED_LOW_LATENCY fail, result is: %{public}d", preferredLowLatency);
        return res;
    }
    OH_AudioSession_BluetoothAndNearlinkPreferredRecordCategory preferredHighQuality = PREFERRED_HIGH_QUALITY;
    if (preferredHighQuality != BLUETOOTH_QUALITY) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("PREFERRED_HIGH_QUALITY fail, result is: %{public}d", preferredHighQuality);
        return res;
    }
    OH_AudioStream_Result unsupportedFormat = AUDIOSTREAM_ERROR_UNSUPPORTED_FORMAT;
    if (unsupportedFormat != ERROR_UNSUPPORTED_FORMAT) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_ERROR_UNSUPPORTED_FORMAT fail, result is: %{public}d", unsupportedFormat);
        return res;
    }
    OH_AudioStream_State stateStopped = AUDIOSTREAM_STATE_STOPPED;
    if (stateStopped != STREAM_STATE_STOPPED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_STATE_STOPPED fail, result is: %{public}d", stateStopped);
        return res;
    }
    OH_AudioStream_PrivacyType typeShared = AUDIO_STREAM_PRIVACY_TYPE_SHARED;
    if (typeShared != STREAM_PRIVACY_TYPE_SHARED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIO_STREAM_PRIVACY_TYPE_SHARED fail, result is: %{public}d", typeShared);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioCommonResult_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    int32_t streamErrorSystem = 3;
    int32_t errorUnsupported = 6800104;
    int32_t errorTimeout = 6800105;
    int32_t errorLimit = 6800201;
    if (streamErrorSystem != AUDIOSTREAM_ERROR_SYSTEM) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_ERROR_SYSTEM fail, result is: %{public}d", AUDIOSTREAM_ERROR_SYSTEM);
        return res;
    }
    if (errorUnsupported != AUDIOCOMMON_RESULT_ERROR_UNSUPPORTED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCOMMON_RESULT_ERROR_UNSUPPORTED fail, result is: %{public}d", AUDIOCOMMON_RESULT_ERROR_UNSUPPORTED);
        return res;
    }
    if (errorTimeout != AUDIOCOMMON_RESULT_ERROR_TIMEOUT) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCOMMON_RESULT_ERROR_TIMEOUT fail, result is: %{public}d", AUDIOCOMMON_RESULT_ERROR_TIMEOUT);
        return res;
    }
    if (errorLimit != AUDIOCOMMON_RESULT_ERROR_STREAM_LIMIT) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCOMMON_RESULT_ERROR_STREAM_LIMIT fail, result is: %{public}d", AUDIOCOMMON_RESULT_ERROR_STREAM_LIMIT);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateRenderBuilder();
    int32_t samplingRate = 48000;
    ...
    int32_t channelCount = 2;
    OH_AudioStreamBuilder_SetChannelCount(builder, channelCount);
    OH_AudioStream_Usage usage = OH_AudioStream_Usage::AUDIOSTREAM_USAGE_MOVIE;
    OH_AudioStreamBuilder_SetRendererInfo(builder, usage);

    OH_AudioRenderer_Callbacks rendererCallBacks;
    OH_AudioRenderer_Callbacks rendererCallbacks = {0};
    ...
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(audioRenderer, AUDIOSTREAM_LATENCY_TYPE_HARDWARE, &latency);
    OH_AudioRenderer_Stop(audioRenderer);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0100 pass, result is: %{public}d", AUDIOSTREAM_SUCCESS);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0200(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateRenderBuilder();
    int32_t samplingRate = 48000;
    ...
    int32_t latency = -1;
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(audioRenderer, AUDIOSTREAM_LATENCY_TYPE_ALL, &latency);
    OH_AudioRenderer_Stop(audioRenderer);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0200 pass, result is: %{public}d", AUDIOSTREAM_SUCCESS);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0300(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateRenderBuilder();
    int32_t samplingRate = 48000;
    ...
    int32_t latency = -1;
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(audioRenderer, AUDIOSTREAM_LATENCY_TYPE_SOFTWARE, &latency);
    OH_AudioRenderer_Stop(audioRenderer);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0300 pass, result is: %{public}d", AUDIOSTREAM_SUCCESS);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0400(napi_env env, napi_callback_info info)
{
    napi_value res;
    int32_t latency = -1;
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(nullptr, AUDIOSTREAM_LATENCY_TYPE_ALL, &latency);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0400 pass, result is: %{public}d", AUDIOSTREAM_ERROR_INVALID_PARAM);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0500(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    int32_t samplingRate = 48000;
    ...
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(audioRenderer, AUDIOSTREAM_LATENCY_TYPE_SOFTWARE, nullptr);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0500 pass, result is: %{public}d", AUDIOSTREAM_ERROR_INVALID_PARAM);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0600(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    int32_t samplingRate = 48000;
    ...
    int32_t latency = 0;
    OH_AudioStream_LatencyType invaildType = static_cast<OH_AudioStream_LatencyType>(3);
    OH_AudioStream_Result result =
        OH_AudioRenderer_GetLatency(audioRenderer, invaildType, &latency);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        LOG("AudioSuiteRendererGetlatency_0600 pass, result is: %{public}d", AUDIOSTREAM_ERROR_INVALID_PARAM);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    return res;
...
static napi_value AudioSuiteRendererGetLatency_0700(napi_env env, napi_callback_info info)
{
    napi_value res;
    int32_t allLatency = 0;
    int32_t swLatency = 1;
    int32_t hdLatency = 2;
    if (OH_AudioStream_LatencyType::AUDIOSTREAM_LATENCY_TYPE_ALL != allLatency) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSuiteRendererGetLatency_0700,AUDIOSTREAM_LATENCY_TYPE0 fail, result is: %{public}d",
        AUDIOSTREAM_LATENCY_TYPE_ALL);
        return res;
    }
    if (OH_AudioStream_LatencyType::AUDIOSTREAM_LATENCY_TYPE_SOFTWARE != swLatency) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSuiteRendererGetLatency_0700,AUDIOSTREAM_LATENCY_TYPE1 fail, result is: %{public}d",
        AUDIOSTREAM_LATENCY_TYPE_SOFTWARE);
        return res;
    }
    if (OH_AudioStream_LatencyType::AUDIOSTREAM_LATENCY_TYPE_HARDWARE != hdLatency) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AudioSuiteRendererGetLatency_0700,AUDIOSTREAM_LATENCY_TYPE2 fail, result is: %{public}d",
        AUDIOSTREAM_LATENCY_TYPE_HARDWARE);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioSessionManagerMuteSuggestion_001(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    result = OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    if (result != AUDIOCOMMON_RESULT_SUCCESS || audioSessionManager == nullptr) {
        LOG("OH_AudioManager_GetAudioSessionManager fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioSessionManager_SetScene fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    result = OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers(audioSessionManager, true);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    napi_create_int32(env, TEST_PASS, &value);
    return value;
}

static napi_value AudioSessionManagerMuteSuggestion_002(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    ...
    result = OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers(audioSessionManager, false);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
...
static napi_value AudioSessionManagerMuteSuggestion_003(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    result = OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers(nullptr, true);
    if (result != AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        LOG("OH_AudioSessionManager_EnableMuteSuggestionWhenMixWithOthers fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
...
static napi_value AudioSessionManagerMuteSuggestion_004(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    OH_AudioSessionManager *audioSessionManager = nullptr;
...
static napi_value AudioSessionManagerMuteSuggestion_005(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    OH_AudioSessionManager *audioSessionManager = nullptr;
...
static napi_value AudioSessionManagerIsOtherMediaPlaying_001(napi_env env, napi_callback_info info)
{
    napi_value value;
    OH_AudioCommon_Result result = AUDIOCOMMON_RESULT_SUCCESS;
    OH_AudioSessionManager *audioSessionManager = nullptr;
    ...
    result = OH_AudioSessionManager_ActivateAudioSession(audioSessionManager, &strategy);
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioSessionManager_ActivateAudioSession fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    bool isOtherMediaPlaying = OH_AudioSessionManager_IsOtherMediaPlaying(audioSessionManager);
    if (isOtherMediaPlaying) {
        LOG("OH_AudioSessionManager_IsOtherMediaPlaying fail, result is: %{public}d", isOtherMediaPlaying);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    ...
    if (result != AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("OH_AudioSessionManager_DeactivateAudioSession fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &value);
        return value;
    }
    napi_create_int32(env, TEST_PASS, &value);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0001(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(g_audioCapturer,
        MyPlaybackCapturerStartCallback, userData);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0001 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0002(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(nullptr,
        MyPlaybackCapturerStartCallback, userData);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0002 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0003(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(g_audioCapturer,
        nullptr, userData);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0003 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0004(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(g_audioCapturer,
        MyPlaybackCapturerStartCallback, nullptr);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0004 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0005(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(nullptr,
        MyPlaybackCapturerStartCallback, nullptr);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0005 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioCapturer_RequestPlaybackStart_Callback_0006(napi_env env, napi_callback_info info)
{
    napi_value res;
    void *userData;
    OH_AudioStream_Result result = OH_AudioCapturer_RequestPlaybackCaptureStart(g_audioCapturer,
        nullptr, nullptr);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioCapturer_RequestPlaybackStart_Callback_0006 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0001 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_MEDIA|AUDIOSTREAM_PLAYBACKCAPTURE_MODE_DEFAULT|
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_EXCLUDING_SELF);
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0001 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0002 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_MEDIA|AUDIOSTREAM_PLAYBACKCAPTURE_MODE_DEFAULT);
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0002 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0003 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_EXCLUDING_SELF|AUDIOSTREAM_PLAYBACKCAPTURE_MODE_MEDIA);
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0003 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0004(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0004 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_DEFAULT|AUDIOSTREAM_PLAYBACKCAPTURE_MODE_EXCLUDING_SELF);
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0004 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0005(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0005 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    ...
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0005 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0006(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0006 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_EXCLUDING_SELF);
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0006 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0007(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0007 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    ...
    if (result != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0007 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0008(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0008 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(nullptr,
        AUDIOSTREAM_PLAYBACKCAPTURE_MODE_MEDIA);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0008 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStreamBuilder_SetPlaybackCaptureMode_0009(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* g_capturerBuilder = nullptr;
    OH_AudioStream_Result createResult = OH_AudioStreamBuilder_Create(&g_capturerBuilder, AUDIOSTREAM_TYPE_CAPTURER);
    if (createResult != AUDIOSTREAM_SUCCESS) {
        LOG("AudioStreamBuilder_Create_0009 fail, result is: %{public}d", createResult);
        napi_value result;
        napi_create_int32(env, TEST_FAIL, &result);
        return result;
    }
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetPlaybackCaptureMode(g_capturerBuilder, 3);
    if (result != AUDIOSTREAM_ERROR_INVALID_PARAM) {
        LOG("AudioStreamBuilder_SetPlaybackCaptureMode_0009 fail, result is: %{public}d", result);
        napi_create_int32(env, TEST_FAIL, &res);
        return res;
    }
    napi_create_int32(env, TEST_PASS, &res);
...
static napi_value AudioStream_PlaybackCaptureStartState_0001(napi_env env, napi_callback_info info)
{
    napi_value res;
    int32_t playbackCaptureSuccess = 0;
    int32_t playbackCaptureFiled = 1;
    int32_t playbackCaptureUnauthorized = 2;
    if (playbackCaptureSuccess != AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_SUCCESS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_SUCCESS fail, result is: %{public}d",
            AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_SUCCESS);
    }
    if (playbackCaptureFiled != AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_FAILED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_FAILED fail, result is: %{public}d",
            AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_FAILED);
    }
    if (playbackCaptureUnauthorized != AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_NOT_AUTHORIZED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSTREAM_PLAYBACKCAPTURE_START_STATE_NOT_AUTHORIZED fail, result is: %{public}d",
...
static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_enum_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    int32_t CONCURRENCY_DEFAULT = 0;
    int32_t CONCURRENCY_MIX_WITH_OTHERS = 1;
    int32_t CONCURRENCY_DUCK_OTHERS = 2;
    int32_t CONCURRENCY_PAUSE_OTHERS = 3;
    if (CONCURRENCY_DEFAULT != OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCAPTURE_CONCURRENCY_DEFAULT fail, result is: %{public}d",
            OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT);
    }
    if (CONCURRENCY_MIX_WITH_OTHERS != OH_AudioSession_ConcurrencyMode::CONCURRENCY_MIX_WITH_OTHERS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCAPTURE_CONCURRENCY_MIX_WITH_OTHERS fail, result is: %{public}d",
            OH_AudioSession_ConcurrencyMode::CONCURRENCY_MIX_WITH_OTHERS);
    }
    if (CONCURRENCY_DUCK_OTHERS != OH_AudioSession_ConcurrencyMode::CONCURRENCY_DUCK_OTHERS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCAPTURE_CONCURRENCY_CONCURRENCY_DUCK_OTHERS fail, result is: %{public}d",
            OH_AudioSession_ConcurrencyMode::CONCURRENCY_DUCK_OTHERS);
    }
    if (CONCURRENCY_PAUSE_OTHERS != OH_AudioSession_ConcurrencyMode::CONCURRENCY_PAUSE_OTHERS) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCAPTURE_CONCURRENCY_PAUSE_OTHERS fail, result is: %{public}d",
            OH_AudioSession_ConcurrencyMode::CONCURRENCY_PAUSE_OTHERS);
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_enum_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    uint32_t DEFAULT_BEHAVIOR = 0x00000000;
    uint32_t MUTE_WHEN_INTERRUPTED = 0x00000002;
    int32_t AUDIO_SESSION_STATE_CHANGE_HINT_MUTE = 8;
    int32_t AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE = 9;
    if (DEFAULT_BEHAVIOR != OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSESSIONMANAGER_DEFAULT_BEHAVIOR fail, result is: %{public}d",
            OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    }
    if (MUTE_WHEN_INTERRUPTED != OH_AudioSession_BehaviorFlags::MUTE_WHEN_INTERRUPTED) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSESSIONMANAGER_MUTE_WHEN_INTERRUPTED fail, result is: %{public}d",
            OH_AudioSession_BehaviorFlags::MUTE_WHEN_INTERRUPTED);
    }
    if (AUDIO_SESSION_STATE_CHANGE_HINT_MUTE !=
        OH_AudioSession_StateChangeHint::AUDIO_SESSION_STATE_CHANGE_HINT_MUTE) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSESSIOSTATECHANGEHINT_AUDIO_SESSION_STATE_CHANGE_HINT_MUTE fail, result is: %{public}d",
            OH_AudioSession_StateChangeHint::AUDIO_SESSION_STATE_CHANGE_HINT_MUTE);
    }
    if (AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE !=
        OH_AudioSession_StateChangeHint::AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSESSIOSTATECHANGEHINT_AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE fail, result is: %{public}d",
            OH_AudioSession_StateChangeHint::AUDIO_SESSION_STATE_CHANGE_HINT_UNMUTE);
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
...
static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, &strategy,
        OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_001 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_MIX_WITH_OTHERS;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, &strategy,
        OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioCapturer_Release(audioCapturer);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_002 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DUCK_OTHERS;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, &strategy,
        OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioCapturer_Release(audioCapturer);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_003 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_004(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_PAUSE_OTHERS;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, &strategy,
        OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioCapturer_Release(audioCapturer);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_004 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_005(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_005 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_006(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_006 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_007(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_007 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_008(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_008 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_009(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_009 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_010(napi_env env, napi_callback_info info)
{
    napi_value res;
    uint32_t behavior = -1;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    ...
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, &strategy, behavior);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_010 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioCapturer_SetIndependentAudioSessionStrategy_011(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioCapturer_SetIndependentAudioSessionStrategy(
        audioCapturer, nullptr, OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioCapturer_SetIndependentAudioSessionStrategy_011 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioRenderer_SetIndependentAudioSessionStrategy(
        audioRenderer, &strategy,
        OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_001 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_002 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_003 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_004(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_004 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_005(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_005 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_006(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_006 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_007(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_007 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_008(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_008 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_009(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSession_Strategy strategy;
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_009 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_010(napi_env env, napi_callback_info info)
{
    napi_value res;
    uint32_t behavior = -1;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    ...
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioRenderer_SetIndependentAudioSessionStrategy(
        audioRenderer, &strategy, behavior);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_010 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioRenderer_SetIndependentAudioSessionStrategy_011(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateRenderBuilder();
    OH_AudioRenderer *audioRenderer;
    ...
    strategy.concurrencyMode = OH_AudioSession_ConcurrencyMode::CONCURRENCY_DEFAULT;
    OH_AudioStream_Result result = OH_AudioRenderer_SetIndependentAudioSessionStrategy(
        audioRenderer, nullptr, OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    OH_AudioRenderer_Release(audioRenderer);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioRenderer_SetIndependentAudioSessionStrategy_011 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManager_SetBehavior_001(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result = OH_AudioSessionManager_SetBehavior(
 	    audioSessionManager, OH_AudioSession_BehaviorFlags::DEFAULT_BEHAVIOR);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioSessionManager_SetBehavior_001 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManager_SetBehavior_002(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result =  OH_AudioSessionManager_SetBehavior(
 	    audioSessionManager, OH_AudioSession_BehaviorFlags::MUTE_WHEN_INTERRUPTED);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioSessionManager_SetBehavior_002 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManager_SetBehavior_003(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result =  OH_AudioSessionManager_SetBehavior(
 	    nullptr, OH_AudioSession_BehaviorFlags::MUTE_WHEN_INTERRUPTED);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioSessionManager_SetBehavior_003 fail, result is: %{public}d", result);
    return res;
}

static napi_value AudioSessionManager_SetBehavior_004(napi_env env, napi_callback_info info)
{
    napi_value res;
    uint32_t behavior = -1;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result =  OH_AudioSessionManager_SetBehavior(
 	    audioSessionManager, behavior);
    if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AudioSessionManager_SetBehavior_004 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioStream_Result result = OH_AudioCapturer_SetMuteHint(audioCapturer, true);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0100 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0200(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioStream_Result result = OH_AudioCapturer_SetMuteHint(audioCapturer, false);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0200 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0300(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioStream_Result result = OH_AudioCapturer_SetMuteHint(nullptr, true);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0300 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0400(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0400 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioManager = nullptr;
    OH_AudioManager_GetAudioSessionManager(&audioManager);
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioCommon_Result result = OH_AudioSessionManager_SetCaptureMuteHint(audioManager, true);
    OH_AudioSessionManager_DeactivateAudioSession(audioManager);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0100 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0200(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioManager = nullptr;
    OH_AudioManager_GetAudioSessionManager(&audioManager);
    ...
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioCommon_Result result = OH_AudioSessionManager_SetCaptureMuteHint(audioManager, false);
    OH_AudioSessionManager_DeactivateAudioSession(audioManager);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOSTREAM_SUCCESS) {
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0200 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0300(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioManager = nullptr;
    OH_AudioManager_GetAudioSessionManager(&audioManager);
    ...
    OH_AudioStreamBuilder_GenerateCapturer(builder, &audioCapturer);
    OH_AudioCapturer_Start(audioCapturer);
    OH_AudioCommon_Result result = OH_AudioSessionManager_SetCaptureMuteHint(nullptr, true);
    OH_AudioSessionManager_DeactivateAudioSession(audioManager);
    OH_AudioCapturer_Release(audioCapturer);
    OH_AudioStreamBuilder_Destroy(builder);
        if (result == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0300 fail, result is: %{public}d", result);
    return res;
}
static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0400(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager *audioManager = nullptr;
    OH_AudioManager_GetAudioSessionManager(&audioManager);
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0400 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERENUM_0300(napi_env env, napi_callback_info info)
{
    napi_value res;
    uint32_t voipPrivacyTypePublic = 0x00000001;
    if (VOIP_PRIVACY_TYPE_PUBLIC != OH_AudioSession_BehaviorFlags::VOIP_PRIVACY_TYPE_PUBLIC) {
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOSESSIONMANAGER_VOIP_PRIVACY_TYPE_PUBLIC fail, result is: %{public}d",
            OH_AudioSession_BehaviorFlags::VOIP_PRIVACY_TYPE_PUBLIC);
    }
    napi_create_int32(env, TEST_PASS, &res);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AUDIOCAPTURERVOICE_0100 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0200(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AUDIOCAPTURERVOICE_0200 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0300(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AUDIOCAPTURERVOICE_0300 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0400(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AUDIOCAPTURERVOICE_0400 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSESSIONMANAGERVOICE_0500(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    OH_AudioCommon_Result result =  OH_AudioSessionManager_SetBehavior(
 	    audioSessionManager, OH_AudioSession_BehaviorFlags::VOIP_PRIVACY_TYPE_PUBLIC);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("AUDIOSESSIONMANAGERVOICE_0500 fail, result is: %{public}d", result);
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_ADUIOSESSIONMANAGERVOICEREAPTED_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioSessionManager* audioSessionManager;
    OH_AudioManager_GetAudioSessionManager(&audioSessionManager);
    for (int i = 0 ; i < 100 ;i++) {
         ...
         return res;
         napi_create_int32(env, TEST_FAIL, &res);
         LOG("ADUIOSESSIONMANAGERVOICEREAPTED_0100 fail, result is: %{public}d", result);
        }
    }
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOCAPTUREVOICEREAPTED_0100(napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder *builder = CreateCapturerBuilder();
    OH_AudioCapturer *audioCapturer;
    ...
    for (int i = 0 ;i < 100; i++) {
        ...
        napi_create_int32(env, TEST_FAIL, &res);
        LOG("AUDIOCAPTUREVOICEREAPTED_0100 fail, result is: %{public}d", result);
    }
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0100(
    napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerLoopbackEffectEnabled(nullptr, true);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0100 result: %{public}d", result);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0100 fail");
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0200(
    napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateCapturerBuilder();
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerLoopbackEffectEnabled(builder, true);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0200 result: %{public}d", result);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0200 fail");
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0300(
    napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateCapturerBuilder();
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerLoopbackEffectEnabled(builder, false);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0300 result: %{public}d", result);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0300 fail");
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0100(
    napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStreamBuilder* builder = CreateCapturerBuilder();
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerLoopbackEffectEnabled(builder, true);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0100 result: %{public}d", result);
    OH_AudioStreamBuilder_Destroy(builder);
    if (result == AUDIOSTREAM_SUCCESS) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    ...
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0100 fail");
    return res;
}

static napi_value SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0200(
    napi_env env, napi_callback_info info)
{
    napi_value res;
    OH_AudioStream_Result result = OH_AudioStreamBuilder_SetCapturerLoopbackEffectEnabled(nullptr, false);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0200 result: %{public}d", result);
    if (result == AUDIOSTREAM_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
        return res;
    }
    napi_create_int32(env, TEST_FAIL, &res);
    LOG("SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0200 fail");
    return res;
}

static int32_t PreferredOutputDeviceChangedCallback(
    OH_AudioDeviceDescriptorArray *audioDeviceDescriptorArray)
{
    LOG("PreferredOutputDeviceChangedCallback triggered");
    if (audioDeviceDescriptorArray == nullptr) {
        LOG("audioDeviceDescriptorArray is nullptr");
        return 0;
    }
    int size = audioDeviceDescriptorArray->size;
        for (int index = 0; index < size; index++) {
    ...
    LOG("audioDeviceDescriptorArray size: %{public}d", size);
    return 0;
}

static int32_t PreferredInputDeviceChangedCallback(
    OH_AudioDeviceDescriptorArray *audioDeviceDescriptorArray)
{
    LOG("PreferredInputDeviceChangedCallback triggered");
    if (audioDeviceDescriptorArray == nullptr) {
        LOG("audioDeviceDescriptorArray is nullptr");
        return 0;
    }
    LOG("audioDeviceDescriptorArray size: %{public}d", audioDeviceDescriptorArray->size);
    return 0;
}

static int32_t PreferredOutputDeviceChangedCallbackTest(
    OH_AudioDeviceDescriptorArray *audioDeviceDescriptorArray)
{
    LOG("PreferredOutputDeviceChangedCallbackTest called");
    return AUDIOCOMMON_RESULT_SUCCESS;
}

static int32_t PreferredInputDeviceChangedCallbackTest(
    OH_AudioDeviceDescriptorArray *audioDeviceDescriptorArray)
{
    LOG("PreferredInputDeviceChangedCallbackTest called");
    return AUDIOCOMMON_RESULT_SUCCESS;
}

OH_AudioRoutingManager* GetAudioRoutingManager()
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioCommon_Result result = OH_AudioManager_GetAudioRoutingManager(&audioRoutingManager);
    if (result == AUDIOCOMMON_RESULT_SUCCESS) {
        LOG("GetAudioRoutingManager success");
        return audioRoutingManager;
    }
    LOG("GetAudioRoutingManager failed, result: %{public}d", result);
    return nullptr;
}

napi_value CreateNapiResult(napi_env env, int32_t resultValue)
{
    napi_value result;
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
        LOG("audioRoutingManager is nullptr");
        return CreateNapiResult(env, TEST_FAIL);
    }
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_MUSIC;
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("RegisterPreferredOutputDevicesChangeCallback result: %{public}d", ret);
    napi_value res;
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_GAME;
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_MUSIC;
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("RegisterPreferredOutputDevicesChangeCallback with nullptr manager result: %{public}d", ret);
    napi_value res;
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_MUSIC;
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback = nullptr;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("RegisterPreferredOutputDevicesChangeCallback with nullptr callback result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0300(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_Usage streamUsage = static_cast<OH_AudioStream_Usage>(-1);
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("RegisterPreferredOutputDevicesChangeCallback with invalid streamUsage result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0400(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_MUSIC;
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("RegisterPreferredOutputDevicesChangeCallback with both nullptr result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    if (ret == AUDIOCOMMON_RESULT_SUCCESS) {
        ret = OH_AudioRoutingManager_UnregisterPreferredOutputDevicesChangeCallback(
            audioRoutingManager, callback);
        LOG("UnregisterPreferredOutputDevicesChangeCallback result: %{public}d", ret);
        napi_value res;
        if (ret == AUDIOCOMMON_RESULT_SUCCESS) {
            napi_create_int32(env, TEST_PASS, &res);
        } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_ERROR_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_UnregisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, callback);
    LOG("UnregisterPreferredOutputDevicesChangeCallback with nullptr manager result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback =
        PreferredInputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("RegisterPreferredInputDevicesChangeCallback result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_SUCCESS) {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioStream_SourceType sourceType = AUDIOSTREAM_SOURCE_TYPE_MIC;
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("RegisterPreferredInputDevicesChangeCallback with nullptr manager result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_SourceType sourceType = AUDIOSTREAM_SOURCE_TYPE_MIC;
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback = nullptr;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("RegisterPreferredInputDevicesChangeCallback with nullptr callback result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0300(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_SourceType sourceType = static_cast<OH_AudioStream_SourceType>(-1);
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback =
        PreferredInputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("RegisterPreferredInputDevicesChangeCallback with invalid sourceType result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0400(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioStream_SourceType sourceType = AUDIOSTREAM_SOURCE_TYPE_MIC;
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("RegisterPreferredInputDevicesChangeCallback with both nullptr result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    if (ret == AUDIOCOMMON_RESULT_SUCCESS) {
        ret = OH_AudioRoutingManager_UnregisterPreferredInputDevicesChangeCallback(
            audioRoutingManager, callback);
        LOG("UnregisterPreferredInputDevicesChangeCallback result: %{public}d", ret);
        napi_value res;
        if (ret == AUDIOCOMMON_RESULT_SUCCESS) {
            napi_create_int32(env, TEST_PASS, &res);
        } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_ERROR_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = nullptr;
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback =
        PreferredInputDeviceChangedCallback;
    OH_AudioCommon_Result ret = OH_AudioRoutingManager_UnregisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, callback);
    LOG("UnregisterPreferredInputDevicesChangeCallback with nullptr manager result: %{public}d", ret);
    napi_value res;
    if (ret == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_Usage streamUsage = AUDIOSTREAM_USAGE_MUSIC;
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallbackTest;
    OH_AudioCommon_Result ret1 = OH_AudioRoutingManager_RegisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, streamUsage, callback);
    LOG("Register result: %{public}d", ret1);
    if (ret1 == AUDIOCOMMON_RESULT_SUCCESS) {
        OH_AudioCommon_Result ret2 = OH_AudioRoutingManager_UnregisterPreferredOutputDevicesChangeCallback(
            audioRoutingManager, callback);
        LOG("Unregister result: %{public}d", ret2);
        napi_value res;
        if (ret2 == AUDIOCOMMON_RESULT_SUCCESS) {
            napi_create_int32(env, TEST_PASS, &res);
        } else {
            napi_create_int32(env, TEST_FAIL, &res);
        }
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioRoutingManager_OnPreferredOutputDeviceChangedCallback callback =
        PreferredOutputDeviceChangedCallbackTest;
    OH_AudioCommon_Result ret1 = OH_AudioRoutingManager_UnregisterPreferredOutputDevicesChangeCallback(
        audioRoutingManager, callback);
    LOG("Unregister unregistered callback result: %{public}d", ret1);
    napi_value res;
    if (ret1 == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
        napi_create_int32(env, TEST_PASS, &res);
    } else {
        napi_create_int32(env, TEST_FAIL, &res);
    }
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0100(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioStream_SourceType sourceType = AUDIOSTREAM_SOURCE_TYPE_MIC;
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback =
        PreferredInputDeviceChangedCallbackTest;
    OH_AudioCommon_Result ret1 = OH_AudioRoutingManager_RegisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, sourceType, callback);
    LOG("Register result: %{public}d", ret1);
    if (ret1 == AUDIOCOMMON_RESULT_SUCCESS) {
        OH_AudioCommon_Result ret2 = OH_AudioRoutingManager_UnregisterPreferredInputDevicesChangeCallback(
            audioRoutingManager, callback);
        LOG("Unregister result: %{public}d", ret2);
        napi_value res;
        if (ret2 == AUDIOCOMMON_RESULT_SUCCESS) {
...
static napi_value SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0200(
    napi_env env, napi_callback_info info)
{
    OH_AudioRoutingManager *audioRoutingManager = GetAudioRoutingManager();
    if (audioRoutingManager == nullptr) {
    ...
    OH_AudioRoutingManager_OnPreferredInputDeviceChangedCallback callback =
        PreferredInputDeviceChangedCallbackTest;
    OH_AudioCommon_Result ret1 = OH_AudioRoutingManager_UnregisterPreferredInputDevicesChangeCallback(
        audioRoutingManager, callback);
    LOG("Unregister unregistered callback result: %{public}d", ret1);
    napi_value res;
    if (ret1 == AUDIOCOMMON_RESULT_ERROR_INVALID_PARAM) {
...
napi_property_descriptor desc1[] = {
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_0200, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0200, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0300", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0300, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0400", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_ERROR_0400, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_ERROR_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_UNREGISTER_ERROR_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_0200, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0200, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0300", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0300, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0400", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_ERROR_0400, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_ERROR_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_UNREGISTER_ERROR_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_OUTPUT_REGISTER_UNREGISTER_0200, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0100", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0100, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0200", nullptr,
        SUB_AUDIO_ROUTING_MANAGER_PREFERRED_INPUT_REGISTER_UNREGISTER_0200, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0200", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0200, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0300", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0300, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0400", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETMUTEHINT_0400, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0200", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0200, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0300", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0300, nullptr, nullptr, nullptr, napi_default, nullptr},
 	{"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0400", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURER_SETCAPTURERMUTEHINT_0400, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0100", nullptr, AudioSuiteRendererGetLatency_0100,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0200", nullptr, AudioSuiteRendererGetLatency_0200,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0300", nullptr, AudioSuiteRendererGetLatency_0300,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0400", nullptr, AudioSuiteRendererGetLatency_0400,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0500", nullptr, AudioSuiteRendererGetLatency_0500,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0600", nullptr, AudioSuiteRendererGetLatency_0600,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSuiteRendererGetLatency_0700", nullptr, AudioSuiteRendererGetLatency_0700,
    nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"audioWorkGroup", nullptr, AudioWorkGroup, nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"audioStreamBuilderSetLatencyMode", nullptr, AudioStreamBuilderSetLatencyMode,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilderSetLatencyMode_001", nullptr, AudioStreamBuilderSetLatencyMode_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"audioStreamBuilderSetCapturerInfo", nullptr, AudioStreamBuilderSetCapturerInfo,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilderSetCapturerInfo_001", nullptr, AudioStreamBuilderSetCapturerInfo_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"audioStreamBuilderSetFrameSizeInCallback", nullptr, AudioStreamBuilderSetFrameSizeInCallback,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioCreateEngine", nullptr, AudioCreateEngine,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"audioManagerGetAudioScene_01", nullptr, AudioManagerGetAudioScene_01, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"AudioLowLatencyOut", nullptr, AudioLowLatencyOut, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioWorkGroupSet", nullptr, AudioWorkGroupSet, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioWorkGroup_001", nullptr, AudioWorkGroup_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"StopAndReleaseRenderer", nullptr, StopAndReleaseRenderer, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"GetRawFileContent", nullptr, &FileRecourceManager::GetRawFileContent, nullptr, nullptr, nullptr, napi_default,
        nullptr},
    {"StartAudioRenderer", nullptr, StartAudioRenderer, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"getFileList", nullptr, &FileRecourceManager::GetFileList, nullptr, nullptr, nullptr, napi_default, nullptr },
    {"AudioSessionManagerIsOtherMediaPlaying_001", nullptr, AudioSessionManagerIsOtherMediaPlaying_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
...
napi_property_descriptor desc2[] = {
    ...
    {"audioAudioInternalRecordingSuccess03", nullptr,
        AudioAudioInternalRecordingSuccess03, nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"AudioSessionManagerSetScene_001", nullptr,
        AudioSessionManagerSetScene_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterStateChangeCallback_001", nullptr,
        AudioSessionManagerRegisterStateChangeCallback_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterStateChangeCallback_002", nullptr,
        AudioSessionManagerRegisterStateChangeCallback_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterStateChangeCallback_003", nullptr,
        AudioSessionManagerRegisterStateChangeCallback_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerSetDefaultOutputDevice_001", nullptr,
        AudioSessionManagerSetDefaultOutputDevice_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerSetDefaultOutputDevice_002", nullptr,
        AudioSessionManagerSetDefaultOutputDevice_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerGetDefaultOutputDevice_001", nullptr,
        AudioSessionManagerGetDefaultOutputDevice_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterDeviceCallback_001", nullptr,
        AudioSessionManagerRegisterDeviceCallback_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterDeviceCallback_002", nullptr,
        AudioSessionManagerRegisterDeviceCallback_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerRegisterDeviceCallback_003", nullptr,
        AudioSessionManagerRegisterDeviceCallback_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerReleaseDevices_001", nullptr,
        AudioSessionManagerReleaseDevices_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerReleaseDevices_002", nullptr,
        AudioSessionManagerReleaseDevices_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    ...
    {"isIntelligentNoiseReductionEnabledForCurrentDevice_001", nullptr,
        IsIntelligentNoiseReductionEnabledForCurrentDevice_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerAvailableDevices_001", nullptr,
        AudioSessionManagerAvailableDevices_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerAvailableDevices_002", nullptr,
        AudioSessionManagerAvailableDevices_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerAvailableDevices_003", nullptr,
        AudioSessionManagerAvailableDevices_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerAvailableDevices_004", nullptr,
        AudioSessionManagerAvailableDevices_004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_005", nullptr,
        AudioSessionManagerAvailableDevices_005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_006", nullptr,
        AudioSessionManagerAvailableDevices_006, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_007", nullptr,
        AudioSessionManagerAvailableDevices_007, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_008", nullptr,
        AudioSessionManagerAvailableDevices_008, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_009", nullptr,
        AudioSessionManagerAvailableDevices_009, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_010", nullptr,
        AudioSessionManagerAvailableDevices_010, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_011", nullptr,
        AudioSessionManagerAvailableDevices_011, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_012", nullptr,
        AudioSessionManagerAvailableDevices_012, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_013", nullptr,
        AudioSessionManagerAvailableDevices_013, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManagerAvailableDevices_014", nullptr,
        AudioSessionManagerAvailableDevices_014, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"bluetoothAndNearlinkPreferredRecordCategory_001", nullptr,
        BluetoothAndNearlinkPreferredRecordCategory_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCommonResult_001", nullptr, AudioCommonResult_001, nullptr, nullptr, nullptr, napi_default, nullptr},
};
napi_property_descriptor desc3[] = {
    ...
    {"audioCaptureGetFastStatus_001", nullptr, AudioCaptureGetFastStatus_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioCaptureGetFastStatus_002", nullptr, AudioCaptureGetFastStatus_002,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioRendererGetFastStatus_001", nullptr, AudioRendererGetFastStatus_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioRendererGetFastStatus_002", nullptr, AudioRendererGetFastStatus_002,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamManagerIsFastPlaybackSupported_001", nullptr, AudioStreamManagerIsFastPlaybackSupported_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamManagerIsFastRecordingSupported_001", nullptr, AudioStreamManagerIsFastRecordingSupported_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamManagerIsMultichannelPlaybackSupported_001", nullptr,
        AudioStreamManagerIsMultichannelPlaybackSupported_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamManagerIsDirectPlaybackSupported_001", nullptr, AudioStreamManagerIsDirectPlaybackSupported_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamManagerIsOffloadPlaybackSupported_001", nullptr, AudioStreamManagerIsOffloadPlaybackSupported_001,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamBuilderSetCapturerFastStatusChangeCallback_001", nullptr,
        AudioStreamBuilderSetCapturerFastStatusChangeCallback_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamBuilderSetCapturerFastStatusChangeCallback_002", nullptr,
        AudioStreamBuilderSetCapturerFastStatusChangeCallback_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamBuilderSetRendererFastStatusChangeCallback_001", nullptr,
        AudioStreamBuilderSetRendererFastStatusChangeCallback_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioStreamBuilderSetRendererFastStatusChangeCallback_002", nullptr,
        AudioStreamBuilderSetRendererFastStatusChangeCallback_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerMuteSuggestion_001", nullptr,
        AudioSessionManagerMuteSuggestion_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerMuteSuggestion_002", nullptr,
        AudioSessionManagerMuteSuggestion_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerMuteSuggestion_003", nullptr,
        AudioSessionManagerMuteSuggestion_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerMuteSuggestion_004", nullptr,
        AudioSessionManagerMuteSuggestion_004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"audioSessionManagerMuteSuggestion_005", nullptr,
        AudioSessionManagerMuteSuggestion_005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0001", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0002", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0003", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0004", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0005", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0006", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0006, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_RequestPlaybackStart_Callback_0007", nullptr,
        AudioCapturer_RequestPlaybackStart_Callback_0007, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0001", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0002", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0003", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0004", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0005", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0006", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0006, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0007", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0007, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0008", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0008, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStreamBuilder_SetPlaybackCaptureMode_0009", nullptr,
        AudioStreamBuilder_SetPlaybackCaptureMode_0009, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioStream_PlaybackCaptureStartState_0001", nullptr,
        AudioStream_PlaybackCaptureStartState_0001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_enum_001", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_enum_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_enum_002", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_enum_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_001", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_002", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_003", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_004", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_005", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_006", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_006, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_007", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_007, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_008", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_008, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_009", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_009, nullptr, nullptr, nullptr, napi_default, nullptr},    
    {"AudioCapturer_SetIndependentAudioSessionStrategy_010", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_010, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioCapturer_SetIndependentAudioSessionStrategy_011", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_011, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_001", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_002", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_003", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_004", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_005", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_005, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_006", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_006, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_007", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_007, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_008", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_008, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_009", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_009, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_010", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_010, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioRenderer_SetIndependentAudioSessionStrategy_011", nullptr,
        AudioCapturer_SetIndependentAudioSessionStrategy_011, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManager_SetBehavior_001", nullptr,
        AudioSessionManager_SetBehavior_001, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManager_SetBehavior_002", nullptr,
        AudioSessionManager_SetBehavior_002, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManager_SetBehavior_003", nullptr,
        AudioSessionManager_SetBehavior_003, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"AudioSessionManager_SetBehavior_004", nullptr,
        AudioSessionManager_SetBehavior_004, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERENUM_0300", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERENUM_0300, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0200", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0200, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0300", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0300, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0400", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTURERVOICE_0400, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSESSIONMANAGERVOICE_0500", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSESSIONMANAGERVOICE_0500, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOCAPTUREVOICEREAPTED_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOCAPTUREVOICEREAPTED_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_ADUIOSESSIONMANAGERVOICEREAPTED_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_ADUIOSESSIONMANAGERVOICEREAPTED_0100, nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0100,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0200", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0200,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0300", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_PARAM_0300,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0100", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0100,
        nullptr, nullptr, nullptr, napi_default, nullptr},
    {"SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0200", nullptr,
        SUB_MULTIMEDIA_AUDIO_AUDIOSTREAMBUILDER_SETCAPTURELOOPBACKEFFECT_RETURN_0200,
        nullptr, nullptr, nullptr, napi_default, nullptr},
};
