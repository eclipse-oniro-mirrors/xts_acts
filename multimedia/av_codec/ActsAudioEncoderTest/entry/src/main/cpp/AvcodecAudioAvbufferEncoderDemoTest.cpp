...
constexpr int32_t SAMPLE_FORMAT = AUDIOSTREAM_SAMPLE_S32LE;
...
static void GetParamsByName(string decoderName, string inputFile,
    int32_t &channelCount, int32_t &sampleRate, long &bitrate)
{
...
static void OnOutputBufferAvailable(OH_AVCodec* codec, uint32_t index, OH_AVBuffer* buffer, void* userData)
{
    ...
    if (buffer) {
        ...
        OH_AVErrCode errCode = OH_AVBuffer_GetBufferAttr(buffer, &attr);
        if (errCode == AV_ERR_OK) {
            cout << "OnOutputBufferAvailable received, index:" << index << ", size:" << attr.size
                 << ", flags:" << attr.flags << ", pts: " << attr.pts << endl;
        } else {
...
bool AudioBufferAacEncDemo::RunCase(std::string inputFile, std::string outputFile)
{
    ...
    DEMO_CHECK_AND_RETURN_RET_LOG(CreateEnc() == AV_ERR_OK, false, "Fatal: CreateEnc fail");
    ...
    if (audioType_ == AudioBufferFormatType::TYPE_OPUS) {
        ...
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_BITS_PER_CODED_SAMPLE, BIT_PER_CODE_COUNT);
        OH_AVFormat_SetIntValue(format, OH_MD_KEY_COMPLIANCE_LEVEL, COMPLEXITY_COUNT);
    ...
    DEMO_CHECK_AND_RETURN_RET_LOG(Configure(format) == AV_ERR_OK, false, "Fatal: Configure fail");
    DEMO_CHECK_AND_RETURN_RET_LOG(Start() == AV_ERR_OK, false, "Fatal: Start fail");
    ...
    std::cout << "Encode finished, time = " << std::chrono::duration_cast<chrono::milliseconds>(end - start).count()
              << " ms" << std::endl;
    DEMO_CHECK_AND_RETURN_RET_LOG(Stop() == AV_ERR_OK, false, "Fatal: Stop fail");
    DEMO_CHECK_AND_RETURN_RET_LOG(Release() == AV_ERR_OK, false, "Fatal: Release fail");
...
{
    if (audioType_ == AudioBufferFormatType::TYPE_AAC) {
        audioEnc_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_ENCODER_AAC_NAME));
    } else if (audioType_ == AudioBufferFormatType::TYPE_FLAC) {
        audioEnc_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_ENCODER_FLAC_NAME));
    } else if (audioType_ == AudioBufferFormatType::TYPE_OPUS) {
        audioEnc_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_ENCODER_OPUS_NAME));
        std::cout << "CreateDec opus ok = " << std::endl;
    } else if (audioType_ == AudioBufferFormatType::TYPE_G711MU) {
        audioEnc_ = OH_AudioCodec_CreateByName((AVCodecCodecName::AUDIO_ENCODER_G711MU_NAME));
    } else {
        return AV_ERR_INVALID_VAL;
    }
    DEMO_CHECK_AND_RETURN_RET_LOG(audioEnc_ != nullptr, AV_ERR_UNKNOWN, "Fatal: CreateByName fail");
    if (signal_ == nullptr) {
        signal_ = new AEncSignal();
...
void AudioBufferAacEncDemo::InputFunc()
{
    ...
    int32_t frameBytes = (audioType_ == AudioBufferFormatType::TYPE_G711MU) ? g77mu_size :
        channels_ * sizeof(short) * sampleRate_ * time_20ms;
    ...
    while (isRunning_.load()) {
        ...
        if (!inputFile_.eof()) {
            ...
            OH_AVCodecBufferAttr attr = { size : frameBytes };
            OH_AVBuffer_SetBufferAttr(buffer, &attr);
        } else {
            OH_AVCodecBufferAttr attr = { size : 1, flags : AVCODEC_BUFFER_FLAGS_EOS };
            OH_AVBuffer_SetBufferAttr(buffer, &attr);
            HandleEOS(index);
            break;
        }
        int32_t ret = AV_ERR_OK;
        uint32_t bufferFlags = isFirstFrame_ ? AVCODEC_BUFFER_FLAGS_CODEC_DATA : AVCODEC_BUFFER_FLAGS_NONE;
        OH_AVCodecBufferAttr attr = { .flags = bufferFlags };
        OH_AVBuffer_SetBufferAttr(buffer, &attr);
...
void AudioBufferAacEncDemo::OutputFunc()
{
    ...
    while (isRunning_.load()) {
        ...
        if (avBuffer != nullptr && errCode == AV_ERR_OK) {
            outputFile_.write(reinterpret_cast<char*>(OH_AVBuffer_GetAddr(avBuffer)), attr.size);
        }
        if (avBuffer != nullptr && errCode == AV_ERR_OK && (attr.flags == AVCODEC_BUFFER_FLAGS_EOS || attr.size == 0)) {
        ...
        if (errCode == AV_ERR_OK && attr.flags == AVCODEC_BUFFER_FLAGS_EOS) {
...
OH_AVErrCode AudioBufferAacEncDemo::Configure(OH_AVCodec* codec,
                                              OH_AVFormat* format,
                                              int32_t channel,
                                              int32_t sampleRate,
                                              int64_t bitRate,
...
{
    const uint32_t TIMEOUT_SEC = 5;
    const auto totalTimeout = std::chrono::seconds(TIMEOUT_SEC);
    std::unique_lock<std::mutex> lock(signal_->inMutex_);
    bool hasData = signal_->inCond_.wait_for(
        lock, std::chrono::milliseconds(100), [this]() { return !signal_->inQueue_.empty(); });
    if (!hasData) {
        return 0;
    }
    uint32_t index = signal_->inQueue_.front();
...
uint32_t AudioBufferAacEncDemo::GetOutputIndex()
{
    ...
    bool hasData = signal_->outCond_.wait_for(
        lock, std::chrono::milliseconds(100), [this]() { return !signal_->outQueue_.empty(); });
    if (!hasData) {
        return 0;
    }
    uint32_t index = signal_->outQueue_.front();
...
OH_AVErrCode AudioBufferAacEncDemo::SetParameter(OH_AVCodec* codec,
                                                 OH_AVFormat* format,
                                                 int32_t channel,
                                                 int32_t sampleRate,
                                                 int64_t bitRate,
...
OH_AVErrCode AudioBufferAacEncDemo::Configure(OH_AVCodec* codec,
                                              ...
                                              int32_t syncMode)
{
    if (codec == nullptr || format == nullptr) {
        return AV_ERR_INVALID_VAL;
    }
    OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_CHANNEL_COUNT, channel);
    OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUD_SAMPLE_RATE, sampleRate);
    OH_AVFormat_SetLongValue(format, OH_MD_KEY_BITRATE, bitRate);
    OH_AVFormat_SetIntValue(format, OH_MD_KEY_AUDIO_SAMPLE_FORMAT, sampleFormat);
    OH_AVFormat_SetIntValue(format, OH_MD_KEY_ENABLE_SYNC_MODE, syncMode);
    OH_AVErrCode ret = OH_AudioCodec_Configure(codec, format);
    return ret;
}