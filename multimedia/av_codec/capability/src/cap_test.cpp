...
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0012, TestSize.Level1)
{
    if (!access("/system/lib64/media/", 0)) {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
            ASSERT_LE(false, OH_AVCapability_IsFeatureSupported(capability, VIDEO_ENCODER_TEMPORAL_SCALABILITY));
        }
    } else {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
...
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0014, TestSize.Level1)
{
    if (!access("/system/lib64/media/", 0)) {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
            ASSERT_LE(false, OH_AVCapability_IsFeatureSupported(capability, VIDEO_ENCODER_LONG_TERM_REFERENCE));
        }
    } else {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
...
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0016, TestSize.Level2)
{
    if (!access("/system/lib64/media/", 0)) {
        ...
        if (capability) {
            ...
            ASSERT_NE(nullptr, &isLowLatencySupported);
        }
    } else {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(
            OH_AVCODEC_MIMETYPE_VIDEO_AVC, false, HARDWARE);
...
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0017, TestSize.Level1)
{
    if (!access("/system/lib64/media/", 0)) {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
            ASSERT_LE(false, OH_AVCapability_IsFeatureSupported(capability, VIDEO_LOW_LATENCY));
        }
    } else {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
        if (capability) {
...
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0022, TestSize.Level1)
{
    if (!access("/system/lib64/media/", 0)) {
        ...
        if (capability) {
            ...
            int ltrnum = 0;
            ASSERT_LE(false,
                OH_AVFormat_GetIntValue(format, OH_FEATURE_PROPERTY_KEY_VIDEO_ENCODER_MAX_LTR_FRAME_COUNT, &ltrnum));
            EXPECT_LE(ltrnum, 10);
        }
    } else {
        OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
 ...
 * @tc.level  Level2
 */
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0026, TestSize.Level2)
{
    OH_AVCapability *capability = OH_AVCodec_GetCapabilityByCategory(OH_AVCODEC_MIMETYPE_VIDEO_AVC, true, HARDWARE);
    if (capability) {
        const OH_NativeBuffer_Format *pixlFormats = nullptr;
        uint32_t pixlFormatNum = 0;
        ASSERT_EQ(AV_ERR_OK, OH_AVCapability_GetVideoSupportedNativeBufferFormats(
            capability, &pixlFormats, &pixlFormatNum));
    }
}
 ...
 * @tc.level  Level2
 */
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0027, TestSize.Level2)
{
    OH_AVCapability *capability = OH_AVCodec_GetCapability(OH_AVCODEC_MIMETYPE_AUDIO_AAC, true);
    if (capability) {
        const OH_NativeBuffer_Format *pixlFormats = nullptr;
        uint32_t pixlFormatNum = 0;
        ASSERT_EQ(AV_ERR_INVALID_VAL, OH_AVCapability_GetVideoSupportedNativeBufferFormats(
            capability, &pixlFormats, &pixlFormatNum));
    }
}
 ...
 * @tc.level  Level2
 */
HWTEST_F(HwCapabilityNdkTest, VIDEO_TEMPORAL_ENCODE_API_0028, TestSize.Level2)
{
    OH_AVRange *sampleRateRanges = nullptr;
    uint32_t rangesNum = -1;
    ASSERT_EQ(AV_ERR_INVALID_VAL, OH_AVCapability_GetAudioSupportedSampleRateRanges(
        nullptr, &sampleRateRanges, &rangesNum));
}
} // namespace