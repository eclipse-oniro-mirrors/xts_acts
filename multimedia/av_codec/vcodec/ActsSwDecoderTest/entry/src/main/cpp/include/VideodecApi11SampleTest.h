...
#ifndef VIDEODEC_API11_SAMPLE_TEST_H
#define VIDEODEC_API11_SAMPLE_TEST_H
...
/** Returned by RunVideoDecSurface when NativeWindow/Surface is unavailable
 *  (e.g. no EGL/RS). Test may skip 0400/4000. */
constexpr int32_t AV_ERR_SURFACE_UNAVAILABLE_SKIP = 2;
/** Returned by RunVideoDec when input file is missing or unreadable (access fails). Test may skip. */
constexpr int32_t AV_ERR_INPUT_FILE_UNAVAILABLE_SKIP = 3;
...
public:
    ...
    std::deque<uint32_t> inIdxQueue_;
    std::queue<uint32_t> outIdxQueue_;
    std::queue<OH_AVCodecBufferAttr> attrQueue_;
    std::deque<OH_AVBuffer *> inBufferQueue_;
...
public:
    ...
    const char *inpDir =
        "/data/storage/el2/base/haps/entry_test/files/1920_1080_10_30Mb.h264";
    const char *outDir = "/data/storage/el2/base/haps/entry_test/files/VDecTest.yuv";
    const char *outDir2 = "/data/storage/el2/base/haps/entry_test/files/VDecTest2.yuv";
    bool sfOutput = false;
...
private:
    int32_t PrepareStartDecoderPipeline();
    int32_t PrepareStartDecoderOpenInputFile();
    void FinishOutputFuncTestOnEos(uint32_t index, OH_AVBuffer *buffer, OH_AVCodecBufferAttr &attr, bool &runFlag);
    void InputFuncTestApplyRepeatControls();
    bool InputFuncTest_FinishIterationAfterPush(int ret, uint32_t index, OH_AVBuffer *buffer, bool &flag);
    bool OutputFuncTest_WaitAndPop(OH_AVCodecBufferAttr &attr, uint32_t &index, OH_AVBuffer *&buffer);
    ...
    uint32_t inputPushRetryCount_ = 0;
...
#endif // VIDEODEC_API11_SAMPLE_TEST_H
