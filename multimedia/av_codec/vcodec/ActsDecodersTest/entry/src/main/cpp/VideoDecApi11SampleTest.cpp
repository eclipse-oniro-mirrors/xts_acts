...
constexpr uint32_t DKIF_SIZE = 4;
constexpr uint32_t FRAME_HEADER_SIZE = 12;
constexpr uint32_t START_CODE_BYTE = 32;
SHA512_CTX g_c;
...
constexpr int32_t MIN_RANGE = 176;
constexpr int32_t MIN_HEIGHT_RANGE = 144;
...
void VDecAPI11Sample::CleanupResources()
{
...
int32_t VDecAPI11Sample::ConfigureVideoDecoder()
{
    ...
    if (useHDRSource) {
        (void)OH_AVFormat_SetIntValue(trackFormat, OH_MD_KEY_PROFILE, defaultProfile);
    }
...
{
    signal_ = new (std::nothrow) VDecAPI11Signal();
    if (signal_ == nullptr) {
...
void VDecAPI11Sample::InputForAV1FuncTest()
{
    if (outputYuvSurface) {
        g_yuvSurface = true;
    }
    ...
    while (flag) {
        ...
        lock.unlock();
        if (!inFile_->eof()) {
            int ret = PushDataForAV1(index, buffer);
            if (ret == 1) {
                flag = false;
                break;
            }
...
int32_t VDecAPI11Sample::PushDataForAV1(uint32_t index, OH_AVBuffer *buffer)
{
    if (beforeEosInput && frameCount_ > TEN) {
        SetEOS(index, buffer);
        return 1;
    ...
    if (beforeEosInputInput && frameCount_ > TEN) {
        OH_AVCodecBufferAttr attr;
        SecureMemset(&attr, sizeof(attr), 0, sizeof(attr));
        attr.flags = AVCODEC_BUFFER_FLAGS_EOS;
        beforeEosInputInput = false;
    }

    char fhdr[12] = {0};
    (void)inFile_->read(fhdr, sizeof(fhdr));
    if (inFile_->eof() || inFile_->gcount() != (int)sizeof(fhdr)) {
        SetEOS(index, buffer);
        return 1;
    }

    uint32_t frameSize = static_cast<uint32_t>(
        (uint8_t)fhdr[0] | ((uint8_t)fhdr[1] << 8) | ((uint8_t)fhdr[2] << 16) | ((uint8_t)fhdr[3] << 24)
    );
...
int32_t VDecAPI11Sample::StartVideoDecoderForAV1()
{
    isRunning_.store(true);
    int ret = OH_VideoDecoder_Start(vdec_);
    if (ret != AV_ERR_OK) {
    ...
    char ivfHeader[32] = {0};
    inFile_->read(ivfHeader, sizeof(ivfHeader));
    if (inFile_->gcount() != (int)sizeof(ivfHeader) ||
        std::string(ivfHeader, ivfHeader + DKIF_SIZE) != "DKIF") {
        cout << "not a valid IVF file " << inpDir << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
        inFile_->close();
        inFile_.reset();
    ...
    return InputOutputLoopForAv1();
}

int32_t VDecAPI11Sample::InputOutputLoopForAv1()
{
    inputLoop_ = make_unique<thread>(&VDecAPI11Sample::InputForAV1FuncTest, this);
    if (inputLoop_ == nullptr) {
        cout << "Failed to create input loop" << endl;
        isRunning_.store(false);
        (void)OH_VideoDecoder_Stop(vdec_);
...
int32_t VDecAPI11Sample::StartVideoDecoderReadStream()
{
    ...
    inFile_->seekg(START_CODE_BYTE, std::ios::beg);
...
uint32_t VDecAPI11Sample::SendData(uint32_t bufferSize, uint32_t index, OH_AVBuffer *buffer)
{
    ...
    if (SecureMemcpy(avBuffer, size, fileBuffer, bufferSize) != EOK) {
        delete[] fileBuffer;
        return 0;
    }
    ...
    if (attr.flags & AVCODEC_BUFFER_FLAGS_EOS) {
        ...
        if (!sfOutput && !nocaleHash) {
            ...
            if (defualtPixelFormat == AV_PIXEL_FORMAT_NV12) {
                source = fileSourcesha256Vc1;
            } else if (defualtPixelFormat == AV_PIXEL_FORMAT_NV21) {
                source = fileSourcesha256Nv21;
            } else {
                source = fileSourcesha256Yuvi;
            }
            if (!MdCompare(g_md, SHA512_DIGEST_LENGTH, source)) {
                cout << "mdcompare filed " << endl;
            }
        }
        return -1;
    }
...
void VDecAPI11Sample::ProcessOutputData(OH_AVBuffer *buffer, uint32_t index, int32_t size)
{
    if (!sfOutput) {
        if (size >= ((defaultWidth * defaultHeight * THREE) >> 1)) {
            ...
            if (SecureMemcpy(cropBuffer, size, OH_AVBuffer_GetAddr(buffer),
                             defaultWidth * defaultHeight) != EOK) {
                cout << "Fatal: memory copy failed Y" << endl;
            }
            uint32_t uvSize = size - defaultWidth * defaultHeight;
            if (SecureMemcpy(cropBuffer + defaultWidth * defaultHeight, uvSize,
...
void VDecAPI11Sample::GetFormat(const char *fileName)
{
    ...
    cout << fileName << "-------" << gFd << "-------" << size << endl;
    videoSource = OH_AVSource_CreateWithFD(gFd, 0, size);
    if (videoSource == nullptr) {
        cout << "Error: videoSource is nullptr" << endl;
        return;
    }
    demuxer = OH_AVDemuxer_CreateWithSource(videoSource);
    if (demuxer == nullptr) {
        cout << "Error: demuxer is nullptr" << endl;
        return;
    }
    sourceFormat = OH_AVSource_GetSourceFormat(videoSource);
    if (sourceFormat == nullptr) {
        cout << "Error: sourceFormat is nullptr" << endl;
        return;
    }
    if (!OH_AVFormat_GetIntValue(sourceFormat, OH_MD_KEY_TRACK_COUNT, &gTrackCount)) {
        cout << "Error: failed to get track count" << endl;
        return;
    }
    cout << "gTrackCount----" << gTrackCount << endl;
    for (int32_t index = 0; index < gTrackCount; index++) {
        if (OH_AVDemuxer_SelectTrackByID(demuxer, index) != AV_ERR_OK) {
            cout << "Error: failed to select track " << index << endl;
        }
    }
    for (int32_t index = 0; index < gTrackCount; index++) {
        trackFormat = OH_AVSource_GetTrackFormat(videoSource, index);
        if (trackFormat == nullptr) {
            cout << "Error: trackFormat is nullptr" << endl;
            continue;
        }
        if (!OH_AVFormat_GetIntValue(trackFormat, OH_MD_KEY_TRACK_TYPE, &trackType)) {
            cout << "Error: failed to get track type" << endl;
            continue;
        }
