...
void main(void)
{
    ...
    {
        vec3 baseColor = col;
        float dist = length(uMyData.vec_2);
        float t = step(1.0, dist);
        col = t * baseColor;
    }
    ...
    {
        vec3 baseColor = col;
        float dist = length(uMyData.vec_3);
        float gray = 1.0;
        vec3 bwColor = vec3(1.0, 0.0, 0.0); 
        float t = smoothstep(0.0, 1.414, dist);
        col = mix(bwColor, baseColor, t);
    }
    ...
    col*=baseColor.xyz;
    //col*=texture(uUserCustomImage0, inUv.xy).xyz;
