...
OH_NativeBuffer_Format bufferFormatType[] = {
    ...
    NATIVEBUFFER_PIXEL_FMT_Y8,           NATIVEBUFFER_PIXEL_FMT_Y16,
};
...
uint64_t usageType[] = {
    ...
    NATIVEBUFFER_USAGE_ALIGNMENT_512, NATIVEBUFFER_USAGE_MEM_MMZ_CACHE, NATIVEBUFFER_USAGE_CPU_READ
    | NATIVEBUFFER_USAGE_CPU_WRITE | NATIVEBUFFER_USAGE_MEM_DMA | NATIVEBUFFER_USAGE_HW_RENDER
    | NATIVEBUFFER_USAGE_HW_TEXTURE | NATIVEBUFFER_USAGE_CPU_READ_OFTEN
    | NATIVEBUFFER_USAGE_ALIGNMENT_512 | NATIVEBUFFER_USAGE_MEM_MMZ_CACHE,
};
...
{
    napi_value result = nullptr;
    for (int i = 0; i < nativeBuffeColorSpace.size(); ++i) {
        OH_NativeBuffer_ColorSpace colorSpace1 = nativeBuffeColorSpace[i];
        InitNativeWindow *initNative = new InitNativeWindow();
        OHNativeWindow *nativeWindow = nullptr;
        nativeWindow = initNative->returnNativeWindow();
        NAPI_ASSERT(env, nativeWindow != nullptr, "nativeWindow == nullptr");
        int32_t flag = OH_NativeWindow_SetColorSpace(nativeWindow, colorSpace1);
        NAPI_ASSERT(env, flag == NATIVE_ERROR_OK, "flag != NATIVE_ERROR_OK");
        OH_NativeBuffer_ColorSpace colorSpace2;
        flag = OH_NativeWindow_GetColorSpace(nativeWindow, &colorSpace2);
        NAPI_ASSERT(env, flag == NATIVE_ERROR_OK, "flag != NATIVE_ERROR_OK");
...
napi_value testNativeWindowSetMetadataValue_metadata_type(napi_env env, napi_callback_info info)
{
    ...
    for (int i = 0; i <= sizeof(hdr_type) / sizeof(hdr_type[0]) - 1; i++) {
        ...
        int32_t errorCode =
            OH_NativeWindow_SetMetadataValue(nativeWindow, OH_HDR_METADATA_TYPE, sizeof(uint8_t), &type);
        if (errorCode != SUCCESS) {
            napi_create_int32(env, CONSTANT_1000 * i + CONSTANT_1, &result);
            return result;
        }
        errorCode = OH_NativeWindow_GetMetadataValue(nativeWindow, OH_HDR_METADATA_TYPE, &bufferSize, &metadata);
        if (*metadata != type) {
    ...
    int32_t errorCode = OH_NativeWindow_SetMetadataValue(nativeWindow, OH_HDR_METADATA_TYPE, sizeof(uint8_t), &type);
    if (errorCode != SUCCESS) {
        napi_create_int32(env, FAIL, &result);
        return result;
    }
