...
static napi_value EglSwapBuffersWithDamageKHRRect1(napi_env env, napi_callback_info info)
{
    EGLDisplay m_eglDisplay = nullptr;
    EGLConfig m_eglConf = nullptr;
    EGLint numConfigs;
    ...
    EGLint rects[] = {10, 10, 100, 100}; // x,y,width,height
...
static napi_value EglSwapBuffersWithDamageKHRSurfaceRectsNull(napi_env env, napi_callback_info info)
{
    EGLDisplay m_eglDisplay = nullptr;
    EGLConfig m_eglConf = nullptr;
    EGLint numConfigs;
    ...
    EGLSurface m_eglSurface = nullptr;
    EGLint *rects = nullptr;
    PFNEGLSWAPBUFFERSWITHDAMAGEKHRPROC eglSwapBuffersWithDamageKHR_ =
        (PFNEGLSWAPBUFFERSWITHDAMAGEKHRPROC)eglGetProcAddress("eglSwapBuffersWithDamageKHR");
    EGLint n_RectsDamageArea = 0;
    m_eglSurface = nullptr;
    if (eglSwapBuffersWithDamageKHR_) {
        EGLBoolean ret = eglSwapBuffersWithDamageKHR_(m_eglDisplay, m_eglSurface, rects, n_RectsDamageArea);
        NAPI_ASSERT(env, ret == EGL_FALSE, "eglSwapBuffersWithDamageKHR update null area error");
    }
...
{
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "eglndk", "case EglClientWaitSync in");
    (void)info;
    MyEGLWindow myEGLWindow = {EGL_NO_DISPLAY, EGL_NO_SURFACE, EGL_NO_CONTEXT};
    createMyEglWindow(env, &myEGLWindow);
    if (myEGLWindow.eglVersion >= GL_VERSION_15) {
        ...
        EGLint waitResult = eglClientWaitSync(myEGLWindow.eglDisplay, eglSync, 0x00, EGL_FOREVER);
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "eglndk", "eglClientWaitSync waitResult: %{public}zu",
            waitResult);
        EGLint eglError = eglGetError();
        OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "eglndk", "eglGetError: %{public}zu", eglError);
        NAPI_ASSERT(env, waitResult == EGL_CONDITION_SATISFIED, "eglClientWaitSync error");
...
static napi_value Init(napi_env env, napi_value exports)
{
    napi_property_descriptor desc[] = {
        ...
        {"eglSwapBuffersWithDamageKHRRect2", nullptr, EglSwapBuffersWithDamageKHRRect2, nullptr, nullptr, nullptr,
         napi_default, nullptr},
        {"EglSwapBuffersWithDamageKHRSurfaceRectsNull", nullptr, EglSwapBuffersWithDamageKHRSurfaceRectsNull, nullptr,
